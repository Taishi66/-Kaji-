...
- Core crates: `kaji` (agent logic and ACP server), `kaji-cli` (CLI), `kaji-mcp` (MCP servers)
- Error handling: Use `anyhow::Result`, not `unwrap()` in production code
- Async runtime: tokio
- MCP protocol implementations require extra scrutiny
- Naming convention: In `documentation/docs` and `documentation/blog`, always refer to the project as "kaji" (lowercase), never "Kaji" (even at the start of sentences)
