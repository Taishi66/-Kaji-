//! Generate manpages for the kaji CLI.
//!
...
//! offline documentation accessible via the `man` command (e.g., `man kaji`).
//!
//! When kaji is packaged for Linux distributions (deb, rpm, etc.), the generated
...
//!
//! Output: target/man/kaji.1, target/man/kaji-session.1, etc.
...
fn generate_see_also(
    ...
    // Always reference the main kaji command if we're not it
    if current_name != "kaji" {
        references.push("kaji".to_string());
    }

    // Reference parent command if exists and not already added
    if let Some(parent) = parent_name {
        if parent != "kaji" && !references.contains(&parent.to_string()) {
    ...
    if current_name == "kaji" {
        ...
        for subcmd in cmd.get_subcommands() {
            ...
            if subcmd_name != "help" && !subcmd.is_hide_set() {
                let full_name = format!("kaji-{}", subcmd_name);
