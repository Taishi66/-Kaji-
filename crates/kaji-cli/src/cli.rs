...
use kaji::posthog::get_telemetry_choice;
...
impl From<ServePlatform> for KajiPlatform {
    fn from(platform: ServePlatform) -> Self {
        match platform {
            ServePlatform::Cli => KajiPlatform::KajiCli,
            ServePlatform::Desktop => KajiPlatform::KajiDesktop,
        }
    }
}
...
pub struct SessionOptions {
    ...
    #[arg(
        ...
        long_help = "Run extensions (stdio and built-in) inside the specified container. The extension must exist in the container. For built-in extensions, kaji must be installed inside the container."
    )]
...
fn parse_streamable_http_extension(input: &str) -> Result<StreamableHttpOptions, String> {
    ...
    let (mut url, mut timeout) = (String::new(), kaji::config::DEFAULT_EXTENSION_TIMEOUT);
...
pub struct ExtensionOptions {
    ...
    #[arg(
        ...
        long_help = "Add one or more builtin extensions that are bundled with kaji by specifying their names, comma-separated",
        value_delimiter = ','
    )]
...
pub struct InputOptions {
    ...
    #[arg(
        ...
        long = "text",
        value_name = "TEXT",
        help = "Input text to provide to kaji directly",
        long_help = "Input text containing commands for kaji. Use this in lieu of the instructions argument.",
    ...
    #[arg(
        ...
        help = "Dynamic parameters (e.g., --params username=alice --params channel_name=kaji-channel)",
...
pub struct ModelOptions {
    ...
    #[arg(
        ...
        long_help = "Override the KAJI_PROVIDER environment variable for this run. Available providers include openai, anthropic, ollama, databricks, gemini-cli, claude-code, and others."
    )]
    ...
    #[arg(
        ...
        long_help = "Override the KAJI_MODEL environment variable for this run. The model must be supported by the specified provider."
    )]
...
enum SessionCommand {
    ...
    Export {
        ...
        #[arg(
            long = "nostr",
            help = "Publish the JSON session export as an encrypted Nostr event and print a Kaji share link"
        )]
        nostr: bool,
    ...
    Import {
        #[arg(
            help = "Path to a kaji session export, a Claude Code, Codex, or Pi .jsonl transcript, or a kaji://sessions/nostr share link"
        )]
        input: String,
...
#[derive(Subcommand)]
enum SkillsCommand {
    /// List all skills available to the kaji agent
    #[command(about = "List all skills available to the kaji agent")]
    List,
}
...
enum RecipeCommand {
    ...
    #[command(about = "Open a recipe in Kaji Desktop")]
    Open {
...
#[derive(Subcommand)]
enum Command {
    /// Configure kaji settings
    #[command(about = "Configure kaji settings")]
    Configure {},

    /// Display kaji configuration information
    #[command(about = "Display kaji information")]
    Info {
    ...
    /// Run kaji as an ACP (Agent Client Protocol) agent
    #[command(about = "Run kaji as an ACP agent server on stdio")]
    Acp {
    ...
    Serve {
        ...
        #[arg(
            ...
            help = "Start the ACP endpoint without requiring KAJI_SERVER__SECRET_KEY"
        )]
    ...
    /// Update the kaji CLI version
    #[cfg(feature = "update")]
    #[command(about = "Update the kaji CLI version")]
    Update {
        ...
        #[arg(
            ...
            long_help = "Update to the latest canary version of the kaji CLI, otherwise updates to the latest stable version."
        )]
        canary: bool,

        /// Enforce to re-configure kaji during update
        #[arg(short, long, help = "Enforce to re-configure kaji during update")]
        reconfigure: bool,
    },

    /// Terminal-integrated session (one session per terminal)
    #[command(
        about = "Terminal-integrated kaji session",
        long_about = "Runs a kaji session tied to your terminal window.\n\
                      Each terminal maintains its own persistent session that resumes automatically.\n\n\
                      Setup:\n  \
                        eval \"$(kaji term init zsh)\"  # zsh/bash\n  \
                        let init = ($nu.cache-dir | path join \"kaji-term-init.nu\"); ^kaji term init nu | save --force $init; source $init\n\n\
                      Usage:\n  \
                        kaji term run \"list files in this directory\"\n  \
                        @kaji \"create a python script\"  # using alias\n  \
    ...
    Completion {
        ...
        #[arg(long, default_value = "kaji", help = "Provide a custom binary name")]
        bin_name: String,
    },
    ...
    /// runs the review through kaji.
    #[command(about = "Review the current diff using kaji")]
    Review {
        ...
        /// orchestrator dispatches one `kaji run` subprocess per check
...
enum TermCommand {
    ...
    #[command(
        ...
        long_about = "Prints shell configuration to set up terminal-integrated sessions.\n\
                      Each terminal gets a persistent kaji session that automatically resumes.\n\n\
                      Setup:\n  \
                        echo 'eval \"$(kaji term init zsh)\"' >> ~/.zshrc\n  \
                        source ~/.zshrc\n\n\
                        Nushell:\n  \
                        let init = ($nu.cache-dir | path join \"kaji-term-init.nu\")\n  \
                        ^kaji term init nu | save --force $init\n  \
                        source $init\n\n\
                      With --default (anything typed that isn't a command goes to kaji):\n  \
                        echo 'eval \"$(kaji term init zsh --default)\"' >> ~/.zshrc\n  \
                        ^kaji term init nu --default | save --force $init"
    )]
    Init {
        ...
        /// Make kaji the default handler for unknown commands
        #[arg(
            long = "default",
            help = "Make kaji the default handler for unknown commands",
            long_help = "When enabled, anything you type that isn't a valid command will be sent to kaji. Supported for zsh, bash, and nu."
        )]
        default: bool,
    },
    ...
    #[command(
        ...
        long_about = "Run a prompt in the terminal-integrated session.\n\n\
                      Examples:\n  \
                        kaji term run list files in this directory\n  \
                        @kaji list files  # using alias\n  \
                        @g why did that fail  # short alias"
    )]
    Run {
        /// The prompt to send to kaji (multiple words allowed without quotes)
...
async fn handle_serve_command(args: ServeCommandArgs) -> Result<()> {
    ...
    use kaji::acp::server::AcpBuiltinSelection;
    use kaji::acp::server_factory::{AcpServer, AcpServerFactoryConfig};
    use kaji::acp::transport::create_router;
    ...
    let server = Arc::new(AcpServer::new(AcpServerFactoryConfig {
        ...
        kaji_platform: platform.into(),
        additional_source_roots,
        enable_scheduler,
    }));
    let env_secret = std::env::var(KAJI_SERVER_SECRET_KEY_ENV)
        .ok()
    ...
    if !require_token && !dangerously_unauthenticated {
        anyhow::bail!(
            "{KAJI_SERVER_SECRET_KEY_ENV} must be set to start `kaji serve`; pass --dangerously-unauthenticated to run without ACP authentication"
        );
    }
    if dangerously_unauthenticated && !require_token {
        warn!(
            "{KAJI_SERVER_SECRET_KEY_ENV} is not set and --dangerously-unauthenticated was passed; the ACP endpoint will accept unauthenticated connections"
        );
    }
    ...
    let tls_cert_path =
        tls_cert_path.or_else(|| config.get_param::<String>("KAJI_TLS_CERT_PATH").ok());
    let tls_key_path =
        tls_key_path.or_else(|| config.get_param::<String>("KAJI_TLS_KEY_PATH").ok());
    let tls = tls
        || config.get_param::<bool>("KAJI_TLS").unwrap_or(false)
    ...
    if tls {
        ...
        {
            let tls_setup = kaji::acp::transport::tls::setup_tls(
...
async fn handle_interactive_session(
    ...
    tracing::info!(
        monotonic_counter.kaji.session_starts = 1,
        session_type,
        interactive = true,
        "Session started"
    );
    ...
    let mut session_id = get_or_create_session_id(identifier, resume, false, kaji_mode).await?;
...
async fn log_session_completion(
    ...
    tracing::info!(
        monotonic_counter.kaji.session_completions = 1,
        session_type,
        exit_type,
    ...
    tracing::info!(
        monotonic_counter.kaji.session_duration_ms = session_duration.as_millis() as u64,
        session_type,
        "Session duration"
    );
    ...
    if total_tokens > 0 {
        tracing::info!(
            monotonic_counter.kaji.session_tokens = total_tokens,
            session_type,
            "Session tokens"
        );
    }
    ...
    match (
        ...
        (Some(file), _, _) => {
            let contents = std::fs::read_to_string(file).unwrap_or_else(|err| {
                eprintln!(
                    "Instruction file not found — did you mean to use kaji run --text?\n{}",
                    err
                );
                std::process::exit(1);
            });
        ...
        (_, _, Some(recipe_name)) => {
            ...
            let recipe_version = crate::recipes::search_recipe::load_recipe_file(recipe_name)
                .ok()
                .and_then(|rf| {
                    kaji::recipe::template_recipe::parse_recipe_content(
                        &rf.content,
            ...
            tracing::info!(
                monotonic_counter.kaji.recipe_runs = 1,
...
#[cfg(feature = "local-inference")]
fn print_download_progress(manager: &kaji::download_manager::DownloadManager) {
    let Some(progress) = manager
        .list_progress()
        .into_iter()
        .find(|progress| progress.status == kaji::download_manager::DownloadStatus::Downloading)
    else {
        return;
    };
...
async fn handle_local_models_command(command: LocalModelsCommand) -> Result<()> {
    use kaji::providers::local_inference::hf_models;
    use kaji::providers::local_inference::local_model_registry::get_registry;

    kaji::providers::local_inference::configure_huggingface_auth();
    ...
    match command {
            ...
            for model in &results {
                ...
                for variant in &model.variants {
                    ...
                    if variant.supported {
                        println!(
                            "    Download: kaji local-models download '{}'",
                            variant.download_id
                        );
                    }
                }
        ...
        LocalModelsCommand::Download { spec } => {
            ...
            let manager = kaji::download_manager::get_download_manager();
...
async fn handle_default_session() -> Result<()> {
    ...
    let kaji_mode = Config::global().get_kaji_mode().unwrap_or_default();
    let session_id = get_or_create_session_id(None, false, false, kaji_mode).await?;
...
pub async fn cli() -> anyhow::Result<()> {
    ...
    tracing::info!(
        monotonic_counter.kaji.cli_commands = 1,
    ...
    match cli.command {
        ...
        Some(Command::Acp {
            builtins,
            enable_scheduler,
        }) => kaji::acp::server::run(builtins, enable_scheduler).await,
        Some(Command::Serve {
            host,
            port,
            tls,
        ...
        Some(Command::ValidateExtensions { file }) => {
            use kaji::agents::validate_extensions::validate_bundled_extensions;
...
mod tests {
    ...
    fn completion_command_accepts_nushell_alias() {
        let cli = Cli::try_parse_from(["kaji", "completion", "nushell"]).expect("parse failed");
    ...
    fn nushell_completion_generation_emits_module() {
        ...
        CompletionShell::Nu.generate(&mut cmd, "kaji", &mut buffer);

        let script = String::from_utf8(buffer).expect("utf8");
        assert!(script.contains("module completions"));
        assert!(script.contains("export extern kaji"));
    ...
    fn term_init_help_mentions_nushell() {
        ...
        assert!(help.contains("kaji term init nu"));
    ...
    fn skills_command_accepts_list_subcommand() {
        let cli = Cli::try_parse_from(["kaji", "skills", "list"]).expect("parse failed");
