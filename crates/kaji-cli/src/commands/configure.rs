use crate::recipes::github_recipe::KAJI_RECIPE_GITHUB_REPO_CONFIG_KEY;
use cliclack::spinner;
use console::style;
use kaji::agents::extension::{ToolInfo, PLATFORM_EXTENSIONS};
use kaji::agents::extension_manager::get_parameter_names;
use kaji::agents::Agent;
use kaji::agents::{extension::Envs, ExtensionConfig};
use kaji::config::declarative_providers::{
    create_custom_provider, remove_custom_provider, CreateCustomProviderParams,
};
use kaji::config::extensions::{
...
use kaji::config::signup_tetrate::TetrateAuth;
use kaji::config::{
    configure_tetrate, Config, ConfigError, ExperimentManager, ExtensionEntry, KajiMode,
    PermissionManager,
};
#[cfg(feature = "telemetry")]
use kaji::posthog::{get_telemetry_choice, TELEMETRY_ENABLED_KEY};
use kaji::providers::base::ConfigKey;
use kaji::providers::provider_test::test_provider_configuration;
use kaji::providers::{create, providers, retry_operation, RetryConfig};
...
pub async fn handle_configure() -> anyhow::Result<()> {
    if !std::io::stdin().is_terminal() {
        anyhow::bail!(
            "kaji configure requires an interactive terminal.\n\
             If you installed via 'curl ... | bash', run 'kaji configure' separately after installation."
        );
    }
...
pub fn configure_telemetry_consent_dialog() -> anyhow::Result<bool> {
    ...
    println!();
    println!("{}", style("Help improve kaji").bold());
    println!();
    println!(
        "{}",
        style("Would you like to help improve kaji by sharing anonymous usage data?").dim()
    );
    println!(
        "{}",
        style("This helps us understand how kaji is used and identify areas for improvement.")
            .dim()
    );
    println!();
    ...
    println!("{}", style("  • kaji version and install method").dim());
    ...
    println!(
        "{}",
        style("or any personal data. You can change this anytime with 'kaji configure'.").dim()
    );
    println!();

    let enabled = cliclack::confirm("Share anonymous usage data to help improve kaji?")
        .initial_value(true)
        .interact()?;
    ...
    if enabled {
        let _ = cliclack::log::success("Thank you for helping improve kaji!");
    } else {
...
async fn handle_first_time_setup(config: &Config) -> anyhow::Result<()> {
    println!();
    println!("{}", style("Welcome to kaji! Let's get you set up.").dim());
    println!(
        "{}",
    ...
    println!();
    cliclack::intro(style(" kaji-configure ").on_cyan().black())?;
...
async fn handle_existing_config() -> anyhow::Result<()> {
    ...
    let action = cliclack::select("What would you like to configure?")
        ...
        .item(
            "settings",
            "kaji settings",
            "Set the kaji mode, Tool Output, Tool Permissions, Experiment, kaji recipe github repo and more",
        )
        .interact()?;
...
fn interactive_model_search(
    models: &[String],
    provider_meta: &kaji::providers::base::ProviderMetadata,
    ...
    match config.set_secret(key_name, &value) {
        ...
        Err(e) => {
            cliclack::outro(style(format!(
                "Failed to store {} securely: {}. Please ensure your system's secure storage is accessible. Alternatively you can run with KAJI_DISABLE_KEYRING=true or set the key in your environment variables",
                key_name, e
            )).on_red().white())?;
            Ok(false)
        }
...
pub async fn configure_provider_dialog() -> anyhow::Result<bool> {
    ...
    let current_provider: Option<String> = config.get_kaji_provider().ok();
    ...
    let models_res = retry_operation(&RetryConfig::default(), || async {
        temp_provider
            .fetch_recommended_models(kaji::model_config::global_toolshim())
            .await
    })
    .await;
    ...
    let model: String = match models_res {
        ...
        Ok(_) => {
            let default_model =
                std::env::var("KAJI_MODEL").unwrap_or(provider_meta.default_model.clone());
...

    {
        let supports_thinking = match temp_provider.fetch_model_info(&model).await {
            ...
            Err(_) => kaji_providers::model::ModelConfig::new(&model).is_reasoning_model(),
        };
        ...
        if supports_thinking {
            ...
            config.set_kaji_thinking_effort(effort)?;
        }
    }
    ...
    let toolshim_enabled = std::env::var("KAJI_TOOLSHIM")
        .map(|val| val == "1" || val.to_lowercase() == "true")
        .unwrap_or(false);
    let toolshim_model = std::env::var("KAJI_TOOLSHIM_OLLAMA_MODEL").ok();

    match test_provider_configuration(&provider_name, &model, toolshim_enabled, toolshim_model)
        .await
    {
        Ok(()) => {
            kaji::config::set_active_provider(config, &provider_name, &model)?;
...
fn prompt_extension_timeout() -> anyhow::Result<u64> {
    Ok(
        cliclack::input("Please set the timeout for this tool (in secs):")
            .placeholder(&kaji::config::DEFAULT_EXTENSION_TIMEOUT.to_string())
...
fn configure_stdio_extension() -> anyhow::Result<()> {
    ...
    let mut parts = kaji::utils::split_command_args(&command_str)?;
...
pub fn configure_extensions_dialog() -> anyhow::Result<()> {
    let extension_type = cliclack::select("What type of extension would you like to add?")
        .item(
            "built-in",
            "Built-in Extension",
            "Use an extension that comes with kaji",
        )
        .item(
            "stdio",
...
pub async fn configure_settings_dialog() -> anyhow::Result<()> {
    ...
    let setting_type = setting_select
        ...
        .item(
            ...
            "kaji will pull recipes from this repo if not found locally.",
        )
        .interact()?;
...
pub fn configure_kaji_mode_dialog() -> anyhow::Result<()> {
    let config = Config::global();

    if std::env::var("KAJI_MODE").is_ok() {
        let _ = cliclack::log::info(
            "Notice: KAJI_MODE environment variable is set and will override the configuration here.",
        );
    }

    let mode = cliclack::select("Which kaji mode would you like to configure?")
        .item(
            KajiMode::Auto,
            "Auto Mode",
    ...
    config.set_kaji_mode(mode)?;
    let msg = match mode {
        KajiMode::Auto => "Set to Auto Mode - full file modification enabled",
        KajiMode::Approve => "Set to Approve Mode - all tools and modifications require approval",
        KajiMode::SmartApprove => "Set to Smart Approve Mode - modifications require approval",
        KajiMode::Chat => "Set to Chat Mode - no tools or modifications enabled",
    };
    cliclack::outro(msg)?;
    Ok(())
}
...
pub fn configure_telemetry_dialog() -> anyhow::Result<()> {
    ...
    if std::env::var("KAJI_TELEMETRY_OFF").is_ok() {
        let _ = cliclack::log::info(
            "Notice: KAJI_TELEMETRY_OFF environment variable is set and will override the configuration here.",
        );
    }
    ...
    if enabled {
        cliclack::outro("Telemetry enabled - thank you for helping improve kaji!")?;
    } else {
...
pub fn configure_tool_output_dialog() -> anyhow::Result<()> {
    ...
    if std::env::var("KAJI_CLI_MIN_PRIORITY").is_ok() {
        let _ = cliclack::log::info(
            "Notice: KAJI_CLI_MIN_PRIORITY environment variable is set and will override the configuration here.",
        );
    }
    ...
    match tool_log_level {
        "high" => {
            config.set_param("KAJI_CLI_MIN_PRIORITY", 0.8)?;
            cliclack::outro("Showing tool output of high importance only.")?;
        }
        "medium" => {
            config.set_param("KAJI_CLI_MIN_PRIORITY", 0.2)?;
            cliclack::outro("Showing tool output of medium importance.")?;
        }
        "all" => {
            config.set_param("KAJI_CLI_MIN_PRIORITY", 0.0)?;
...
pub fn configure_keyring_dialog() -> anyhow::Result<()> {
    ...
    if std::env::var("KAJI_DISABLE_KEYRING").is_ok() {
        let _ = cliclack::log::info(
            "Notice: KAJI_DISABLE_KEYRING environment variable is set and will override the configuration here.",
        );
    }

    let currently_disabled = config.get_param::<String>("KAJI_DISABLE_KEYRING").is_ok();
    ...
    match storage_option {
        "keyring" => {
            ...
            config.set_param("KAJI_DISABLE_KEYRING", Value::String("".to_string()))?;
            cliclack::outro("Secret storage set to system keyring (secure)")?;
            let _ =
                cliclack::log::info("You may need to restart kaji for this change to take effect");
        }
        "file" => {
            // Set the disable flag to use file storage
            config.set_param("KAJI_DISABLE_KEYRING", Value::String("true".to_string()))?;
            cliclack::outro(format!(
...
/// Configure experiment features that can be used with kaji
...
pub async fn configure_tool_permissions_dialog() -> anyhow::Result<()> {
    ...
    let model_config = kaji::model_config::model_config_from_user_config(&provider_name, &model)?;
...
fn configure_recipe_dialog() -> anyhow::Result<()> {
    let key_name = KAJI_RECIPE_GITHUB_REPO_CONFIG_KEY;
    let config = Config::global();
    let default_recipe_repo = std::env::var(key_name)
        .ok()
        .or_else(|| config.get_param(key_name).unwrap_or(None));
    let mut recipe_repo_input =
        cliclack::input("Enter your kaji recipe GitHub repo (owner/repo): eg: my_org/kaji-recipes")
            .required(false);
...
pub fn configure_max_turns_dialog() -> anyhow::Result<()> {
    ...
    let current_max_turns: u32 = config.get_param("KAJI_MAX_TURNS").unwrap_or(1000);
    ...
    config.set_param("KAJI_MAX_TURNS", max_turns)?;

    cliclack::outro(format!(
        "Set maximum turns to {} - kaji will ask for input after {} consecutive actions",
        max_turns, max_turns
    ))?;
...
pub async fn handle_openrouter_auth() -> anyhow::Result<()> {
    use kaji::config::{configure_openrouter, signup_openrouter::OpenRouterAuth};
    use kaji::conversation::message::Message;
    use kaji::providers::create;
    ...
    let configured_model: String = config.get_kaji_model()?;
    let model_config =
        match kaji::model_config::model_config_from_user_config("openrouter", &configured_model) {
            Ok(config) => config,
            Err(e) => {
        ...
        Ok(provider) => {
            ...
            match test_result {
                Ok(_) => {
                    ...
                    if !has_developer {
                        set_extension(ExtensionEntry {
                            ...
                            config: ExtensionConfig::Platform {
                                ...
                                display_name: Some(kaji::config::DEFAULT_DISPLAY_NAME.to_string()),
                                bundled: Some(true),
                                available_tools: Vec::new(),
                            },
                        });
                    ...
                    cliclack::outro("OpenRouter setup complete! You can now use kaji.")?;
                }
                Err(e) => {
...
pub async fn handle_tetrate_auth() -> anyhow::Result<()> {
    ...
    println!("\nTesting configuration...");
    let configured_model: String = config.get_kaji_model()?;
    if let Err(e) = kaji::model_config::model_config_from_user_config("tetrate", &configured_model)
    {
        eprintln!("⚠️  Invalid model configuration: {}", e);
        eprintln!("Your settings have been saved. Please check your model configuration.");
        return Ok(());
...
fn add_provider() -> anyhow::Result<()> {
    ...
    if !provider_config.models.is_empty() {
        ...
        if let Ok(model) = cliclack::select("Which model should be the default?")
            ...
            .interact()
        {
            config.set_kaji_provider(&provider_config.name)?;
            config.set_kaji_model(model)?;
        }
    }
...
async fn remove_provider() -> anyhow::Result<()> {
    let custom_providers_dir = kaji::config::declarative_providers::custom_providers_dir();
    let custom_providers = if custom_providers_dir.exists() {
        kaji::config::declarative_providers::load_custom_providers(&custom_providers_dir)?
    } else {
        Vec::new()
    };
    ...
    if let Err(e) = kaji::providers::cleanup_provider(selected_id).await {
