use anyhow::Result;
use kaji::execution::manager::AgentManager;
use kaji::gateway::manager::GatewayManager;
use std::sync::Arc;
...
pub async fn handle_gateway_start(
    ...
    let mut config = kaji::gateway::GatewayConfig {
        gateway_type,
        platform_config,
        max_sessions: 0,
    };

    let gw = kaji::gateway::create_gateway(&mut config)?;
