    ...
    match generate_deeplink(recipe_name, params_map) {
        Ok((deeplink_url, recipe)) => match opener(&deeplink_url) {
            ...
            Err(err) => {
                writeln!(
                    out,
                    "{} Failed to open recipe in Kaji Desktop: {}",
                    style("✗").red().bold(),
                    err
                )?;
                writeln!(out, "Generated deeplink: {}", deeplink_url)?;
                writeln!(out, "You can manually copy and open the URL above, or ensure Kaji Desktop is installed.")?;
...
"#;
    ...
    fn test_handle_deeplink_valid_recipe() {
        ...
        assert!(url.starts_with("kaji://recipe?config="));
        let encoded_part = url.strip_prefix("kaji://recipe?config=").unwrap();
    ...
    fn test_handle_open_opener_fails() {
        ...
        assert!(result.is_err());
        assert!(output.contains("Failed to open recipe in Kaji Desktop"));
