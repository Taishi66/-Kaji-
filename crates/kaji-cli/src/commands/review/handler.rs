...
use kaji::checks::{discover, DiscoveredReview};
use kaji::subprocess::git_command;
...
/// Entry point for the `kaji review` subcommand.
pub async fn handle_review(opts: ReviewOptions) -> Result<()> {
    ...
    if diff.trim().is_empty() {
        eprintln!("kaji review: no changes to review");
        return Ok(());
    }
    ...
    if !use_orchestrator {
        ...
        if opts.checks_only {
            ...
            if !opts.quiet {
                ...
                eprintln!(
                    "kaji review: emitted {total_emitted} finding(s) from {} check(s) ({suppressed} hidden below severity={:?})",
                    discovered.checks.len(),
                    min_sev
                );
            }
    ...
    if !opts.quiet {
        ...
        if suppressed == 0 {
            eprintln!(
                "kaji review: orchestrator emitted {total_emitted} finding(s) from {} check(s) (main: {main_pass_label}, {} finding(s))",
                discovered.checks.len(),
                main_findings.len()
            );
        } else {
            eprintln!(
                "kaji review: orchestrator emitted {total_emitted} finding(s) from {} check(s) (main: {main_pass_label}, {} finding(s); {suppressed} hidden below severity={:?})",
                discovered.checks.len(),
                main_findings.len(),
                min_sev
            );
...
fn print_discovered_summary(d: &DiscoveredReview) {
    if d.checks.is_empty() {
        eprintln!("kaji review: no checks or REVIEW.md rules discovered");
        return;
    }
    eprintln!("kaji review: discovered {} check(s):", d.checks.len());
    for c in &d.checks {
...
/// `discovery_root` so [`kaji::checks::discover`] doesn't double-
