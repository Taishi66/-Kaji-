...
pub async fn handle_session_export(
    ...
    if nostr {
        return Err(anyhow::anyhow!("kaji was not built with nostr support"));
    }
...
pub async fn handle_session_import(input: String, nostr: bool) -> Result<()> {
    let json = if nostr || input.starts_with("kaji://sessions/nostr") {
        #[cfg(feature = "nostr")]
        {
    ...
    let format = kaji::session::import_formats::detect_format(&json);
    let label = match format {
        kaji::session::import_formats::ImportFormat::Kaji => "kaji",
        kaji::session::import_formats::ImportFormat::ClaudeCode => "Claude Code",
        kaji::session::import_formats::ImportFormat::Codex => "Codex",
        kaji::session::import_formats::ImportFormat::Pi => "Pi",
    };
