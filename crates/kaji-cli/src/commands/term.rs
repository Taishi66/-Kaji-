...
use kaji::conversation::message::{Message, MessageContent, MessageMetadata};
...
alias @kaji='{kaji_bin} term run'
alias @g='{kaji_bin} term run'

kaji_preexec() {
    [[ "$1" =~ ^kaji\ term ]] && return
    [[ "$1" =~ ^(@kaji|@g)($|[[:space:]]) ]] && return
    ('{kaji_bin}' term log "$1" &) 2>/dev/null
}

if [[ -z "$kaji_preexec_installed" ]]; then
    kaji_preexec_installed=1
    trap 'kaji_preexec "$BASH_COMMAND"' DEBUG
fi{command_not_found_handler}"#,
    command_not_found: Some(
        r#"

command_not_found_handle() {
    echo "⚔ Command '$1' not found. Asking kaji..."
    '{kaji_bin}' term run "$@"
    return 0
}"#,
    ),
...
add-zsh-hook preexec kaji_preexec{command_not_found_handler}"#,
    command_not_found: Some(
        r#"
...
function @kaji; {kaji_bin} term run $argv; end
function @g; {kaji_bin} term run $argv; end

function kaji_preexec --on-event fish_preexec
    string match -q -r '^kaji term' -- $argv[1]; and return
    string match -q -r '^(@kaji|@g)($|\s)' -- $argv[1]; and return
    {kaji_bin} term log "$argv[1]" 2>/dev/null &
end"#,
...
def --wrapped @kaji [...args] { run-external "{kaji_bin}" "term" "run" ...$args }
def --wrapped @g [...args] { run-external "{kaji_bin}" "term" "run" ...$args }

if (($env | get -o KAJI_NU_PREEXEC_INSTALLED | default false) != true) {
    $env.KAJI_NU_PREEXEC_INSTALLED = true
    $env.config.hooks.pre_execution = (
        ...
        | append {||
            ...
            job spawn { run-external "{kaji_bin}" "term" "log" $line | complete | ignore } | ignore
        }
    )
}
{command_not_found_handler}"#,
...
$env.config.hooks.command_not_found = {|command_name|
    ...
    print $"⚔ Command '($command_name)' not found. Asking kaji..."
    run-external "{kaji_bin}" "term" "run" $prompt | complete | ignore
    null
}"#,
    ),
};
...
static POWERSHELL_CONFIG: ShellConfig = ShellConfig {
    script_template: r#"$env:AGENT_SESSION_ID = "{session_id}"
function @kaji {{ & '{kaji_bin}' term run @args }}
function @g {{ & '{kaji_bin}' term run @args }}

Set-PSReadLineKeyHandler -Chord Enter -ScriptBlock {{
    $line = $null
    [Microsoft.PowerShell.PSConsoleReadLine]::GetBufferState([ref]$line, [ref]$null)
    if ($line -notmatch '^kaji term' -and $line -notmatch '^(@kaji|@g)($|\s)') {{
        Start-Job -ScriptBlock {{ & '{kaji_bin}' term log $using:line }} | Out-Null
    }}
...
fn render_term_init_script(
    ...
    let command_not_found_handler = if with_command_not_found {
        config
            .command_not_found
            .map(|handler| handler.replace("{kaji_bin}", kaji_bin))
            .unwrap_or_default()
    } else {
        String::new()
    };
...
pub async fn handle_term_init(
    ...
    let kaji_bin = std::env::current_exe()
        .map(|p| p.to_string_lossy().into_owned())
        .unwrap_or_else(|_| "kaji".to_string());

    println!(
        "{}",
        render_term_init_script(shell, &session.id, &kaji_bin, with_command_not_found)
    );
    Ok(())
}
...
pub async fn handle_term_log(command: String) -> Result<()> {
    let session_id = std::env::var("AGENT_SESSION_ID").map_err(|_| {
        anyhow!(
            "AGENT_SESSION_ID not set. Initialize terminal integration with `kaji term init <shell>` and reload your shell first."
        )
    })?;
    ...
    let session_id = std::env::var("AGENT_SESSION_ID").map_err(|_| {
        anyhow!(
            "AGENT_SESSION_ID not set.\n\n\
             Initialize terminal integration with `kaji term init <shell>` in your shell profile, \
...
/// Handle `kaji term info` - print compact session info for prompt integration
pub async fn handle_term_info() -> Result<()> {
    ...
    let config = kaji::config::Config::global();
    let model_name = config
        .get_kaji_model()
        .ok()
        .map(|name| {
            let short = name.rsplit('/').next().unwrap_or(&name);
            if let Some(stripped) = short.strip_prefix("kaji-") {
                stripped.to_string()
            } else {
                short.to_string()
            }
    ...
    let context_limit = config
        ...
        .ok()
        .and_then(|model_name| {
            config.get_kaji_provider().ok().and_then(|provider_name| {
                kaji::model_config::model_config_from_user_config(&provider_name, &model_name).ok()
            })
        })
...
mod tests {
    ...
    fn render_term_init_script_includes_nushell_hooks() {
        let script = render_term_init_script(Shell::Nu, "session-123", "/tmp/kaji", false);

        assert!(script.contains("$env.AGENT_SESSION_ID = \"session-123\""));
        assert!(script.contains("def --wrapped @kaji [...args]"));
        assert!(script.contains("def --wrapped @g [...args]"));
        assert!(script.contains("KAJI_NU_PREEXEC_INSTALLED"));
        assert!(script.contains("$env.config.hooks.pre_execution"));
        assert!(script.contains("job spawn { run-external \"/tmp/kaji\" \"term\" \"log\" $line | complete | ignore } | ignore"));
        assert!(!script.contains("command_not_found = {|command_name|"));
    }

    #[test]
    fn render_term_init_script_includes_nushell_default_handler() {
        let script = render_term_init_script(Shell::Nu, "session-123", "/tmp/kaji", true);

        assert!(script.contains("$env.config.hooks.command_not_found = {|command_name|"));
        assert!(script
            .contains("run-external \"/tmp/kaji\" \"term\" \"run\" $prompt | complete | ignore"));
    }

    #[test]
    fn render_term_init_script_skips_unsupported_default_handler() {
        let script = render_term_init_script(Shell::Fish, "session-123", "/tmp/kaji", true);
