...
fn asset_name() -> &'static str {
    ...
    {
        "kaji-aarch64-apple-darwin.tar.bz2"
    }
    #[cfg(all(target_os = "macos", target_arch = "x86_64"))]
    {
        "kaji-x86_64-apple-darwin.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "x86_64", target_env = "gnu"))]
    {
        "kaji-x86_64-unknown-linux-gnu.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "aarch64", target_env = "gnu"))]
    {
        "kaji-aarch64-unknown-linux-gnu.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "x86_64", target_env = "musl"))]
    {
        "kaji-x86_64-unknown-linux-musl.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "aarch64", target_env = "musl"))]
    {
        "kaji-aarch64-unknown-linux-musl.tar.bz2"
    }
    #[cfg(all(target_os = "windows", target_arch = "x86_64", feature = "cuda"))]
    {
        "kaji-x86_64-pc-windows-msvc-cuda.zip"
    }
    #[cfg(all(target_os = "windows", target_arch = "x86_64", not(feature = "cuda")))]
    {
        "kaji-x86_64-pc-windows-msvc.zip"
    }
}
...
fn sha256_hex(data: &[u8]) -> String {
    ...
    hasher.update(data);
    kaji::utils::bytes_to_hex(hasher.finalize())
}
...
async fn fetch_attestations(digest: &str, token: Option<&str>) -> Result<Vec<serde_json::Value>> {
    let url = format!(
        "https://api.github.com/repos/Taishi66/-Kaji-/attestations/sha256:{digest}\
...
/// Update the kaji binary to the latest release.
///
...
pub async fn update(canary: bool, reconfigure: bool) -> Result<()> {
    ...
    #[cfg(not(feature = "disable-update"))]
    {
        ...
        let url = format!("https://github.com/Taishi66/-Kaji-/releases/download/{tag}/{asset}");
        ...
        println!("kaji updated successfully (verified with Sigstore SLSA provenance).");

        // --- Reconfigure if requested -------------------------------------------
        if reconfigure {
            println!("Running kaji configure...");
            let status = Command::new(current_exe)
                .arg("configure")
                .status()
                .context("Failed to run kaji configure")?;
            if !status.success() {
                eprintln!("Warning: kaji configure exited with {status}");
            }
        }
...
///   1. A `kaji-package/` subdirectory (Windows releases)
...
fn find_binary(extract_dir: &Path, binary_name: &str) -> Option<PathBuf> {
    // 1. Check kaji-package subdir (matches download_cli.sh / download_cli.ps1)
    let package_dir = extract_dir.join("kaji-package");
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if old_exe.exists() {
            fs::remove_file(&old_exe).with_context(|| {
                format!(
                    "Failed to remove old backup {}. Is another kaji process running?",
                    old_exe.display()
                )
            })?;
        }
        ...
        fs::rename(current_exe, &old_exe).with_context(|| {
            format!(
                "Failed to rename running binary to {}. Try closing Kaji Desktop if it's open.",
                old_exe.display()
            )
        })?;
...
mod tests {
    ...
    fn test_find_binary_in_package_subdir() {
        ...
        let pkg = tmp.path().join("kaji-package");
    ...
    fn test_replace_binary_basic() {
        let tmp = tempdir().unwrap();
        let new_bin = tmp.path().join("new_kaji");
        let current = tmp.path().join("current_kaji");
    ...
    fn test_replace_binary_windows_rename_away() {
        let tmp = tempdir().unwrap();
        let current = tmp.path().join("kaji.exe");
        let new_bin = tmp.path().join("new_kaji.exe");
    ...
    fn test_extract_zip_with_package_dir() {
        ...
        // Create a zip in memory with kaji-package/ structure
        let mut buf = Vec::new();
        {
            ...
            writer.add_directory("kaji-package/", options).unwrap();
            writer.start_file("kaji-package/kaji.exe", options).unwrap();
            writer.write_all(b"fake kaji binary").unwrap();
            writer
                .start_file("kaji-package/libtest.dll", options)
                .unwrap();
        ...
        let binary = find_binary(tmp.path(), "kaji.exe");
        assert!(binary.is_some());

        let content = fs::read_to_string(binary.unwrap()).unwrap();
        assert_eq!(content, "fake kaji binary");

        // DLL should be in kaji-package too
        assert!(tmp.path().join("kaji-package/libtest.dll").exists());
    }
    ...
    fn test_validate_entry_path_accepts_safe_paths() {
        assert!(validate_entry_path(Path::new("kaji")).is_ok());
        assert!(validate_entry_path(Path::new("kaji-package/kaji")).is_ok());
    ...
    fn test_extract_tar_bz2_safe_archive() {
        ...
        let mut builder_buf = Vec::new();
        {
            ...
            let data = b"kaji binary content";
            let mut header = tar::Header::new_gnu();
            header.set_size(data.len() as u64);
            header.set_mode(0o755);
            header.set_cksum();
            builder
                .append_data(&mut header, "kaji-package/kaji", &data[..])
                .unwrap();
        ...
        let extracted = tmp.path().join("kaji-package/kaji");
