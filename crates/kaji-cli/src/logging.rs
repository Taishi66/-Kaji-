use anyhow::Result;
use kaji::providers::utils::init_kaji_request_log;
...
pub fn setup_logging(name: Option<&str>) -> &'static Result<()> {
    INIT.get_or_init(|| {
        ...
        init_kaji_request_log()?;
        let config = kaji::logging::LoggingConfig {
            component: "cli",
            name,
            extra_directives: &["kaji_cli=info"],
            console: false,
            json: true,
        };
        let subscriber = kaji::logging::build_logging_subscriber(&config)?;
...
#[cfg(test)]
mod tests {
    use kaji::tracing::langfuse_layer;
    use std::env;
    ...
    fn test_log_directory_creation() {
        ...
        let log_dir = kaji::logging::prepare_log_directory("cli", true).unwrap();
        assert!(log_dir.exists());
        assert!(log_dir.is_dir());

        let path_components: Vec<_> = log_dir.components().collect();
        assert!(path_components.iter().any(|c| c.as_os_str() == "kaji"));
    ...
    async fn test_default_filter_avoids_debug_by_default() {
        ...
        assert!(kaji::logging::build_logging_subscriber(&config).is_ok());
    }
}
