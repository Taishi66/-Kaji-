...
use anyhow::Result;
use kaji::agents::{Agent, AgentConfig, KajiPlatform};
use kaji::config::permission::PermissionManager;
use kaji::config::KajiMode;
use kaji::providers::{create, testprovider::TestProvider};
...
{
    if let Ok(only_provider) = std::env::var("KAJI_TEST_PROVIDER") {
    ...
    F: Fn(&ScenarioResult) -> Result<()>,
{
    ...
    kaji::agents::moim::SKIP.with(|f| f.set(true));
    ...
    let scenario_model_config =
        kaji::model_config::model_config_from_user_config(&factory_name, config.model_name)?;
    agent
        .update_provider(
            provider_arc as Arc<dyn kaji::providers::base::Provider>,
