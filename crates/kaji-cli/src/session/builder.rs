...
use console::style;
use kaji::agents::{Agent, Container, ExtensionError};
use kaji::config::resolve_extensions_for_new_session;
use kaji::config::{Config, ExtensionConfig, KajiMode};
...
use kaji::session::EnabledExtensionsState;
...
/// Configuration for building a new Kaji session
///
...
async fn load_extensions(
    ...
    for (id, err) in failed {
        ...
        eprintln!(
            "{}",
            style(format!(
                "  Hint: once the session starts, ask kaji to help debug the '{}' extension",
                label
            ))
            .dim()
        );
...
fn resolve_provider_and_model(
    ...
    saved_model_config: Option<kaji_providers::model::ModelConfig>,
    ...
    let provider_name = session_config
        ...
        .clone()
        .or(saved_provider)
        .or_else(|| recipe_settings.and_then(|s| s.kaji_provider.clone()))
        .or_else(|| config.get_kaji_provider().ok())
        .unwrap_or_else(|| {
            output::render_error("No provider configured. Run 'kaji configure' first.");
            process::exit(1);
        });
    ...
    let model_name = session_config
        ...
        .or_else(|| recipe_settings.and_then(|s| s.kaji_model.clone()))
        .or_else(|| config.get_kaji_model().ok())
        .unwrap_or_else(|| {
            output::render_error("No model configured. Run 'kaji configure' first.");
            process::exit(1);
        });
...
async fn resolve_session_id(
    session_config: &SessionBuilderConfig,
    session_manager: &kaji::session::session_manager::SessionManager,
    kaji_mode: KajiMode,
) -> String {
...
async fn collect_extension_configs(
    ...
    if !session_config.no_profile && !session_config.resume && recipe_extensions.is_none() {
        ...
        all.extend(kaji::plugins::mcp_servers::enabled_plugin_mcp_servers(
...
async fn configure_session_prompts(
    ...
    let system_prompt_file: Option<String> = config.get_param("KAJI_SYSTEM_PROMPT_FILE_PATH").ok();
...
pub async fn build_session(session_config: SessionBuilderConfig) -> CliSession {
    ...
    kaji::posthog::set_session_context("cli", session_config.resume);
    ...
    let session_id =
        resolve_session_id(&session_config, &session_manager, agent.config.kaji_mode).await;
    ...
    let (new_provider, effective_provider_name, effective_model_name, effective_model_config) =
        match create(&resolved.provider_name, extensions_for_provider.clone()).await {
            ...
            {
                let fallback_provider = config.get_kaji_provider().unwrap_or_else(|_| {
                    output::render_error("No provider configured. Run 'kaji configure' first.");
                    process::exit(1);
                });
                let fallback_model = config.get_kaji_model().unwrap_or_else(|_| {
                    output::render_error("No model configured. Run 'kaji configure' first.");
                    process::exit(1);
                });
                eprintln!(
    ...
    agent
        .update_kaji_mode(agent.config.kaji_mode, &session_id)
        .await
        .unwrap_or_else(|e| {
    ...
    let debug_mode = session_config.debug || config.get_param("KAJI_DEBUG").unwrap_or(false);
...
mod tests {
    ...
    fn test_session_builder_config_creation() {
        let config = SessionBuilderConfig {
            ...
            streamable_http_extensions: vec![StreamableHttpOptions {
                ...
                timeout: kaji::config::DEFAULT_EXTENSION_TIMEOUT,
            }],
    ...
    async fn test_implicit_resume_ignores_newer_scheduled_sessions() {
        ...
        let kaji_mode = KajiMode::default();
