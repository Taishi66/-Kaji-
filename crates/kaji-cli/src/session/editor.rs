...
/// Checks KAJI_PROMPT_EDITOR, then $VISUAL, then $EDITOR.
pub fn resolve_editor_command() -> Option<String> {
    let config = Config::global();
    let config_editor = config.get_kaji_prompt_editor().ok().flatten();
...
fn build_template(messages: &[&str], prefill: Option<&str>) -> String {
    let mut content = String::from("# Kaji Prompt Editor\n\n");
...
pub fn get_editor_input(
    ...
    let symlink_path = PathBuf::from(".kaji_prompt_temp.md");
...
mod tests {
    ...
    fn test_extract_user_input_with_editor_output() {
        let content = r#"# Kaji Prompt Editor
...
"#;
    ...
    fn test_create_temp_file_with_messages() {
        ...
        assert!(path.exists());
        assert!(path.to_str().unwrap().contains("kaji_prompt_"));
        assert!(path.to_str().unwrap().ends_with(".md"));

        let content = fs::read_to_string(path).unwrap();
        assert!(content.contains("# Kaji Prompt Editor"));
    ...
    fn test_create_temp_file_with_prefix_suffix() {
        ...
        assert!(name.starts_with("kaji_test_"));
...
"#;
    ...
    fn test_build_template_no_prefill_no_messages() {
        ...
        assert_eq!(content, "# Kaji Prompt Editor\n\n# Your prompt:\n\n");
    }
