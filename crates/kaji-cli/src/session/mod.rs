...
use kaji::conversation::{fix_conversation, merge_consecutive_messages_for_request, Conversation};
use std::env;
use std::io::Write;
use std::str::FromStr;
...
use console::Color;
use kaji::agents::platform_extensions::developer::shell::{
    parse_shell_output_notification, ShellOutputNotificationParams, ShellOutputStream,
};
use kaji::agents::AgentEvent;
use kaji::agents::SUBAGENT_TOOL_REQUEST_TYPE;
use kaji::permission::permission_confirmation::PrincipalType;
use kaji::permission::Permission;
use kaji::permission::PermissionConfirmation;
use kaji::providers::base::Provider;
use kaji::providers::base::ProviderUsage;
use kaji::utils::safe_truncate;

use anyhow::{Context, Result};
use completion::KajiCompleter;
use input::InputResult;
use kaji::agents::extension::{Envs, ExtensionConfig, PLATFORM_EXTENSIONS};
use kaji::agents::types::RetryConfig;
use kaji::agents::{Agent, SessionConfig, COMPACT_TRIGGERS};
use kaji::config::extensions::name_to_key;
...
use kaji::config::providers;
use kaji::conversation::message::{ActionRequiredData, Message, MessageContent};
use kaji::providers::inventory::ProviderInventoryService;
...
const KAJI_PLANNER_CONTEXT_LIMIT: &str = "KAJI_PLANNER_CONTEXT_LIMIT";
...
impl HistoryManager {
    ...
    fn load(
        &self,
        editor: &mut rustyline::Editor<KajiCompleter, rustyline::history::DefaultHistory>,
    ) {
...
pub async fn classify_planner_response(
    ...
    let (result, _usage) = kaji::session_context::with_session_id(
...
impl CliSession {
    ...
    pub fn parse_stdio_extension(extension_command: &str) -> Result<ExtensionConfig> {
        let mut parts = kaji::utils::split_command_args(extension_command)?;
        ...
        Ok(ExtensionConfig::Stdio {
            ...
            envs: Envs::new(envs),
            env_keys: Vec::new(),
            description: kaji::config::DEFAULT_EXTENSION_DESCRIPTION.to_string(),
            timeout: Some(kaji::config::DEFAULT_EXTENSION_TIMEOUT),
            cwd: None,
            bundled: None,
    ...
    pub async fn add_streamable_http_extension(&mut self, extension_url: String) -> Result<()> {
        let config = Self::parse_streamable_http_extension(
            &extension_url,
            kaji::config::DEFAULT_EXTENSION_TIMEOUT,
        );
        self.add_and_persist_extensions(vec![config]).await
    }
    ...
    pub async fn interactive(&mut self, prompt: Option<String>) -> Result<()> {
        ...
        self.agent
            .emit_hook(kaji::hooks::HookEvent::SessionEnd, &self.session_id)
            .await;
    ...
    fn create_editor(
        &self,
    ) -> Result<rustyline::Editor<KajiCompleter, rustyline::history::DefaultHistory>> {
        let builder =
        ...
        let mut editor =
            rustyline::Editor::<KajiCompleter, rustyline::history::DefaultHistory>::with_config(
                config,
            )?;
        let completer = KajiCompleter::new(self.completion_cache.clone());
    ...
    async fn handle_input(
        ...
        match input {
            ...
            InputResult::KajiMode(mode) => {
                history.save(editor);
                self.handle_kaji_mode(&mode).await?;
            }
                    ...
                    None => {
                        output::render_error(
                            "No editor found. Set one with:\n  \
                                 kaji configure set kaji_prompt_editor \"vim\"\n  \
    ...
    async fn handle_kaji_mode(&self, mode: &str) -> Result<()> {
        let config = Config::global();
        let mode = match KajiMode::from_str(&mode.to_lowercase()) {
            Ok(mode) => mode,
            Err(_) => {
                output::render_error(&format!(
                    "Invalid mode '{mode}'. Mode must be one of: {}",
                    KajiMode::VARIANTS.join(", ")
                ));
                return Ok(());
            }
        };
        self.agent.update_kaji_mode(mode, &self.session_id).await?;
        config.set_kaji_mode(mode)?;
        output::kaji_mode_message(&format!("Kaji mode set to '{mode}'"));
        Ok(())
    }
    ...
    async fn handle_model(&mut self, options: input::ModelCommandOptions) -> Result<()> {
        ...
        let target_entry = match kaji::providers::get_from_registry(target_provider_name).await {
            Ok(entry) => entry,
            Err(_) => {
                output::render_error(&format!(
        ...
        let new_provider = match kaji::providers::create(target_provider_name, extensions).await {
            Ok(p) => p,
            Err(e) => {
                output::render_error(&format!(
                    "Cannot switch to provider '{}': {}\n\
                         Set credentials via `kaji configure` or the appropriate environment variable.\n\
        ...
        let mode = self.agent.kaji_mode().await;
        self.agent.update_kaji_mode(mode, &self.session_id).await?;
    ...
    async fn handle_list_skills(&mut self) -> Result<()> {
        ...
        use kaji::custom_requests::SourceType;
        use kaji::skills::list_installed_skills;
    ...
    async fn plan_with_reasoner_model(
        ...
        output::show_thinking();
        let (plan_response, _usage) = kaji::session_context::with_session_id(
        ...
        match planner_response_type {
            PlannerResponseType::Plan => {
                ...
                if should_act {
                    ...
                    self.run_mode = RunMode::Normal;
                    // set kaji mode: auto if that isn't already the case
                    let config = Config::global();
                    let curr_kaji_mode = config.get_kaji_mode().unwrap_or_default();
                    if curr_kaji_mode != KajiMode::Auto {
                        config.set_kaji_mode(KajiMode::Auto).unwrap();
                    }
                    ...
                    // Reset run & kaji mode
                    if curr_kaji_mode != KajiMode::Auto {
                        config.set_kaji_mode(curr_kaji_mode)?;
                    }
                } else {
                        ...
                        Some(Ok(AgentEvent::Message(message))) => {
                            ...
                            if let Some((id, security_prompt)) = find_tool_confirmation(&message) {
                                let permission = if interactive {
                                    ...
                                    if kaji_mode == KajiMode::Approve || kaji_mode == KajiMode::SmartApprove {
                                        cancel_token_clone.cancel();
                                        drop(stream);
                                        return Err(anyhow::anyhow!(
                                            "Tool approval required in non-interactive mode with KajiMode::{kaji_mode}. \
                                             This is an invalid configuration — Approve/SmartApprove modes require an \
                                             interactive terminal. Use KajiMode::Auto for headless sessions."
                                        ));
                                    }
                                    tracing::warn!(
    ...
    pub async fn update_completion_cache(&mut self) -> Result<()> {
        let prompts = self.agent.list_extension_prompts(&self.session_id).await;
        let all_providers = kaji::providers::providers().await;
    ...
    pub async fn get_session(&self) -> Result<kaji::session::Session> {
        self.agent
            .config
            .session_manager
    ...
    pub async fn display_context_usage(&self) -> Result<()> {
        ...
        let show_cost = config
            .get_param::<bool>("KAJI_CLI_SHOW_COST")
            .unwrap_or(false);
...
fn prompt_tool_confirmation(security_prompt: &Option<String>) -> Result<Permission> {
    ...
    let prompt = if let Some(security_message) = security_prompt {
        ...
        "Kaji would like to call the above tool, do you allow?".to_string()
    };
                ...
                let formatted = match notification_type {
                    ...
                    Some("response_generated") => {
                        ...
                        let min_priority = config
                            .get_param::<f32>("KAJI_CLI_MIN_PRIORITY")
                            .ok()
    ...
    for content in &message.content {
        if let MessageContent::ToolRequest(tool_request) = content {
            if let Ok(tool_call) = &tool_request.tool_call {
                tracing::info!(
                    monotonic_counter.kaji.tool_calls = 1,
        ...
        if let MessageContent::ToolResponse(tool_response) = content {
            ...
            tracing::info!(
                monotonic_counter.kaji.tool_completions = 1,
                tool_name = %tool_name,
...
fn handle_agent_error(e: &anyhow::Error, is_stream_json_mode: bool) {
    ...
    if e.downcast_ref::<kaji_providers::errors::ProviderError>()
        .map(|provider_error| {
            matches!(
                provider_error,
                kaji_providers::errors::ProviderError::ContextLengthExceeded(_)
            )
        })
        .unwrap_or(false)
    {
...
async fn get_reasoner(
) -> Result<(Arc<dyn Provider>, kaji_providers::model::ModelConfig), anyhow::Error> {
    use kaji::providers::create;

    let config = Config::global();

    // Try planner-specific provider first, fall back to default provider
    let provider = if let Ok(provider) = config.get_param::<String>("KAJI_PLANNER_PROVIDER") {
        provider
    } else {
        println!("WARNING: KAJI_PLANNER_PROVIDER not found. Using default provider...");
        config
            .get_kaji_provider()
    ...
    let model = if let Ok(model) = config.get_param::<String>("KAJI_PLANNER_MODEL") {
        model
    } else {
        println!("WARNING: KAJI_PLANNER_MODEL not found. Using default model...");
        config
            .get_kaji_model()
            .expect("No model configured. Run 'kaji configure' first")
    };

    let planner_context_limit = match env::var(KAJI_PLANNER_CONTEXT_LIMIT)
        .ok()
        .map(|v| v.parse::<usize>())
    {
        Some(Ok(n)) if n >= 4096 => Some(n),
        Some(Ok(_)) => anyhow::bail!("{} must be at least 4096", KAJI_PLANNER_CONTEXT_LIMIT),
        Some(Err(e)) => anyhow::bail!("{}: {}", KAJI_PLANNER_CONTEXT_LIMIT, e),
        None => None,
    };

    let model_config =
        kaji::model_config::model_config_from_user_config(&provider, model.as_str())?
            .with_context_limit(planner_context_limit);
    let extensions = kaji::config::extensions::get_enabled_extensions_with_config(config);
...
fn build_switched_model_config(
    provider_name: &str,
    model_name: &str,
    current_model_config: &kaji_providers::model::ModelConfig,
...
mod tests {
    use super::*;
    use kaji::agents::extension::Envs;
    use kaji::config::ExtensionConfig;
    ...
    fn test_build_switched_model_config_rebuilds_target_model_settings() {
        let _guard = env_lock::lock_env([
            ("KAJI_MAX_TOKENS", None::<&str>),
            ("KAJI_TEMPERATURE", None::<&str>),
            ("KAJI_CONTEXT_LIMIT", None::<&str>),
            ("KAJI_TOOLSHIM", None::<&str>),
            ("KAJI_TOOLSHIM_OLLAMA_MODEL", None::<&str>),
        ]);

        let current_model_config = kaji_providers::model::ModelConfig {
        ...
        let expected = kaji_providers::model::ModelConfig::new("gpt-5.4")
    ...
    fn test_build_switched_model_config_detects_effort_suffix_change() {
        let _guard = env_lock::lock_env([
            ...
            ("KAJI_TOOLSHIM", None::<&str>),
            ("KAJI_TOOLSHIM_OLLAMA_MODEL", None::<&str>),
            ("KAJI_THINKING_EFFORT", None::<&str>),
        ]);

        let current =
            kaji_providers::model::ModelConfig::new("gpt-5.4-high").with_canonical_limits("openai");
        assert_eq!(current.model_name, "gpt-5.4");
        assert_eq!(
            current.thinking_effort(),
            Some(kaji_providers::thinking::ThinkingEffort::High)
        );
    ...
    fn test_split_command_args_windows_paths() {
        assert_eq!(
            kaji::utils::split_command_args(r"C:\tools\mcp.exe --arg value").unwrap(),
            vec![r"C:\tools\mcp.exe", "--arg", "value"]
        );
        assert_eq!(
            kaji::utils::split_command_args(r#""C:\Program Files\server\mcp.exe" --arg"#).unwrap(),
    ...
    fn test_split_command_args_unmatched_quote() {
        assert!(kaji::utils::split_command_args(r#""unmatched"#).is_err());
    }
