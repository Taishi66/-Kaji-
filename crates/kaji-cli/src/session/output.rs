...
use kaji::providers::canonical::maybe_get_canonical_model;
    ...
    fn as_str(&self) -> String {
        match self {
            Theme::Light => Config::global()
                .get_param::<String>("KAJI_CLI_LIGHT_THEME")
                .unwrap_or(DEFAULT_CLI_LIGHT_THEME.to_string()),
            Theme::Dark => Config::global()
                .get_param::<String>("KAJI_CLI_DARK_THEME")
...
thread_local! {
    static CURRENT_THEME: RefCell<Theme> = RefCell::new(
        std::env::var("KAJI_CLI_THEME").ok()
            .map(|val| Theme::from_config_str(&val))
            .unwrap_or_else(||
                Config::global().get_param::<String>("KAJI_CLI_THEME").ok()
                    .map(|val| Theme::from_config_str(&val))
                    .unwrap_or(Theme::Ansi)
            )
    );
    static SHOW_FULL_TOOL_OUTPUT: RefCell<bool> = RefCell::new(
        Config::global().get_param::<bool>("KAJI_SHOW_FULL_OUTPUT").unwrap_or(false)
    );
}
...
pub fn set_theme(theme: Theme) {
    ...
    config
        .set_param("KAJI_CLI_THEME", theme.as_config_string())
    ...
    if let Err(e) = config.set_param("KAJI_CLI_THEME", theme_str) {
...
pub fn run_status_hook(status: &str) {
    if let Ok(hook) = Config::global().get_param::<String>("KAJI_STATUS_HOOK") {
...
pub fn kaji_mode_message(text: &str) {
    println!("\n{} {}", accent("mode:"), text);
}

fn should_show_thinking() -> bool {
    Config::global()
        .get_param::<bool>("KAJI_CLI_SHOW_THINKING")
        .unwrap_or(false)
