...
pub use kaji_download_manager as download_manager;
...
use kaji_provider_types::base::{MessageStream, Provider, ProviderDescriptor, ProviderMetadata};
use kaji_provider_types::conversation::message::{Message, MessageContent, SystemNotificationType};
use kaji_provider_types::conversation::token_usage::{ProviderUsage, Usage};
use kaji_provider_types::errors::ProviderError;
use kaji_provider_types::images::ImageFormat;
use kaji_provider_types::model::ModelConfig;
use kaji_provider_types::request_log::{start_log, LoggerHandleExt, RequestLogHandle};
    ...
    if let Ok(registry) = get_registry().lock() {
        if let Some(entry) = registry.get_model(model_id) {
            ...
            let draft_model = settings
                ...
                .clone()
                .or_else(|| {
                    config_resolver::string_param("KAJI_LOCAL_DRAFT_MODEL")
                        .ok()
                        .flatten()
                })
...
fn build_openai_messages_json(
    ...
    use kaji_provider_types::formats::openai::format_messages;
...
impl Provider for LocalInferenceProvider {
    ...
    async fn stream(
        ...
        if let Some(false) = model_config
            ...
            .or_else(|| {
                config_resolver::bool_param("KAJI_LOCAL_ENABLE_THINKING")
                    .ok()
                    .flatten()
            })
        {
