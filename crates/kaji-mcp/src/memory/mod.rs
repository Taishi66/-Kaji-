...
impl MemoryServer {
    pub fn new() -> Self {
        let instructions = formatdoc! {r#"
             ...
             Storage:
             - Local: .kaji/memory/ (project-specific)
             - Global: ~/.config/kaji/memory/ (user-wide)
        ...
        let global_memory_dir = choose_app_strategy(crate::APP_STRATEGY.clone())
            ...
            .unwrap_or_else(|_| PathBuf::from(".config/kaji/memory"));
    ...
    fn get_memory_file(
        ...
        let base_dir = if is_global {
            ...
            local_base.join(".kaji").join("memory")
        };
...
mod tests {
    ...
    fn test_lazy_directory_creation() {
        ...
        let local_memory_dir = working_dir.join(".kaji").join("memory");
