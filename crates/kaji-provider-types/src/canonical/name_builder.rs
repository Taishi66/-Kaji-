...
pub fn map_provider_name(provider: &str) -> &str {
    match provider {
        // Kaji provider names that differ from models.dev names
        "xai" => "x-ai",
...
mod tests {
    ...
    fn test_map_to_canonical_model() {
        ...
        assert_eq!(
            map_to_canonical_model("databricks", "kaji-claude-sonnet-4-5-bedrock", r),
            Some("anthropic/claude-sonnet-4.5".to_string())
        );
        assert_eq!(
        ...
        assert_eq!(
            map_to_canonical_model("databricks", "kkaji-gpt-4o", r),
            Some("openai/gpt-4o".to_string())
        );
        assert_eq!(
        ...
        // === OpenAI O-series ===
        assert_eq!(
            map_to_canonical_model("databricks", "kaji-o1", r),
            Some("openai/o1".to_string())
        );
        assert_eq!(
            map_to_canonical_model("databricks", "kkaji-o3", r),
            Some("openai/o3".to_string())
        );
        assert_eq!(
            map_to_canonical_model("databricks", "headless-kaji-o3-mini", r),
        ...
        assert_eq!(
            map_to_canonical_model("databricks", "kkaji-grok-4.3", r),
            Some("x-ai/grok-4.3".to_string())
        );
        ...
        assert_eq!(
            map_to_canonical_model("databricks", "kaji-command-r-08-2024", r),
