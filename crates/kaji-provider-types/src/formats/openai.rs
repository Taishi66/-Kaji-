...
pub fn create_request_for_model_with_options(
    ...
    if model_config.model_name.starts_with("o1-mini") {
        return Err(anyhow!(
            "o1-mini model is not currently supported since kaji uses tool calling and o1-mini does not support it. Please use o1 or o3 models instead."
        ));
    }
    ...
    // KAJI_MAX_TOKENS) or a canonical model record has supplied a value.
...
/// hosted aliases like `databricks-gpt-5.4`, `kaji-o3-mini`, or
/// `headless-kaji-o3-mini` work without provider-specific normalization.
...
/// Maps Kaji's effort levels to values accepted by xAI Chat Completions.
...
mod tests {
    ...
    fn test_create_request_omits_max_tokens_when_unset() -> anyhow::Result<()> {
        ...
        // lmstudio) have no canonical record and no KAJI_MAX_TOKENS, so the
...
data: [DONE]"#;
    ...
    fn test_extract_reasoning_effort_for_responses_models() {
        for (model, expected_name, expected_effort) in [
            ...
            ("kaji-gpt-5-high", "kaji-gpt-5", Some("high")),
            ("gpt-4o", "gpt-4o", None),
        ] {
        ...
        fn turn_context(time: &str, turn_budget: &str) -> String {
            format!(
                "<turn-context>\n\
                 ...
                 <working-directory>/Users/me/code/kaji</working-directory>\n\
