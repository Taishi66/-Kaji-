...
#[notification(method = "_kaji/unstable/session/update")]
#[serde(rename_all = "camelCase")]
pub struct KajiSessionNotification {
    pub session_id: String,
    pub update: KajiSessionUpdate,
}
...
pub enum KajiSessionUpdate {
    UsageUpdate(SessionUsageUpdate),
    StatusMessage(StatusMessageUpdate),
    MessageUsage(MessageUsageUpdate),
}

impl Default for KajiSessionUpdate {
    fn default() -> Self {
        KajiSessionUpdate::UsageUpdate(SessionUsageUpdate::default())
    }
}
...
/// Schemas for every kaji-custom outbound notification. To register a new
...
pub fn custom_notification_schemas(generator: &mut SchemaGenerator) -> Vec<CustomMethodSchema> {
    vec![notification_schema::<KajiSessionNotification>(generator)]
}
...
mod tests {
    ...
    fn status_message_serializes_to_expected_wire_shape() {
        let notification = KajiSessionNotification {
    ...
    fn message_usage_serializes_to_expected_wire_shape() {
        let notification = KajiSessionNotification {
            session_id: "s1".to_string(),
            update: KajiSessionUpdate::MessageUsage(MessageUsageUpdate {
