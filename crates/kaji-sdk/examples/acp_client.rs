//! ACP Client Example
//!
//! Spawns `kaji acp` as a child process and sends it a completion request
//! using the Agent Client Protocol over stdio.
//!
//! # Prerequisites
//!
//! You must have kaji built and a provider configured (`kaji configure`).
//!
//! # Usage
//!
//! ```bash
//! cargo run -p kaji-sdk --example acp_client -- "What is 2 + 2?"
//! ```
//!
//! Or with a custom kaji binary path:
//!
//! ```bash
//! cargo run -p kaji-sdk --example acp_client -- --kaji-bin ./target/debug/kaji "Explain Rust's ownership model in one sentence"
//! ```
...
use kaji_sdk::custom_requests::GetExtensionsRequest;
use std::path::PathBuf;
use tokio_util::compat::{TokioAsyncReadCompatExt, TokioAsyncWriteCompatExt};

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Parse args: [--kaji-bin PATH] PROMPT
    let args: Vec<String> = std::env::args().skip(1).collect();
    let (kaji_bin, prompt) = parse_args(&args)?;

    eprintln!("🚀 Spawning: {} acp", kaji_bin.display());

    let mut child = tokio::process::Command::new(&kaji_bin)
        .arg("acp")
        .stdin(std::process::Stdio::piped())
        .stdout(std::process::Stdio::piped())
        .stderr(std::process::Stdio::inherit())
        .spawn()
        .map_err(|e| format!("Failed to spawn '{}': {e}", kaji_bin.display()))?;
...
fn parse_args(args: &[String]) -> Result<(PathBuf, String), String> {
    let mut kaji_bin = PathBuf::from("kaji");
    let mut i = 0;
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--kaji-bin" => {
                i += 1;
                kaji_bin = PathBuf::from(args.get(i).ok_or("--kaji-bin requires a value")?);
            }
            _ => break,
        }
        i += 1;
    ...
    if prompt.is_empty() {
        return Err("Usage: acp_client [--kaji-bin PATH] PROMPT".into());
    }
