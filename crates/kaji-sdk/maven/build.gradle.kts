...
group = "io.github.aaif-kaji"
version = kajiSdkVersion()

fun kajiSdkVersion(): String {
    val cargoToml = file("../Cargo.toml").readText()
    return Regex("(?m)^version\\s*=\\s*\"([^\"]+)\"")
        .find(cargoToml)
        ?.groupValues
        ?.get(1)
        ?: error("Could not find kaji-sdk version in ../Cargo.toml")
}
...
tasks.jar {
    manifest {
        attributes(
            "Implementation-Title" to "Kaji SDK",
...
mavenPublishing {
    ...
    pom {
        name.set("Kaji GDK")
        description.set("Kotlin/JVM bindings for the Kaji SDK")
