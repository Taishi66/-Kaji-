//! In-process uniffi bindings for the Kaji SDK.
//!
//! This is the API surface exposed to Python and Kotlin. It focuses on native
//! Kaji providers and mirrors the provider message/tool/streaming model closely
...
use kaji_providers::{
    ...
    base::{MessageStream, Provider as KajiProvider},
    conversation::{
        message::{Message, MessageContent as KajiMessageContent},
        token_usage::ProviderUsage,
    },
    databricks::DatabricksProvider as KajiDatabricksProvider,
    databricks_auth::DatabricksAuth,
    databricks_v2::DatabricksV2Provider as KajiDatabricksV2Provider,
...
impl From<anyhow::Error> for KajiError {
    fn from(error: anyhow::Error) -> Self {
        Self::generic(error)
    }
}

impl From<kaji_providers::errors::ProviderError> for KajiError {
    fn from(error: kaji_providers::errors::ProviderError) -> Self {
        match error {
            kaji_providers::errors::ProviderError::Authentication(message) => {
                Self::Authentication { details: message }
            }
            kaji_providers::errors::ProviderError::ContextLengthExceeded(message) => {
                Self::ContextLengthExceeded { details: message }
            }
            kaji_providers::errors::ProviderError::RateLimitExceeded { retry_delay, .. } => {
            ...
            kaji_providers::errors::ProviderError::ServerError(message)
            | kaji_providers::errors::ProviderError::EndpointNotFound(message)
            | kaji_providers::errors::ProviderError::CreditsExhausted {
                details: message, ..
            } => Self::ProviderUnavailable { details: message },
            kaji_providers::errors::ProviderError::NetworkError(message)
                if is_timeout(&message) =>
            {
                Self::Timeout { details: message }
            }
            kaji_providers::errors::ProviderError::RequestFailed(message)
                if is_timeout(&message) =>
            {
                Self::Timeout { details: message }
            }
            kaji_providers::errors::ProviderError::ExecutionError(message)
...
impl From<serde_json::Error> for KajiError {
...
pub trait RequestLogger: Send + Sync {
    fn start(&self) -> Result<u64, KajiError>;
    fn write(&self, request_id: u64, record: String) -> Result<(), KajiError>;
}
...
impl kaji_providers::request_log::RequestLogger for RequestLoggerAdapter {
    fn start(
        &self,
    ) -> Result<
        Box<dyn kaji_providers::request_log::RequestLogHandle>,
...
impl kaji_providers::request_log::RequestLogHandle for RequestLogHandleAdapter {
...
#[uniffi::export]
pub fn install_request_logger(logger: Box<dyn RequestLogger>) -> Result<(), KajiError> {
    kaji_providers::request_log::install_logger(RequestLoggerAdapter {
        logger: Arc::from(logger),
    })
    .map_err(|error| KajiError::generic(error.to_string()))
}
...
impl ProviderMessage {
    fn to_kaji_message(&self) -> Result<Option<Message>, KajiError> {
        ...
        for content in &self.content {
            message = message.with_content(content.to_kaji_content()?);
        }
        Ok(Some(message))
    }
}
...
impl MessageContent {
    fn to_kaji_content(&self) -> Result<KajiMessageContent, KajiError> {
        match self {
            MessageContent::Text { text } => {
                Ok(KajiMessageContent::text(sanitize_unicode_tags(text)))
            }
            MessageContent::Image { mime_type, data } => Ok(KajiMessageContent::image(
            ...
            MessageContent::ToolRequest {
                ...
                Ok(KajiMessageContent::tool_request(
                    id.clone(),
            ...
            MessageContent::ToolResult {
                ...
                Ok(KajiMessageContent::tool_response(id.clone(), tool_result))
            }
            MessageContent::Thinking {
                thinking,
                signature,
            } => Ok(KajiMessageContent::thinking(
                thinking.clone(),
                signature.clone(),
            )),
            MessageContent::RedactedThinking { data } => {
                Ok(KajiMessageContent::redacted_thinking(data.clone()))
            }
        }
    }
}
...
impl ProviderTool {
    fn to_kaji_tool(&self) -> Result<Tool, KajiError> {
...
fn parse_json_object(json: &str) -> Result<serde_json::Map<String, Value>, KajiError> {
    match serde_json::from_str(json)? {
        Value::Object(object) => Ok(object),
        _ => Err(KajiError::generic("expected a JSON object")),
    }
}
...
impl ProviderModelConfig {
    fn to_kaji_model_config(&self) -> Result<ModelConfig, KajiError> {
...
impl Usage {
    fn from_provider_usage(usage: &ProviderUsage) -> Result<Self, KajiError> {
        Ok(Self {
...
pub enum StreamChunk {
    ...
    ErrorChunk {
        error: KajiStreamError,
    },
}

#[derive(Debug, Clone, uniffi::Record)]
pub struct KajiStreamError {
    pub kind: KajiStreamErrorKind,
    pub message: String,
    pub retry_after_ms: Option<u64>,
}

#[derive(Debug, Clone, uniffi::Enum)]
pub enum KajiStreamErrorKind {
    RateLimited,
...
impl From<KajiError> for KajiStreamError {
    fn from(error: KajiError) -> Self {
        match error {
            KajiError::RateLimited {
                retry_after_ms,
                retry_after_suffix,
            } => Self {
                kind: KajiStreamErrorKind::RateLimited,
                message: format!("Rate limit exceeded{retry_after_suffix}"),
                retry_after_ms,
            },
            KajiError::OutputTokenLimitExceeded { details } => Self {
                kind: KajiStreamErrorKind::OutputTokenLimitExceeded,
                message: details,
                retry_after_ms: None,
            },
            KajiError::ContextLengthExceeded { details } => Self {
                kind: KajiStreamErrorKind::ContextLengthExceeded,
                message: details,
                retry_after_ms: None,
            },
            KajiError::Authentication { details } => Self {
                kind: KajiStreamErrorKind::Authentication,
                message: details,
                retry_after_ms: None,
            },
            KajiError::Timeout { details } => Self {
                kind: KajiStreamErrorKind::Timeout,
                message: details,
                retry_after_ms: None,
            },
            KajiError::ProviderUnavailable { details } => Self {
                kind: KajiStreamErrorKind::ProviderUnavailable,
                message: details,
                retry_after_ms: None,
            },
            KajiError::Generic { details } => Self {
                kind: KajiStreamErrorKind::Generic,
                message: details,
                retry_after_ms: None,
            },
        }
...
fn runtime() -> Result<&'static tokio::runtime::Runtime, KajiError> {
...
async fn run_on_runtime<T>(future: impl Future<Output = T> + Send + 'static) -> Result<T, KajiError>
where
    T: Send + 'static,
{
    ...
    (&mut task.handle).await.map_err(|error| {
        if error.is_cancelled() {
            ...
            KajiError::generic(error)
        }
    })
}

struct ProviderHandle {
    provider: Arc<dyn KajiProvider>,
}

impl ProviderHandle {
    fn new(provider: Box<dyn KajiProvider>) -> Self {
        Self {
    ...
    ) -> Result<Arc<ProviderStream>, KajiError> {
        let timeout_ms = model.timeout_ms;
        let model = model.to_kaji_model_config()?;
    ...
    async fn complete(
        ...
        messages: Vec<ProviderMessage>,
        tools: Vec<ProviderTool>,
    ) -> Result<ProviderCompletion, KajiError> {
        let timeout_ms = model.timeout_ms;
        let model = model.to_kaji_model_config()?;
        let messages = convert_messages(messages)?;
        let tools = convert_tools(tools)?;
...
fn convert_messages(messages: Vec<ProviderMessage>) -> Result<Vec<Message>, KajiError> {
    messages
        .iter()
        .filter_map(|message| message.to_kaji_message().transpose())
        .collect()
}

fn convert_tools(tools: Vec<ProviderTool>) -> Result<Vec<Tool>, KajiError> {
    tools.iter().map(ProviderTool::to_kaji_tool).collect()
}
...
impl Provider {
    fn new(provider: Box<dyn KajiProvider>) -> Arc<Self> {
        Arc::new(Self {
...
#[uniffi::export]
pub fn declarative_provider_from_json(json: String) -> Result<Arc<Provider>, KajiError> {
    let provider = kaji_providers::declarative::from_json(&json, None, EnvKeyResolver {})?;
    Ok(Provider::new(provider))
}

#[uniffi::export]
pub fn openai_default_model() -> String {
    kaji_providers::openai::OPEN_AI_DEFAULT_MODEL.to_string()
}

#[uniffi::export]
pub fn openai_provider(api_key: String) -> Result<Arc<Provider>, KajiError> {
...
pub fn anthropic_default_model() -> String {
    kaji_providers::anthropic::ANTHROPIC_DEFAULT_MODEL.to_string()
}
...
pub fn anthropic_provider(
    ...
    .with_header(
        "anthropic-version",
        kaji_providers::anthropic::ANTHROPIC_API_VERSION,
    )?;
...
#[uniffi::export]
pub fn groq_provider(api_key: String) -> Result<Arc<Provider>, KajiError> {
    let json = kaji_providers::groq::JSON.replace("${GROQ_API_KEY}", &api_key);
    let provider = kaji_providers::declarative::from_json(
        &json,
        None,
        StaticKeyResolver {
...
fn groq_config() -> DeclarativeProviderConfig {
    kaji_providers::declarative::deserialize_provider_config(kaji_providers::groq::JSON)
...
impl kaji_providers::declarative::KeyResolver for StaticKeyResolver {
...
pub fn databricks_default_model() -> String {
    kaji_providers::databricks::DATABRICKS_DEFAULT_MODEL.to_string()
}

#[uniffi::export]
pub fn databricks_provider(host: String, token: String) -> Result<Arc<Provider>, KajiError> {
    let retry_config = KajiDatabricksProvider::load_retry_config(|key| std::env::var(key).ok());
    let provider = KajiDatabricksProvider::new(
        host,
...
pub fn databricks_v2_default_model() -> String {
    kaji_providers::databricks_v2::DATABRICKS_V2_DEFAULT_MODEL.to_string()
}

#[uniffi::export]
pub fn databricks_v2_provider(host: String, token: String) -> Result<Arc<Provider>, KajiError> {
    let retry_config = KajiDatabricksV2Provider::load_retry_config(|key| std::env::var(key).ok());
    let provider = KajiDatabricksV2Provider::new(
        host,
        DatabricksAuth::token(token),
        retry_config,
        None,
...
#[uniffi::export]
impl ProviderStream {
    pub async fn next_chunk(&self) -> Result<Option<StreamChunk>, KajiError> {
                ...
                match next {
                    ...
                    Some(Err(error)) => {
                        ...
                        return Ok(Some(StreamChunk::ErrorChunk {
                            error: KajiStreamError::from(KajiError::from(error)),
                        }));
                    }
                    None => {
                        state.ended = true;
...
fn message_to_chunks(message: Message) -> Vec<StreamChunk> {
    message
        ...
        .filter_map(|content| match content {
            KajiMessageContent::Text(text) if !text.text.is_empty() => {
                Some(StreamChunk::TextChunk {
                    text: text.text.clone(),
                })
            }
            KajiMessageContent::ToolRequest(request) => match request.tool_call {
            ...
            KajiMessageContent::Thinking(thinking) => Some(StreamChunk::ThinkingChunk {
                thinking: thinking.thinking,
                signature: thinking.signature,
            }),
            KajiMessageContent::RedactedThinking(redacted) => {
...
mod tests {
    ...
    fn model_config_rejects_invalid_request_params_json() {
        ...
        assert!(config.to_kaji_model_config().is_err());
    }
    ...
    fn tool_result_content_converts() {
        ...
        let KajiMessageContent::ToolResponse(response) = content else {
