...
use crate::config::{ExtensionConfig, KajiMode};
...
pub struct AcpProviderConfig {
    ...
    pub mode_mapping: HashMap<KajiMode, Vec<String>>,
...
pub struct AcpProvider {
    name: String,
    kaji_mode: Arc<Mutex<KajiMode>>,
    mode_mapping: HashMap<KajiMode, Vec<String>>,
...
impl AcpProvider {
    ...
    async fn start(
        ...
        let kaji_mode_shared = Arc::new(Mutex::new(kaji_mode));
    ...
    pub(crate) async fn send_set_mode(&self, _kaji_id: &str, mode_id: String) -> Result<()> {
...
impl Provider for AcpProvider {
    ...
    async fn update_mode(&self, session_id: &str, mode: KajiMode) -> Result<(), ProviderError> {
        ...
        if let Ok(mut guard) = self.kaji_mode.lock() {
            *guard = mode;
        }
        Ok(())
    }
    ...
    async fn stream(
        ...
        let kaji_mode = *self
            .kaji_mode
            .lock()
            .map_err(|_| ProviderError::RequestFailed("kaji_mode lock poisoned".into()))?;

        let reject_all_tools = kaji_mode == KajiMode::Chat;
                ...
                match update {
                    ...
                    AcpUpdate::ToolCallStart { id, name, kind, raw_input } => {
                        ...
                        if reject_all_tools {
                            ...
                            // call. kaji.acp.kind preserves ACP's stable categorization for
                    ...
                    AcpUpdate::PermissionRequest { request, response_tx } => {
                        text_run = None;
                        thought_run = None;
                        if let Some(decision) = permission_decision_from_mode(kaji_mode) {
    ...
    async fn run(
        ...
        Client
                        ...
                        match &notification.update {
                            SessionUpdate::CurrentModeUpdate(update) => {
                                if let Some(mode) = resolve_mode(
                                    ...
                                    if let Ok(mut guard) = kaji_mode.lock() {
                                        *guard = mode;
                                    }
                                }
                            }
                            ...
                            .and_then(|g| g.as_ref())
                        {
                            match notification.update {
                                ...
                                SessionUpdate::ToolCall(tool_call) => {
                                    ...
                                    // kaji.external_dispatch marker keeps `name` off the
            ...
            .connect_with(transport, async move |cx: ConnectionTo<Agent>| {
                handle_requests(config, kaji_mode, cx, rx, prompt_response_tx, init_tx).await
            })
            .await?;
...
async fn forward_child_stderr(mut stderr: tokio::process::ChildStderr) {
    ...
    loop {
        match stderr.read(&mut chunk).await {
            ...
            Err(e) => {
                tracing::debug!(target: "kaji::acp::child::stderr", error = %e, "stderr read error");
                break;
            }
        }
    }
...
fn emit_stderr_line(line: &mut Vec<u8>) {
    ...
    tracing::info!(target: "kaji::acp::child::stderr", "{}", String::from_utf8_lossy(trimmed));
    line.clear();
}
                ...
                let result = match session {
                    Ok(session) => {
                        ...
                        apply_session_config_options(&config, &cx, session.session_id.clone())
                            .await?;
                        apply_session_mode(&config, &kaji_mode, &cx, session).await
                    }
...
async fn apply_session_mode(
    ...
    let current_mode = kaji_mode.lock().ok().map(|mode| *mode);
...
fn build_handoff_context_memo(prior_messages: &[Message]) -> Option<String> {
    ...
    Some(format!(
        "Conversation context from kaji before this ACP provider session was created:\n\n\
{handoff_context}\n\n\
...
/// Convert ACP `ToolCallContent` blocks into the rmcp `Content` shape kaji's
...
fn reverse_mode_mapping(
    mode_mapping: &HashMap<KajiMode, Vec<String>>,
) -> HashMap<String, Vec<KajiMode>> {
    let mut reverse: HashMap<String, Vec<KajiMode>> = HashMap::new();
...
fn resolve_mode(
    reverse_modes: &HashMap<String, Vec<KajiMode>>,
    mode_id: &str,
    current: &Arc<Mutex<KajiMode>>,
) -> Option<KajiMode> {
...
fn permission_decision_from_mode(kaji_mode: KajiMode) -> Option<PermissionDecision> {
    match kaji_mode {
        KajiMode::Auto => Some(PermissionDecision::AllowOnce),
        KajiMode::Chat => Some(PermissionDecision::RejectOnce),
        KajiMode::Approve | KajiMode::SmartApprove => None,
    }
}
...
mod tests {
    ...
    fn test_provider_with_tx(
        ...
        (
            AcpProvider {
                ...
                kaji_mode: Arc::new(Mutex::new(KajiMode::Auto)),
    ...
    async fn chat_mode_denied_tool_messages_get_distinct_provider_ids() {
        ...
        let (tx, mut rx) = mpsc::channel(1);
        let (provider, model) = test_provider_with_tx(Some(tx));
        *provider.kaji_mode.lock().unwrap() = KajiMode::Chat;
    ...
    async fn get_context_limit_surfaces_captured_context_size() {
        ...
        assert_eq!(
            ...
            kaji_providers::model::DEFAULT_CONTEXT_LIMIT
        );
    ...
    #[test_case(KajiMode::Auto)]
    #[test_case(KajiMode::Approve)]
    #[test_case(KajiMode::SmartApprove)]
    #[test_case(KajiMode::Chat)]
    fn initial_mode_candidates_empty_when_mode_negotiation_disabled(mode: KajiMode) {
    ...
    fn initial_mode_candidates_prefer_mapping_then_fallback() {
        let mapping = HashMap::from([(KajiMode::Auto, vec!["bypassPermissions".to_string()])]);
        let config = test_acp_config(mapping, Some("default".to_string()));

        assert_eq!(
            initial_mode_candidates(&config, Some(KajiMode::Auto)),
            vec!["bypassPermissions".to_string()]
        );
        assert_eq!(
            initial_mode_candidates(&config, Some(KajiMode::Chat)),
    ...
    async fn update_mode_without_mapping_skips_acp_request_but_tracks_mode() {
        ...
        provider
            .update_mode("session", KajiMode::Chat)
            .await
            .unwrap();

        assert!(rx.try_recv().is_err());
        assert_eq!(*provider.kaji_mode.lock().unwrap(), KajiMode::Chat);
    }
    ...
    async fn update_mode_with_mapping_sends_set_mode() {
        let (tx, mut rx) = mpsc::channel(1);
        let (mut provider, _) = test_provider_with_tx(Some(tx));
        provider.mode_mapping = HashMap::from([(KajiMode::Chat, vec!["plan".to_string()])]);
    ...
    async fn update_mode_sends_candidate_offered_by_agent() {
        ...
        let handle = tokio::spawn(async move {
            provider
                .update_mode("session", KajiMode::Auto)
                .await
                .unwrap();
            provider
        });
        ...
        let provider = handle.await.unwrap();
        assert_eq!(*provider.kaji_mode.lock().unwrap(), KajiMode::Auto);
    }
    ...
    async fn update_mode_errors_when_no_candidate_offered() {
        let (tx, mut rx) = mpsc::channel(1);
        let (mut provider, _) = test_provider_with_tx(Some(tx));
        provider.mode_mapping = HashMap::from([(KajiMode::Chat, vec!["read-only".to_string()])]);
        provider.session.response = NewSessionResponse::new("test-session")
            .modes(mode_state("agent", &["agent", "agent-full-access"]));

        let result = provider.update_mode("session", KajiMode::Chat).await;
    ...
    #[test_case(KajiMode::Auto => Some(PermissionDecision::AllowOnce) ; "auto allows")]
    #[test_case(KajiMode::Chat => Some(PermissionDecision::RejectOnce) ; "chat rejects")]
    #[test_case(KajiMode::Approve => None ; "approve defers")]
    #[test_case(KajiMode::SmartApprove => None ; "smart_approve defers")]
    fn test_permission_decision_from_mode(mode: KajiMode) -> Option<PermissionDecision> {
        permission_decision_from_mode(mode)
    }

    #[test_case(
        HashMap::from([
            (KajiMode::Auto, vec!["yolo".to_string()]),
            (KajiMode::Approve, vec!["default".to_string()]),
            (KajiMode::SmartApprove, vec!["auto_edit".to_string()]),
            (KajiMode::Chat, vec!["plan".to_string()]),
        ]),
        HashMap::from([
            ("yolo".to_string(), vec![KajiMode::Auto]),
            ("default".to_string(), vec![KajiMode::Approve]),
            ("auto_edit".to_string(), vec![KajiMode::SmartApprove]),
            ("plan".to_string(), vec![KajiMode::Chat]),
        ])
        ; "gemini provider mapping"
    )]
    #[test_case(
        HashMap::from([
            (KajiMode::Auto, vec!["bypassPermissions".to_string()]),
            (KajiMode::Approve, vec!["default".to_string()]),
            (KajiMode::SmartApprove, vec!["acceptEdits".to_string()]),
            (KajiMode::Chat, vec!["plan".to_string()]),
        ]),
        HashMap::from([
            ("bypassPermissions".to_string(), vec![KajiMode::Auto]),
            ("default".to_string(), vec![KajiMode::Approve]),
            ("acceptEdits".to_string(), vec![KajiMode::SmartApprove]),
            ("plan".to_string(), vec![KajiMode::Chat]),
        ])
    ...
    #[test_case(
        HashMap::from([
            (KajiMode::Auto, vec!["full-access".to_string(), "agent-full-access".to_string()]),
            (KajiMode::Approve, vec!["read-only".to_string()]),
            (KajiMode::SmartApprove, vec!["auto".to_string(), "agent".to_string()]),
            (KajiMode::Chat, vec!["read-only".to_string()]),
        ]),
        HashMap::from([
            ("full-access".to_string(), vec![KajiMode::Auto]),
            ("agent-full-access".to_string(), vec![KajiMode::Auto]),
            ("read-only".to_string(), vec![KajiMode::Approve, KajiMode::Chat]),
            ("auto".to_string(), vec![KajiMode::SmartApprove]),
            ("agent".to_string(), vec![KajiMode::SmartApprove]),
        ])
        ; "codex candidates for both bridge generations"
    )]
    fn test_reverse_mode_mapping(
        forward: HashMap<KajiMode, Vec<String>>,
        expected: HashMap<String, Vec<KajiMode>>,
    ) {
    ...
    fn duplicate_read_only_reverse_modes() -> HashMap<String, Vec<KajiMode>> {
        HashMap::from([
            ("full-access".to_string(), vec![KajiMode::Auto]),
            (
                "read-only".to_string(),
                vec![KajiMode::Approve, KajiMode::Chat],
            ),
            ("auto".to_string(), vec![KajiMode::SmartApprove]),
        ])
    }
    ...
    #[test_case(
        "full-access", KajiMode::Auto, Some(KajiMode::Auto)
        ; "unique mapping returns the only candidate"
    )]
    #[test_case(
        "read-only", KajiMode::Approve, Some(KajiMode::Approve)
        ; "duplicate prefers current when current is Approve"
    )]
    #[test_case(
        "read-only", KajiMode::Chat, Some(KajiMode::Chat)
        ; "duplicate prefers current when current is Chat"
    )]
    #[test_case(
        "read-only", KajiMode::Auto, Some(KajiMode::Approve)
        ; "duplicate falls back to first when current not in candidates"
    )]
    #[test_case(
        "unknown-id", KajiMode::Auto, None
        ; "unknown mode id returns None"
    )]
    fn test_resolve_mode(mode_id: &str, current: KajiMode, expected: Option<KajiMode>) {
        ...
        if mode_id == "read-only" && expected == Some(KajiMode::Approve) {
            assert!(result == Some(KajiMode::Approve) || result == Some(KajiMode::Chat));
        } else {
    ...
    fn tool_meta_pairs_external_dispatch_marker_with_acp_kind() {
        ...
        for (kind, expected) in cases {
            ...
            assert_eq!(
                tool_meta["kaji.acp.kind"],
                serde_json::Value::String(expected.to_string()),
                "kaji.acp.kind serialized wrong for kind={kind:?}"
            );
        }
    }
}
