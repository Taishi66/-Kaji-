...
///
/// Replaces the removed `SessionModelState` ACP schema type; kaji now surfaces
...
pub(super) fn build_mode_state(
    current_mode: KajiMode,
) -> Result<SessionModeState, agent_client_protocol::Error> {
    let mut available = Vec::with_capacity(KajiMode::VARIANTS.len());
    for &name in KajiMode::VARIANTS {
        let kaji_mode: KajiMode = name.parse().map_err(|_| {
            agent_client_protocol::Error::internal_error() // impossible but satisfy linters
                .data(format!("Failed to parse KajiMode variant: {}", name))
        })?;
        let mut mode = SessionMode::new(SessionModeId::new(name), name);
        mode.description = kaji_mode.get_message().map(Into::into);
        available.push(mode);
    }
...
pub(super) async fn build_session_setup_config(
    ...
    let mode_state = build_mode_state(session.kaji_mode)?;
...
fn current_thinking_effort_value(model_config: &ModelConfig) -> String {
    if model_config.is_reasoning_model() {
        model_config
            .thinking_effort()
            .or_else(|| Config::global().get_kaji_thinking_effort())
...
mod tests {
    ...
    #[test_case(
        build_mode_state(KajiMode::Auto).unwrap(),
        "openai",
        vec![
    ...
    #[test_case(
        build_mode_state(KajiMode::Approve).unwrap(),
        "openai",
    ...
    fn test_build_config_options_uses_current_thinking_effort() {
        let mode_state = build_mode_state(KajiMode::Auto).unwrap();
