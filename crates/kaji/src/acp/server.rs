...
use crate::agents::mcp_client::{KajiMcpHostInfo, McpClientTrait};
use crate::agents::platform_extensions::developer::DeveloperClient;
use crate::agents::{
    Agent, AgentConfig, ExtensionConfig, ExtensionLoadResult, KajiPlatform, SessionConfig,
};
...
pub(super) const DEFAULT_PROVIDER_ID: &str = "kaji";
pub(super) const DEFAULT_PROVIDER_LABEL: &str = "Kaji (Default)";
...
/// a durable, append-only exchange of messages. Internally, kaji also has a concept
...
pub struct KajiAcpAgentOptions {
...
pub struct KajiAcpAgent {
    sessions: Arc<Mutex<HashMap<String, KajiAcpSession>>>,
    ...
    client_mcp_host_info: OnceCell<KajiMcpHostInfo>,
    client_supports_acp_elicitation: OnceCell<bool>,
    client_supports_kaji_custom_notifications: OnceCell<bool>,
...
fn agent_capabilities_meta() -> Option<Meta> {
    let mut kaji = serde_json::Map::new();
    if cfg!(feature = "local-inference") {
        kaji.insert("localInference".to_string(), serde_json::json!({}));
    }
    ...
    meta.insert("kaji".to_string(), serde_json::Value::Object(kaji));
    Some(meta)
}
...
struct ClientCapabilitiesMeta {
    #[serde(default)]
    kaji: Option<KajiClientCapabilities>,
}

#[derive(Debug, Default, Deserialize)]
struct KajiClientCapabilities {
    #[serde(rename = "mcpHostCapabilities", default)]
    mcp_host_capabilities: Option<KajiMcpHostCapabilities>,
...
#[derive(Debug, Default, Deserialize)]
struct KajiMcpHostCapabilities {
    #[serde(default)]
...
fn extract_client_mcp_host_info(
    ...
    kaji_client_capabilities: Option<&KajiClientCapabilities>,
) -> KajiMcpHostInfo {
    let host_capabilities =
        kaji_client_capabilities.and_then(|kaji| kaji.mcp_host_capabilities.as_ref());
...
fn extract_use_login_shell_path(args: &InitializeRequest) -> bool {
    args.meta
        .as_ref()
        .and_then(|meta| meta.get("kaji/useLoginShellPath"))
...
fn resolve_default_provider_model_config(
    config: &Config,
) -> Result<(String, kaji_providers::model::ModelConfig), agent_client_protocol::Error> {
    let resolved_provider = config.get_kaji_provider().map_err(|error| {
        agent_client_protocol::Error::internal_error()
            .data(format!("Failed to resolve provider: {}", error))
    })?;
    let resolved_model = config.get_kaji_model().map_err(|error| {
        agent_client_protocol::Error::internal_error()
...
async fn resolve_provider_default_model_config(
    provider_name: &str,
) -> Result<kaji_providers::model::ModelConfig, agent_client_protocol::Error> {
...
pub(super) struct UsageUpdates {
    pub(super) custom: KajiSessionNotification,
...
pub(super) fn build_usage_updates(
    ...
    Some(UsageUpdates {
        custom: KajiSessionNotification {
            session_id: session.id.clone(),
            update: KajiSessionUpdate::UsageUpdate(SessionUsageUpdate {
                used,
...
impl KajiAcpAgent {
    ...
    pub(super) fn supports_kaji_custom_notifications(&self) -> bool {
        self.client_supports_kaji_custom_notifications
            .get()
            .copied()
            .unwrap_or(false)
    }
    ...
    pub(super) async fn notify_session_setup(
        ...
        send_session_setup_notifications(
            ...
            session,
            &totals,
            self.supports_kaji_custom_notifications(),
        )
    }
    ...
    // TODO: kaji reads Paths::in_state_dir globally (e.g. RequestLog), ignoring this data_dir.
    pub async fn new(options: KajiAcpAgentOptions) -> Result<Self> {
        ...
        let agent_config = AgentConfig::new(
            ...
            options.scheduler,
            Config::global().get_kaji_mode().unwrap_or_default(),
            options.disable_session_naming,
            options.kaji_platform.clone(),
        );
        ...
        Ok(Self {
            ...
            client_supports_kaji_custom_notifications: OnceCell::new(),
    ...
    async fn maybe_refresh_provider_inventory_with_agent(
        &self,
        kaji_session: &Session,
        agent: &Arc<Agent>,
    ) {
        let Some(provider_name) = kaji_session.provider_name.as_deref() else {
            return;
        };
    ...
    fn initial_session_extensions(
        ...
        kaji_extensions: Option<Vec<KajiExtension>>,
        ...
        } else if let Some(kaji_extensions) = kaji_extensions {
            for extension in extensions::kaji_extensions_to_configs(kaji_extensions)? {
    ...
    async fn register_acp_session(&self, session_id: String, agent: Arc<Agent>) {
        let acp_session = KajiAcpSession { agent };
...
fn extract_client_supports_kaji_custom_notifications(
    kaji_client_capabilities: Option<&KajiClientCapabilities>,
) -> bool {
    kaji_client_capabilities
        .and_then(|kaji| kaji.custom_notifications)
        .unwrap_or(false)
}
...
fn extract_client_supports_recipe_param_requests(
    ...
    kaji_client_capabilities
        .and_then(|kaji| kaji.recipe_parameter_requests)
        .unwrap_or(false)
}
...
fn send_status_message_update(
    cx: &ConnectionTo<Client>,
    supports_kaji_custom_notifications: bool,
    session_id: &str,
    notification: &SystemNotificationContent,
) -> Result<(), agent_client_protocol::Error> {
    if let Some(status) = status_message_from_system_notification(notification) {
        if supports_kaji_custom_notifications {
            cx.send_notification(KajiSessionNotification {
                session_id: session_id.to_string(),
                update: KajiSessionUpdate::StatusMessage(StatusMessageUpdate { status }),
            })?;
        }
    }
    Ok(())
...
fn send_progress_message_update(
    ...
    if supports_kaji_custom_notifications {
        cx.send_notification(KajiSessionNotification {
            session_id: session_id.to_string(),
            update: KajiSessionUpdate::StatusMessage(StatusMessageUpdate {
...
impl KajiAcpAgent {
    async fn on_initialize(
        ...
        let kaji_client_capabilities =
            extract_client_capabilities_meta(&args).and_then(|meta| meta.kaji);
        let _ = self.client_mcp_host_info.set(extract_client_mcp_host_info(
            &args,
            kaji_client_capabilities.as_ref(),
        ));
        let _ = self.client_supports_kaji_custom_notifications.set(
            extract_client_supports_kaji_custom_notifications(kaji_client_capabilities.as_ref()),
        );
        let _ = self.client_supports_recipe_param_requests.set(
            extract_client_supports_recipe_param_requests(kaji_client_capabilities.as_ref()),
        );
        let client_requests_tool_call_label_enrichment = kaji_client_capabilities
            .as_ref()
            .and_then(|kaji| kaji.tool_call_label_enrichment)
            .unwrap_or(false);
        let _ = self
        ...
        Ok(InitializeResponse::new(args.protocol_version)
            .agent_info(Implementation::new("kaji", env!("CARGO_PKG_VERSION")))
            .agent_capabilities(capabilities)
            .auth_methods(vec![AuthMethod::Agent(
                AuthMethodAgent::new("kaji-provider", "Configure Provider")
                    .description("Run `kaji configure` to set up your AI provider and API key"),
            )]))
    }
    ...
    async fn start_active_run(
        ...
        if let Some(active_run) = active_prompt_runs.get(session_id) {
            return Err(agent_client_protocol::Error::invalid_params().data(format!(
                "session already has active run `{}`; use _kaji/unstable/session/steer",
    ...
    async fn on_prompt(
        ...
        while let Some(event) = stream.next().await {
            ...
            match event {
                ...
                Ok(crate::agents::AgentEvent::MessageUsage { message_id, usage }) => {
                    if self.supports_kaji_custom_notifications() {
                        cx.send_notification(KajiSessionNotification {
                            session_id: session_id.clone(),
                            update: KajiSessionUpdate::MessageUsage(message_usage_update(
                                message_id, &usage,
                            )),
                        })?;
                    }
        ...
        if let Some(updates) = build_usage_updates(&session, &totals) {
            ...
            // migrated to `_kaji/unstable/session/update`.
            cx.send_notification(SessionNotification::new(
    ...
    async fn build_config_update(
        ...
        let kaji_mode = agent.kaji_mode().await;
        let inventory = self
            .provider_inventory
        ...
        let mode_state = build_mode_state(kaji_mode)?;
    ...
    async fn on_set_mode(
        ...
        let mode = mode_id.parse::<KajiMode>().map_err(|_| {
            agent_client_protocol::Error::invalid_params()
        ...
        let agent = self.get_session_agent(session_id).await?;
        agent
            .update_kaji_mode(mode, session_id)
            .await
            .internal_err_ctx("Failed to update mode")?;

        // kaji_mode is already updated on the session above.
    ...
    async fn on_set_thinking_effort(
        ...
        let effort = effort_id
            .parse::<kaji_providers::thinking::ThinkingEffort>()
            .map_err(|_| {
                agent_client_protocol::Error::invalid_params()
...
pub struct KajiAcpHandler {
    pub agent: Arc<KajiAcpAgent>,
}

pub fn serve<R, W>(
    agent: Arc<KajiAcpAgent>,
    read: R,
    write: W,
...
{
    Box::pin(async move {
        let handler = KajiAcpHandler { agent };
...
pub async fn run(builtins: Vec<String>, enable_scheduler: bool) -> Result<()> {
    ...
    let server = crate::acp::server_factory::AcpServer::new(
        crate::acp::server_factory::AcpServerFactoryConfig {
            ...
            kaji_platform: KajiPlatform::KajiCli,
...
mod tests {
    ...
    use kaji_providers::conversation::token_usage::Usage as TokenUsage;
    use std::io::Write;
...
```",
    ...
    fn test_build_usage_update_clamps_negative_used_to_zero() {
        ...
        session.model_config = Some(
            kaji_providers::model::ModelConfig::new("test-model").with_context_limit(Some(258_000)),
        );
        ...
        let usage = match updates.custom.update {
            KajiSessionUpdate::UsageUpdate(usage) => usage,
    ...
    #[test]
    fn test_kaji_custom_notifications_capability_defaults_to_false() {
        let request =
            InitializeRequest::new(agent_client_protocol::schema::ProtocolVersion::LATEST);
        let kaji_client_capabilities =
            extract_client_capabilities_meta(&request).and_then(|meta| meta.kaji);

        assert!(!extract_client_supports_kaji_custom_notifications(
            kaji_client_capabilities.as_ref()
        ));
    }

    #[test]
    fn test_kaji_custom_notifications_capability_reads_client_meta() {
        let mut kaji_meta = serde_json::Map::new();
        kaji_meta.insert(
            "customNotifications".to_string(),
            serde_json::Value::Bool(true),
        );
        let mut meta = serde_json::Map::new();
        meta.insert("kaji".to_string(), serde_json::Value::Object(kaji_meta));
    ...
    fn test_tool_call_label_enrichment_capability() {
        ...
        let kaji_client_capabilities =
            extract_client_capabilities_meta(&request).and_then(|meta| meta.kaji);
        assert!(!kaji_client_capabilities
            .and_then(|kaji| kaji.tool_call_label_enrichment)
            .unwrap_or(false));
