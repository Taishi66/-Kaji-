...
impl KajiAcpAgent {
    ...
    pub(super) async fn on_config_read(
        ...
        if req.key == "KAJI_PROVIDER" || req.key == "active_provider" {
            let value = config
                .get_kaji_provider()
    ...
    pub(super) async fn on_config_upsert(
        ...
        if req.key == "KAJI_MODEL" {
            if let Some(model) = req.value.as_str() {
                if let Ok(provider) = config.get_kaji_provider() {
    ...
    pub(super) async fn on_config_remove(
        ...
        } else if req.key == "KAJI_PROVIDER" || req.key == "active_provider" {
            config.delete("active_provider").internal_err()?;
            config.delete("KAJI_PROVIDER").internal_err()?;
        } else if req.key == "KAJI_MODEL" {
            if let Ok(provider) = config.get_kaji_provider() {
                crate::config::set_active_provider(config, &provider, "").internal_err()?;
            }
            config.delete("KAJI_MODEL").internal_err()?;
        } else {
    ...
    pub(super) async fn on_defaults_read(
        ...
        Ok(DefaultsReadResponse {
            provider_id: config.get_kaji_provider().ok(),
            model_id: config.get_kaji_model().ok(),
        })
    }
...
const PREFERENCE_DEFS: &[PreferenceDef] = &[
    PreferenceDef {
        ...
        config_key: "KAJI_AUTO_COMPACT_THRESHOLD",
        prepare: prepare_auto_compact_threshold,
    },
    PreferenceDef {
        key: PreferenceKey::KajiThinkingEffort,
        config_key: "KAJI_THINKING_EFFORT",
...
fn prepare_thinking_effort(
    ...
    let Some(value) = value.as_str() else {
        return Err(agent_client_protocol::Error::invalid_params()
            .data("kajiThinkingEffort must be a string"));
    };
    let effort = value.parse::<ThinkingEffort>().map_err(|err| {
        agent_client_protocol::Error::invalid_params()
            .data(format!("Invalid kajiThinkingEffort: {err}"))
    })?;
