...
impl KajiAcpAgent {
    pub(super) async fn on_add_session_extension(
        ...
        let config = kaji_extension_to_config_without_secrets(req.extension)?;
    ...
    pub(super) async fn on_get_config_extensions(
        ...
        let extensions = extensions
            .into_iter()
            .map(config_entry_to_kaji_entry)
    ...
    pub(super) async fn on_get_available_extensions(
        ...
        let extensions = crate::config::get_available_extensions()
            .into_iter()
            .map(|config| config_to_kaji_extension(&config))
            .collect::<Result<Vec<_>, _>>()?
            .into_iter()
            .flatten()
            .collect::<Vec<_>>();
    ...
    pub(super) async fn on_add_config_extension(
        ...
        let conversion = kaji_extension_to_config(req.extension)?;
...
fn config_to_kaji_extension(
    config: &ExtensionConfig,
) -> Result<Option<KajiExtension>, agent_client_protocol::Error> {
...
fn kaji_extension_to_config(
    extension: KajiExtension,
...
fn kaji_extension_to_config_without_secrets(
    extension: KajiExtension,
) -> Result<ExtensionConfig, agent_client_protocol::Error> {
    let conversion = kaji_extension_to_config(extension)?;
...
pub(super) fn kaji_extensions_to_configs(
    extensions: Vec<KajiExtension>,
) -> Result<Vec<ExtensionConfig>, agent_client_protocol::Error> {
    extensions
        .into_iter()
        .map(kaji_extension_to_config_without_secrets)
        .collect()
}

fn config_entry_to_kaji_entry(
    entry: ExtensionEntry,
) -> Result<Option<KajiExtensionEntry>, agent_client_protocol::Error> {
    let config_key = entry.config.key();
    let Some(extension) = config_to_kaji_extension(&entry.config)? else {
        return Ok(None);
    };
    Ok(Some(KajiExtensionEntry {
        extension,
...
mod tests {
    ...
    #[test]
    fn builtin_config_converts_to_kaji_builtin_extension() {
        ...
        let extension = config_to_kaji_extension(&config)
            .expect("conversion should succeed")
            .expect("builtin should be supported");

        let KajiExtension::Builtin {
            name,
            description,
            display_name,
            timeout,
    ...
    #[test]
    fn platform_config_converts_to_kaji_platform_extension() {
    ...
    #[test]
    fn stdio_config_converts_to_kaji_mcp_extension_without_literal_envs() {
    ...
    #[test]
    fn streamable_http_config_converts_to_kaji_mcp_extension_without_literal_envs() {
    ...
    fn inline_python_config_is_skipped() {
        ...
        let extension = config_to_kaji_extension(&config).expect("conversion should succeed");
    ...
    #[test]
    fn kaji_mcp_stdio_extension_converts_to_config_without_literal_envs() {
        let extension = KajiExtension::Mcp {
        ...
        let conversion = kaji_extension_to_config(extension).expect("conversion should succeed");
    ...
    #[test]
    fn kaji_mcp_stdio_extension_extracts_literal_envs_for_config_add() {
        let extension = KajiExtension::Mcp {
    ...
    #[test]
    fn kaji_mcp_streamable_http_extension_converts_to_config_without_literal_envs() {
        let extension = KajiExtension::Mcp {
    ...
    #[test]
    fn kaji_builtin_extension_converts_to_config() {
        let builtin = KajiExtension::Builtin {
            name: "developer".to_string(),
        ...
        let conversion = kaji_extension_to_config(builtin).expect("conversion should succeed");
        assert!(conversion.secret_updates.is_empty());
    ...
    #[test]
    fn kaji_platform_extension_converts_to_config() {
        let platform = KajiExtension::Platform {
            name: "todo".to_string(),
        ...
        let conversion = kaji_extension_to_config(platform).expect("conversion should succeed");
        assert!(conversion.secret_updates.is_empty());
    ...
    #[test]
    fn kaji_mcp_sse_extension_is_rejected_for_config_add() {
        let extension = KajiExtension::Mcp {
        ...
        assert!(kaji_extension_to_config(extension).is_err());
    }
}
