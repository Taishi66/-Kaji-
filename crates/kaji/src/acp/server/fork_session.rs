...
impl KajiAcpAgent {
    ...
    pub(super) async fn handle_fork_session(
        ...
        let (agent, extension_results) = self.prepare_acp_session_agent(cx, &kaji_session).await?;
        self.apply_session_recipe(&agent, &kaji_session).await?;
        self.register_acp_session(kaji_session.id.clone(), agent)
            .await;

        let acp_session_id = SessionId::new(new_session_id.clone());
        let mut meta = session_meta(&kaji_session);
        if let Ok(v) = serde_json::to_value(&extension_results) {
            meta.insert("extensionResults".to_string(), v);
        }

        let (mode_state, config_options) =
            build_session_setup_config(&self.provider_inventory, &kaji_session).await?;
        ...
        self.notify_session_setup(cx, &kaji_session).await?;
        Ok(response)
    }
}
