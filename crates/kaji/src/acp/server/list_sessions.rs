use super::{build_session_info, meta_string, KajiAcpAgent, ResultExt};
...
struct SessionListCursorToken {
    ...
    // Kaji stores timestamps with second precision in common write paths, so the
...
fn include_last_message_snippet_from_meta(
    ...
    let Some(value) = meta.and_then(|meta| meta.get("kaji")) else {
        return Ok(false);
    };
    if value.is_null() {
        return Ok(false);
    ...
    let Some(kaji_meta) = value.as_object() else {
        return Err(agent_client_protocol::Error::invalid_params().data("kaji must be an object"));
    };
    let Some(value) = kaji_meta.get("includeLastMessageSnippet") else {
        return Ok(false);
    };
    if value.is_null() {
        return Ok(false);
    ...
    value.as_bool().ok_or_else(|| {
        agent_client_protocol::Error::invalid_params()
            .data("kaji.includeLastMessageSnippet must be a boolean")
    })
}
