...
fn kaji_message_meta(message: &Message, steer: bool) -> serde_json::Map<String, serde_json::Value> {
    let message_meta = KajiMessageMeta {
...
fn extend_message_meta(meta: &mut Meta, message: &Message, steer: bool) {
    let message_kaji = kaji_message_meta(message, steer);
    let kaji_value = meta
        .entry("kaji".to_string())
        .or_insert_with(|| serde_json::Value::Object(serde_json::Map::new()));

    if let serde_json::Value::Object(kaji) = kaji_value {
        kaji.extend(message_kaji);
    } else {
        *kaji_value = serde_json::Value::Object(message_kaji);
    }
}
...
mod tests {
    ...
    fn message_meta_serializes_message_fields() {
        ...
        assert_eq!(
            message_meta(&steer_message).get("kaji"),
            Some(&serde_json::json!({
                "created": 1_700_000_000,
                "messageId": "msg_live",
                "steer": true,
        ...
        assert_eq!(
            message_meta_without_steer(&steer_message).get("kaji"),
            Some(&serde_json::json!({
                "created": 1_700_000_000,
                "messageId": "msg_live",
            })),
        ...
        assert_eq!(
            message_meta(&message_without_id).get("kaji"),
            Some(&serde_json::json!({
                "created": 1_700_000_000,
            })),
        );
        ...
        assert_eq!(
            message_meta(&limited_message).get("kaji"),
            Some(&serde_json::json!({
                "created": 1_700_000_000,
                "messageId": "msg_live",
    ...
    fn content_chunk_carries_message_id_and_metadata() {
        ...
        assert_eq!(
            chunk.meta.as_ref().and_then(|meta| meta.get("kaji")),
            Some(&serde_json::json!({
                "created": 1_700_000_000,
                "messageId": "msg_live",
                "outputTokenLimitReached": true,
    ...
    #[test]
    fn merge_message_meta_replaces_non_object_kaji_metadata() {
