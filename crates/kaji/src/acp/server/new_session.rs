use crate::acp::custom_requests::KajiExtension;
...
impl KajiAcpAgent {
    pub(super) async fn handle_new_session(
        ...
        let current_mode: KajiMode = config.get_kaji_mode().unwrap_or_default();
    ...
    async fn configure_new_session(
        ...
        let kaji_extensions = meta_kaji_extensions(args.meta.as_ref())?;
    ...
    async fn resolve_provider_and_model(
        ...
        let recipe_provider = recipe_settings.and_then(|s| s.kaji_provider.clone());
        let recipe_model = recipe_settings.and_then(|s| s.kaji_model.clone());
            ...
            None => match meta_string(meta, "provider")? {
                ...
                None => {
                    if let Some(model) = recipe_model.as_deref() {
                        let provider = config.get_kaji_provider().map_err(|error| {
...
fn meta_kaji_extensions(
    meta: Option<&Meta>,
) -> Result<Option<Vec<KajiExtension>>, agent_client_protocol::Error> {
