...
impl KajiAcpAgent {
    ...
    pub(super) async fn on_list_provider_supported_models(
        ...
        let models = match provider.fetch_supported_models().await {
            Ok(models) => models,
            Err(kaji_providers::errors::ProviderError::Authentication(error)) => {
                return Err(agent_client_protocol::Error::auth_required().data(error));
            }
            Err(kaji_providers::errors::ProviderError::NotConfigured) => {
                return Err(agent_client_protocol::Error::invalid_params()
    ...
    pub(super) async fn on_delete_custom_provider(
        ...
        if Config::global().get_kaji_provider().ok().as_deref() == Some(req.provider_id.as_str()) {
