...
use kaji_sdk_types::custom_requests::{ToolListItem, ToolPermissionLevel};
...
impl KajiAcpAgent {
            ...
            .map(|tool| {
                let permission = permission_manager
                    ...
                    .or_else(|| {
                        ...
                        } else if kaji_mode == KajiMode::Approve {
    ...
    pub(super) async fn on_call_tool(
        &self,
        req: KajiToolCallRequest,
    ) -> Result<KajiToolCallResponse, agent_client_protocol::Error> {
        let session_id = &req.session_id;
        let agent = self.get_session_agent(&req.session_id).await?;
