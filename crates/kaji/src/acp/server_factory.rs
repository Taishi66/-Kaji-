use crate::acp::server::{
    AcpBuiltinSelection, AcpProviderFactory, KajiAcpAgent, KajiAcpAgentOptions,
};
...
impl AcpServer {
    ...
    /// headless `kaji serve` runs scheduled jobs; on failure `create_agent`
    ...
    pub async fn create_agent(&self) -> Result<Arc<KajiAcpAgent>> {
        let config = crate::config::Config::global();
        let disable_session_naming = config.get_kaji_disable_session_naming().unwrap_or(false);
        ...
        let agent = KajiAcpAgent::new(KajiAcpAgentOptions {
            ...
            kaji_platform: self.config.kaji_platform.clone(),
