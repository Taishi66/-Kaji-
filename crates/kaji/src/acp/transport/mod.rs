...
use crate::acp::server::KajiAgentConnection;
use crate::acp::server_factory::AcpServer;

// The upstream ACP HTTP server only supports exact origin allowlists for
// WebSocket upgrades; Kaji applies its richer loopback predicate before this.
...
/// `kaji serve`. This allows the `x-secret-key` auth header the proxy routes
/// rely on.
...
fn create_acp_router_inner(server: Arc<AcpServer>, policy: AcpOriginPolicy) -> Router {
    AcpHttpServer::new(move || KajiAgentConnection::new(server.clone()))
...
/// or kaji-specific auxiliary routes.
...
/// The full standalone ACP server router used by `kaji serve`: ACP transport,
