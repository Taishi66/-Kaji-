...
const COMPACTION_PROGRESS_TEXT: &str = "kaji is compacting the conversation...";
...
pub(crate) fn stop_hook_block_cap_warning(plugin: &str, cap: u32) -> Message {
    Message::assistant().with_system_notification(
        SystemNotificationType::InlineMessage,
        format!(
            "Stop hook `{plugin}` blocked the turn from ending more than {cap} consecutive times — overriding and ending turn to avoid an infinite loop. Set KAJI_STOP_HOOK_BLOCK_CAP to raise this limit."
        ),
    )
}
...
pub struct ReplyContext {
    ...
    pub model_config: kaji_providers::model::ModelConfig,
}
...
#[derive(Clone, Debug)]
pub enum KajiPlatform {
    KajiDesktop,
    KajiCli,
}

impl fmt::Display for KajiPlatform {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            KajiPlatform::KajiCli => write!(f, "kaji-cli"),
            KajiPlatform::KajiDesktop => write!(f, "kaji-desktop"),
        }
    }
}
...
pub struct AgentConfig {
    ...
    pub kaji_platform: KajiPlatform,
    pub mcp_host_info: Option<KajiMcpHostInfo>,
...
impl AgentConfig {
    ...
    pub fn with_mcp_host_info(mut self, mcp_host_info: Option<KajiMcpHostInfo>) -> Self {
    ...
    fn resolve_use_login_shell_path(&self) -> bool {
        resolve_use_login_shell_path(self.use_login_shell_path, &self.kaji_platform)
    }
}

fn resolve_use_login_shell_path(explicit: Option<bool>, platform: &KajiPlatform) -> bool {
    explicit.unwrap_or(matches!(platform, KajiPlatform::KajiDesktop))
}
...
pub struct Agent {
    ...
    pub(super) current_kaji_mode: Mutex<KajiMode>,
...
/// La racine du projet courant : le premier ancêtre du répertoire de travail
/// qui porte un `.git`, sinon le répertoire lui-même. `.kaji/hooks.yaml` vit à
/// la racine du dépôt ; le chercher dans le seul `current_dir` le rendrait
...
impl Agent {
    pub fn new() -> Self {
        ...
        Self::with_config(AgentConfig::new(
            ...
            None,
            config.get_kaji_mode().unwrap_or_default(),
            config.get_kaji_disable_session_naming().unwrap_or(false),
    ...
    pub fn with_config(config: AgentConfig) -> Self {
        ...
        let kaji_platform = config.kaji_platform.clone();
        let initial_mode = config.kaji_mode;
        ...
        let mcpui = explicit_mcp_host_info
            ...
            .map(KajiMcpHostInfo::mcpui_enabled)
            .unwrap_or_else(|| match config.kaji_platform {
                KajiPlatform::KajiDesktop => true,
                KajiPlatform::KajiCli => false,
            });
        ...
        let client_name = explicit_mcp_host_info
            ...
            .unwrap_or_else(|| kaji_platform.to_string());
        ...
        Self {
            ...
            config,
            current_kaji_mode: Mutex::new(initial_mode),
            ...
            hook_manager: crate::hooks::HookManager::load(
                project_root_from_cwd().as_deref(),
                use_login_shell_path,
            ),
            #[cfg(test)]
    ...
    pub(crate) fn stop_hook_block_cap(&self) -> u32 {
        ...
        Config::global()
            .get_param::<u32>("KAJI_STOP_HOOK_BLOCK_CAP")
    ...
    /// premier tour d'une session que l'utilisateur ouvre lui-même, puis
    ...
    async fn apply_prompt_hooks(&self, session_id: &str, user_message: Message) -> Message {
        ...
        if self.is_replay() {
            if let Some(replay) = self.replay_source() {
                for event in [HookEvent::SessionStart, HookEvent::UserPromptSubmit] {
                    if let Some(output) = replay.hook_output(event.config_name(), "") {
                        debug!(
                            %event,
                            bytes = output.len(),
        ...
        let wants_session_start = self.hook_manager.has_hooks(HookEvent::SessionStart);
        let wants_prompt_submit = self.hook_manager.has_hooks(HookEvent::UserPromptSubmit);
        if !wants_session_start && !wants_prompt_submit {
            return user_message;
        }
        ...
        let session = match self
            .config
            .session_manager
            .get_session(session_id, wants_session_start)
            .await
        {
            Ok(session) => session,
            Err(error) => {
        ...
        if wants_session_start && opens_a_user_session(&session) {
    ...
    /// Le hook de fin de tour, au même endroit que la borne qu'il accompagne :
    /// un seul site pour les deux boucles. Sa sortie n'entre nulle part — rien
    /// à journaliser, rien à servir au rejeu, et le manager de rejeu ne
    /// l'exécute pas.
    async fn emit_turn_end_hook(
        ...
        hooks
            .emit(
                ...
                crate::hooks::HookContext::new(crate::hooks::HookEvent::TurnEnd, session_id)
    ...
    /// `prior_blocks` adresse la décision au journal : un hook `stop` peut
    /// refuser plusieurs fois dans le même tour, et chaque évaluation sauf la
    /// dernière est un refus — le nombre de refus déjà rendus identifie donc
    /// l'évaluation en cours, à l'enregistrement comme au rejeu, sans compteur
    /// à tenir. Sans cette clé la décision ne serait pas servie, le rejeu
    /// s'arrêterait un tour plus tôt et rien ne le signalerait.
    ...
    fn create_tool_inspection_manager(
        ...
        // Add adversary inspector (LLM-based review, enabled by ~/.config/kaji/adversary.md)
    ...
    async fn prepare_reply_context(
        ...
        let tool_call_cut_off = match Config::global().get_param::<usize>("KAJI_TOOL_CALL_CUTOFF") {
            Ok(v) => v,
            Err(_) => {
                let context_limit = match self.provider().await {
                    ...
                    Err(_) => kaji_providers::model::DEFAULT_CONTEXT_LIMIT,
                };
    ...
    pub(super) fn create_state_machine(
        ...
        model_config: kaji_providers::model::ModelConfig,
        context_limit: usize,
        ...
        let max_turns = max_turns.unwrap_or_else(|| {
            Config::global()
                .get_param::<u32>("KAJI_MAX_TURNS")
                .unwrap_or(DEFAULT_MAX_TURNS)
        });
        let retry_timeout = Config::global()
            .get_param::<u64>("KAJI_RECIPE_RETRY_TIMEOUT_SECONDS")
            .unwrap_or(DEFAULT_RETRY_TIMEOUT_SECONDS);
        let on_failure_timeout = Config::global()
            .get_param::<u64>("KAJI_RECIPE_ON_FAILURE_TIMEOUT_SECONDS")
        ...
        let tool_call_cutoff = Config::global()
            .get_param::<usize>("KAJI_TOOL_CALL_CUTOFF")
            .unwrap_or_else(|_| {
                crate::context_mgmt::compute_tool_call_cutoff(context_limit, compaction_threshold)
            });
    ...
    pub async fn reply(
        ...
        *self.current_turn_recorder.lock().unwrap() = journal_seq.map(|turn_seq| {
        ...
        // Un hook de fin de tour a besoin de savoir où il tourne, comme tous
        // les autres. La lecture n'est payée que s'il en existe un.
        let turn_end_working_dir = if turn_end_hooks.has_hooks(crate::hooks::HookEvent::TurnEnd) {
            self.config
                .session_manager
                .get_session(&session_id, false)
                .await
            ...
            loop {
                match events.next().await {
                    ...
                    Some(err @ Err(_)) => {
                        ...
                        // Le hook de fin de tour accompagne la borne, pas le
                        // succès : un tour en erreur est une fin de tour, et un
                        // hook qui compte les tours ou libère une ressource
                        // raterait sinon exactement les cas qui comptent.
                        Self::emit_turn_end_hook(
                            &turn_end_hooks,
                            &session_id,
                            &turn_end_working_dir,
                        )
                        .await;
                        err?;
                    }
        ...
        let inner = Box::pin(async_stream::try_stream! {
            ...
            loop {
                ...
                if let Some(output) = final_output {
                    ...
                    match self
                        .emit_stop_hook_blocking(&session_config.id, &last_assistant_text, &session.working_dir.to_string_lossy(), consecutive_stop_hook_blocks)
                        .await
                    {
    ...
    pub async fn update_kaji_mode(&self, mode: KajiMode, session_id: &str) -> Result<()> {
        if let Some(provider) = self.provider.lock().await.as_ref() {
            provider
        ...
        *self.current_kaji_mode.lock().await = mode;
        self.config
            .session_manager
            .clone()
            .update(session_id)
            ...
            .apply()
            .await
            .context("Failed to persist kaji_mode to session")
    }

    pub async fn kaji_mode(&self) -> KajiMode {
        *self.current_kaji_mode.lock().await
    }
    ...
    pub async fn recreate_provider_for_session(
        ...
        let mode = self.kaji_mode().await;
        self.update_kaji_mode(mode, session_id).await
    }
    ...
    pub async fn restore_provider_from_session(&self, session: &Session) -> Result<bool> {
        ...
        let provider_name = session
            .provider_name
            .clone()
            .or_else(|| config.get_kaji_provider().ok())
        ...
        let (provider, active_model_config, provider_changed) =
                ...
                .is_ok()
            {
                ...
                let fallback_model_name = config.get_kaji_model().ok().ok_or_else(|| {
        ...
        if let Some(provider) = self.provider.lock().await.as_ref() {
            provider
                .update_mode(&session.id, session.kaji_mode)
                .await
                .map_err(|e| anyhow!("Failed to propagate mode to provider: {}", e))?;
        }
        *self.current_kaji_mode.lock().await = session.kaji_mode;
        Ok(provider_changed)
    }
    ...
    pub async fn create_recipe(
        ...
        let provider_name: String = config
            .get_kaji_provider()
            .expect("No provider configured. Run 'kaji configure' first");

        let settings = Settings {
            kaji_provider: Some(provider_name.clone()),
            kaji_model: Some(model_name.clone()),
...
mod tests {
    ...
    async fn tool_dispatch_records_gen_ai_span_attributes() {
        use kaji_test_support::otel::clear_otel_env;
        use rmcp::object;
    ...
    fn resolve_use_login_shell_path_defaults_by_platform() {
        assert!(resolve_use_login_shell_path(
            None,
            &KajiPlatform::KajiDesktop
        ));
        assert!(!resolve_use_login_shell_path(None, &KajiPlatform::KajiCli));
    }
    ...
    impl crate::providers::base::Provider for ActionRequiredProvider {
        ...
        async fn stream(
            &self,
            _: &kaji_providers::model::ModelConfig,
            _: &str,
...
"#,
    ...
    async fn create_test_agent(
        ...
        create_test_agent_of_type(data_dir, hook_manager, provider, SessionType::Hidden).await
    }

    /// `session_start` ne tire que dans une session qu'un utilisateur ouvre
    /// lui-même : un test de ce hook doit donc en créer une (voir
    /// [`opens_a_user_session`]).
    async fn create_test_agent_of_type(
        data_dir: PathBuf,
        hook_manager: crate::hooks::HookManager,
        provider: Arc<dyn crate::providers::base::Provider>,
    ...
    async fn session_start_hook_emits_once_for_first_reply_turn() -> Result<()> {
        ...
        let provider = Arc::new(CountingTextProvider::new());
        let (agent, session_id) = create_test_agent_of_type(
            env.data_dir(),
            env.hook_manager(),
            provider.clone(),
            SessionType::User,
                ...
                matches!(
                    ...
                    MessageContent::SystemNotification(notification)
                        if notification.msg.contains("more than 2 consecutive times")
                            && notification.msg.contains("KAJI_STOP_HOOK_BLOCK_CAP")
                )
            })
        }));
    ...
    async fn test_add_final_output_tool() -> Result<()> {
        ...
        let system_prompt = prompt_manager
            .builder()
            .with_kaji_mode(KajiMode::default())
            .build();
