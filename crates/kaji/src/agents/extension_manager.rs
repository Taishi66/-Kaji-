...
fn resolve_timeout(timeout: Option<u64>) -> u64 {
    timeout.unwrap_or_else(|| {
        Config::global()
            .get_kaji_default_extension_timeout()
...
pub struct KajiMcpAppToolAttachment {
...
pub(crate) const TRUSTED_TOOL_UPDATE_META_KEY: &str = "__kaji_tool_update_meta";

/// Manages kaji extensions / MCP clients and their interactions
...
const TOOL_EXTENSION_META_KEY: &str = "kaji_extension";
...
fn remove_untrusted_mcp_app_meta(result: &mut CallToolResult) {
    ...
    let remove_kaji = meta
        .0
        .get_mut("kaji")
        .and_then(Value::as_object_mut)
        .map(|kaji_meta| {
            kaji_meta.remove("mcpApp");
            kaji_meta.is_empty()
        })
        .unwrap_or(false);
...
fn insert_trusted_tool_update_meta(
    ...
    attachment: &KajiMcpAppToolAttachment,
) {
...
const KAJI_USER_AGENT: reqwest::header::HeaderValue =
    reqwest::header::HeaderValue::from_static(concat!("kaji/", env!("CARGO_PKG_VERSION")));
...
async fn connect_with_auth(
    ...
    auth_headers.insert(reqwest::header::USER_AGENT, KAJI_USER_AGENT);
...
async fn create_streamable_http_client(
    ...
    default_headers.insert(reqwest::header::USER_AGENT, KAJI_USER_AGENT);
...
async fn create_unix_socket_http_client(
    ...
    custom_headers.insert(
        ...
        KAJI_USER_AGENT
            ...
            .unwrap_or("kaji")
            .parse()
            .unwrap_or_else(|_| HeaderValue::from_static("kaji")),
    );
...
impl ExtensionManager {
    fn mcp_client_capabilities(&self) -> KajiMcpClientCapabilities {
    ...
    pub async fn add_extension(
        ...
        let effective_working_dir = working_dir
            .clone()
            .or_else(|| std::env::var("KAJI_WORKING_DIR").ok().map(PathBuf::from))
            ...
            ExtensionConfig::StreamableHttp {
                ...
                create_streamable_http_client(
                    ...
                    *timeout,
                    ...
                    Box::new(KajiCredentialStore::new(name.to_string())),
                    self.provider.clone(),
    ...
    ) -> Option<KajiMcpAppToolAttachment> {
        let resource_uri = resolved_tool.resource_uri.clone()?;

        let mut attachment = KajiMcpAppToolAttachment {
...
mod tests {
    ...
    fn test_insert_trusted_tool_update_meta_stores_backend_payload() {
        let mut result = CallToolResult::success(vec![]);
        let attachment = KajiMcpAppToolAttachment {
    ...
    async fn test_invalid_header_name_returns_config_error() {
        ...
        let capabilities = KajiMcpClientCapabilities {
            mcpui: false,
            host_info: None,
        };
