...
/// MCP client implementation for Kaji
#[expect(deprecated)]
...
fn resolve_sampling_model_config() -> anyhow::Result<kaji_providers::model::ModelConfig> {
    let config = crate::config::Config::global();
    let provider_name = config.get_kaji_provider()?;
    let model_name = config.get_kaji_model()?;
...
pub struct KajiClient {
    notification_handlers: Arc<Mutex<Vec<Sender<ServerNotification>>>>,
    provider: SharedProvider,
    session_id: Mutex<Option<String>>,
    active_tool_calls: Arc<StdMutex<HashMap<String, Vec<String>>>>,
    client_name: String,
    capabilities: KajiMcpClientCapabilities,
...
impl KajiClient {
    ...
    fn resolved_client_info(&self) -> Implementation {
        ...
        let version = self
            ...
            .unwrap_or_else(|| {
                std::env::var("KAJI_MCP_CLIENT_VERSION")
...
impl ClientHandler for KajiClient {
    #[expect(deprecated)]
    async fn list_roots(
        &self,
    ...
    async fn create_message(
        ...
        let system_prompt = params
            .system_prompt
            .as_deref()
            .unwrap_or("You are a general-purpose AI agent called kaji");
...
#[derive(Debug, Clone)]
pub struct KajiMcpClientCapabilities {
    pub mcpui: bool,
    pub host_info: Option<KajiMcpHostInfo>,
}

/// The MCP client is the interface for MCP operations.
pub struct McpClient {
    client: Mutex<RunningService<RoleClient, KajiClient>>,
...
impl McpClient {
        ...
        E: std::error::Error + From<std::io::Error> + Send + Sync + 'static,
    {
        ...
        let client: rmcp::service::RunningService<rmcp::RoleClient, KajiClient> =
...
mod tests {
    ...
    fn new_client(platform: KajiPlatform) -> KajiClient {
        let capabilities = match platform {
            KajiPlatform::KajiDesktop => KajiMcpClientCapabilities {
                mcpui: true,
                host_info: None,
            },
            KajiPlatform::KajiCli => KajiMcpClientCapabilities {
                mcpui: false,
                host_info: None,
            },
        };
    ...
    async fn tool_list_changed_during_fetch_prevents_stale_cache() {
        ...
        let kaji_client = KajiClient::new(
            Arc::new(Mutex::new(Vec::new())),
            Arc::new(Mutex::new(None)),
            "kaji-test".to_string(),
            KajiMcpClientCapabilities {
                mcpui: false,
                host_info: None,
            },
            temp_dir.path().to_path_buf(),
        ...
        kaji_client.handle_tool_list_changed().await;
    ...
    fn test_resolve_session_id(
        ...
        runtime.block_on(async {
            let client = new_client(KajiPlatform::KajiCli);
    ...
    fn test_request_injects_session(request_builder: fn(Extensions) -> ClientRequest) {
        ...
        extensions.insert(
            serde_json::from_value::<MetaObject>(json!({
                "Kaji-Session-Id": "old-session-id",
    ...
    fn test_client_info_advertises_mcp_apps_ui_extension() {
        let client = new_client(KajiPlatform::KajiDesktop);
    ...
    fn test_explicit_host_info_passes_through_client_identity() {
        let client = KajiClient::new(
            Arc::new(Mutex::new(Vec::new())),
            Arc::new(Mutex::new(None)),
            KajiPlatform::KajiDesktop.to_string(),
            KajiMcpClientCapabilities {
                mcpui: true,
                host_info: Some(KajiMcpHostInfo {
                    explicit_extensions: true,
                    extensions: ExtensionCapabilities::new(),
                    client_name: Some("kaji2".to_string()),
        ...
        let info = ClientHandler::get_info(&client);
        assert_eq!(info.client_info.name, "kaji2");
    ...
    fn test_explicit_host_extensions_override_platform_fallback() {
        let client = KajiClient::new(
            Arc::new(Mutex::new(Vec::new())),
            Arc::new(Mutex::new(None)),
            KajiPlatform::KajiCli.to_string(),
            KajiMcpClientCapabilities {
                mcpui: false,
                host_info: Some(KajiMcpHostInfo {
                    explicit_extensions: true,
