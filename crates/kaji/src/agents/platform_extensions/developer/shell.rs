...
/// respecting `KAJI_SHELL`.
///
/// Defaults to `cmd` when `KAJI_SHELL` is unset. The invocation flags are
...
fn windows_shell() -> String {
    std::env::var("KAJI_SHELL").unwrap_or_else(|_| "cmd".to_string())
}
...
fn unix_shell() -> String {
    if let Ok(shell) = std::env::var("KAJI_SHELL") {
        return shell;
    }
...
///
/// When kajid is launched from a desktop app (e.g. Electron), it may inherit
...
pub(crate) fn resolve_login_shell_path() -> Option<String> {
    ...
    // (TIOCSPGRP) cannot steal the terminal foreground from kaji, which
    // would cause kaji to receive SIGTTIN and be suspended on startup.
...
/// name, so setting `KAJI_SHELL` to a bare executable is enough — users never
