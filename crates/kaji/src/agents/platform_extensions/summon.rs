...
pub fn discover_filesystem_sources(working_dir: &Path) -> Vec<SourceEntry> {
    ...
    let local_recipe_dirs: Vec<PathBuf> = vec![
        working_dir.join(".kaji/recipes"),
        working_dir.join(".agents/recipes"),
    ];

    let global_recipe_dirs: Vec<PathBuf> = std::env::var("KAJI_RECIPE_PATH")
        .ok()
        .into_iter()
        .flat_map(|p| {
        ...
        .chain(
            [
                home.as_ref().map(|h| h.join(".kaji/recipes")),
...
fn max_background_tasks() -> usize {
    Config::global()
        .get_param::<usize>("KAJI_MAX_BACKGROUND_TASKS")
        .unwrap_or(5)
}

fn completed_task_ttl() -> Duration {
    let secs = Config::global()
        .get_param::<u64>("KAJI_COMPLETED_TASK_TTL_SECS")
        .unwrap_or(600);
...
impl SummonClient {
        ...
        let schema = serde_json::json!({
            ...
            "properties": {
                ...
                "max_turns": {
                    "type": "integer",
                    "minimum": 1,
                    "description": "Maximum turns for this delegate. Overrides recipe settings.max_turns and KAJI_SUBAGENT_MAX_TURNS."
                },
                "context": {
                    "type": "string",
    ...
    async fn handle_delegate(
        ...
        let agent_config = AgentConfig::new(
            ...
            crate::agents::KajiPlatform::KajiCli,
        )
    ...
    fn build_recipe_from_agent(
        ...
        let settings = model.map(|m| Settings {
            kaji_model: Some(m),
            kaji_provider: params.provider.clone(),
    ...
    fn resolve_model_config(
        ...
        session: &crate::session::Session,
        provider_name: &str,
    ) -> Result<kaji_providers::model::ModelConfig, anyhow::Error> {
        ...
        let override_model = params
            .model
            .clone()
            .or_else(|| recipe.settings.as_ref().and_then(|s| s.kaji_model.clone()))
            .or_else(|| {
                Config::global()
                    .get_param::<String>("KAJI_SUBAGENT_MODEL")
                    .ok()
            });
    ...
    async fn resolve_provider(
        ...
        (
            ...
            kaji_providers::model::ModelConfig,
        ),
        anyhow::Error,
    > {
        let provider_name = params
            ...
            .or_else(|| {
                recipe
                    .settings
                    .as_ref()
                    .and_then(|s| s.kaji_provider.clone())
            })
            .or_else(|| {
                Config::global()
                    .get_param::<String>("KAJI_SUBAGENT_PROVIDER")
                    .ok()
            })
    ...
    fn resolve_max_turns(&self, session: &crate::session::Session) -> usize {
        session
            ...
            .or_else(|| {
                std::env::var("KAJI_SUBAGENT_MAX_TURNS")
                    .ok()
                    .and_then(|v| v.parse().ok())
            })
            .or_else(|| {
                Config::global()
                    .get_param::<usize>("KAJI_SUBAGENT_MAX_TURNS")
                    .ok()
            })
...
You review code."#;
    ...
    async fn test_discover_recipes_and_agents() {
        ...
        let recipes = temp_dir.path().join(".kaji/recipes");
        ...
        let agents = temp_dir.path().join(".kaji/agents");
    ...
    async fn test_recipe_deduplication_local_wins() {
        ...
        let local = temp_dir.path().join(".kaji/recipes");
    ...
    fn test_resolve_max_turns_recipe_overrides_env_var() {
        ...
        std::env::set_var("KAJI_SUBAGENT_MAX_TURNS", "99");
        let result = client.resolve_max_turns(&session);
        std::env::remove_var("KAJI_SUBAGENT_MAX_TURNS");
    ...
    fn test_resolve_max_turns_falls_back_to_env_var() {
        ...
        std::env::set_var("KAJI_SUBAGENT_MAX_TURNS", "7");
        let result = client.resolve_max_turns(&session);
        std::env::remove_var("KAJI_SUBAGENT_MAX_TURNS");

        assert_eq!(
            result, 7,
            "should fall back to KAJI_SUBAGENT_MAX_TURNS env var"
        );
    }
    ...
    async fn test_resolve_provider_reuses_unregistered_parent_provider() {
        ...
        *extension_manager.get_provider().lock().await = Some(Arc::clone(&parent_provider));
        ...
        let session = crate::session::Session {
            ...
            model_config: Some(kaji_providers::model::ModelConfig::new("test-model")),
            ..Default::default()
        };
    ...
    fn session_with(parent: kaji_providers::model::ModelConfig) -> crate::session::Session {
    ...
    fn resolve_with_override(
        model: Option<&str>,
        parent: kaji_providers::model::ModelConfig,
    ) -> kaji_providers::model::ModelConfig {
        let client = SummonClient::new(create_test_context()).unwrap();
        let params = DelegateParams {
    ...
    fn parent_config() -> kaji_providers::model::ModelConfig {
        kaji_providers::model::ModelConfig::new(PARENT_MODEL).with_canonical_limits(PROVIDER)
    }
    ...
    async fn test_resolve_model_config_applies_canonical_limits_to_overridden_model() {
        let _env = env_lock::lock_env([
            ...
            ("KAJI_SUBAGENT_MODEL", None::<&str>),
        ]);

        let parent = parent_config();
        let overridden =
            kaji_providers::model::ModelConfig::new(OVERRIDE_MODEL).with_canonical_limits(PROVIDER);
