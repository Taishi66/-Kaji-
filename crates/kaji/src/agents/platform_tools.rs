...
pub fn manage_schedule_tool() -> Tool {
    Tool::new(
        ...
        indoc! {r#"
            Manage kaji's internal scheduled recipe execution.

            Actions:
            - "list": List all kaji scheduled jobs
            - "create": Create a new kaji scheduled job from a recipe file
            - "run_now": Execute a kaji scheduled job immediately
            - "pause": Pause a kaji scheduled job
            - "unpause": Resume a paused kaji scheduled job
            - "delete": Remove a kaji scheduled job
            - "kill": Terminate a currently running kaji scheduled job
            - "inspect": Get details about a running kaji scheduled job
            - "sessions": List execution history for a kaji scheduled job
