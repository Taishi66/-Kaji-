                    ...
                    if let Ok(ref mut tool_call) = coerced_req.tool_call {
                        if let Some(tool) = tools.iter().find(|t| t.name == tool_call.name) {
                            ...
                            if let Some(ref meta) = tool.meta {
                                ...
                                // kaji.external_dispatch) survive coercion.
...
mod tests {
    ...
    use kaji_providers::conversation::token_usage::{ProviderStats, ProviderUsage, Usage};
    use kaji_providers::model::ModelConfig;
    ...
    async fn categorize_tool_requests_skips_externally_dispatched_and_preserves_marker() {
        // External requests must (1) survive coercion with kaji.external_dispatch
        ...
        assert!(
            ...
            "kaji.external_dispatch marker was clobbered by coercion; merged tool_meta = {:?}",
            tool_req.tool_meta
        );
        let merged = tool_req
            .tool_meta
