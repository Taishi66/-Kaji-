...
const KAJI_RECIPE_RETRY_TIMEOUT_SECONDS: &str = "KAJI_RECIPE_RETRY_TIMEOUT_SECONDS";

/// Environment variable for configuring on_failure timeout globally
const KAJI_RECIPE_ON_FAILURE_TIMEOUT_SECONDS: &str = "KAJI_RECIPE_ON_FAILURE_TIMEOUT_SECONDS";
...
fn get_retry_timeout(retry_config: &RetryConfig) -> Duration {
    let timeout_seconds = retry_config
        ...
        .or_else(|| {
            ...
            config.get_param(KAJI_RECIPE_RETRY_TIMEOUT_SECONDS).ok()
        })
...
fn get_on_failure_timeout(retry_config: &RetryConfig) -> Duration {
    let timeout_seconds = retry_config
        ...
        .or_else(|| {
            let config = Config::global();
            config
                .get_param(KAJI_RECIPE_ON_FAILURE_TIMEOUT_SECONDS)
                .ok()
        })
