...
impl Provider for ResolvedModelProvider {
    ...
    ) -> Result<crate::providers::base::MessageStream, kaji_providers::errors::ProviderError> {
        self.inner
    ...
    async fn get_context_limit(
        &self,
        model_config: &ModelConfig,
    ) -> Result<usize, kaji_providers::errors::ProviderError> {
    ...
    async fn fetch_model_info(
        &self,
        model_name: &str,
    ) -> Result<kaji_providers::base::ModelInfo, kaji_providers::errors::ProviderError> {
...
impl TestPipeline {
    ...
    pub(super) async fn with_kaji_mode(self, mode: KajiMode) -> Self {
        *self.kaji_mode.lock().await = mode;
...
async fn test_pipeline_with_components(
    ...
    let session = session_manager
        ...
        .await?;
    let model_config = ModelConfig::new(kaji_providers::openai::OPEN_AI_DEFAULT_MODEL)
...
async fn build_test_pipeline(
    ...
    let api_client = kaji_providers::api_client::ApiClient::new_with_tls(
        api.uri(),
        kaji_providers::api_client::AuthMethod::NoAuth,
        None,
    )?;
    let provider: Arc<dyn Provider> = Arc::new(
        kaji_providers::openai::OpenAiProviderBuilder::new(api_client)
            .name(provider_name)
    ...
    let pipeline = TestPipeline {
        ...
        model_config,
        extension_manager,
        kaji_mode: TokioMutex::new(session.kaji_mode),
