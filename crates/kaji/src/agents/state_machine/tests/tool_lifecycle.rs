...
async fn recover_from_missing_tool() -> Result<()> {
    ...
    let (pipeline, api) = test_pipeline().await?;
    let model_config =
        kaji_providers::model::ModelConfig::new(kaji_providers::openai::OPEN_AI_DEFAULT_MODEL)
...
async fn approvals_and_per_tool_permissions() -> Result<()> {
    ...
    let pipeline = pipeline.with_kaji_mode(KajiMode::Auto).await;
...
async fn tool_availability_tracks_mode_and_extension_removal() -> Result<()> {
    ...
    let pipeline = pipeline.with_kaji_mode(KajiMode::Chat).await;
