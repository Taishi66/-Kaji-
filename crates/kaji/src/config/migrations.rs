...
fn cleanup_legacy_provider_keys(config: &mut Mapping) -> bool {
    ...
    let stale_keys: Vec<serde_yaml::Value> = config
        ...
        .filter(|k| {
            k.as_str()
                .map(|s| {
                    s == "KAJI_PROVIDER" || s == "KAJI_MODEL" || s.ends_with(configured_suffix)
                })
                .unwrap_or(false)
        })
        .cloned()
...
fn migrate_provider_config(config: &mut Mapping) -> bool {
    ...
    if config.contains_key(&providers_key) {
        ...
        if !config.contains_key(&ap_key) {
            if let Some(legacy) = config
                .get(serde_yaml::Value::String("KAJI_PROVIDER".to_string()))
                .and_then(|v| v.as_str())
            {
    ...
    let active_model = config
        .get(serde_yaml::Value::String("KAJI_MODEL".to_string()))
        .and_then(|v| v.as_str())
        .map(|s| s.to_string())
        .unwrap_or_default();
    ...
    config.shift_remove(serde_yaml::Value::String("KAJI_PROVIDER".to_string()));
    config.shift_remove(serde_yaml::Value::String("KAJI_MODEL".to_string()));
    for name in &discovered_providers {
...
mod tests {
    ...
    fn test_migrate_provider_config_basic() {
        let mut config = Mapping::new();
        config.insert(
            serde_yaml::Value::String("KAJI_PROVIDER".to_string()),
            serde_yaml::Value::String("claude-acp".to_string()),
        );
        config.insert(
            serde_yaml::Value::String("KAJI_MODEL".to_string()),
        ...
        assert!(!config.contains_key(serde_yaml::Value::String("KAJI_PROVIDER".to_string())));
        assert!(!config.contains_key(serde_yaml::Value::String("KAJI_MODEL".to_string())));
    ...
    fn test_cleanup_legacy_keys_when_providers_exists() {
        ...
        // active_provider should be backfilled from legacy KAJI_PROVIDER
        assert_eq!(
            config
                .get(serde_yaml::Value::String("active_provider".to_string()))
                .and_then(|v| v.as_str()),
