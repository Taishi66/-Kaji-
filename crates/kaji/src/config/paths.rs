...
impl Paths {
    fn get_dir(dir_type: DirType) -> PathBuf {
        if let Some(base) = Self::path_root() {
            ...
            // user config/data directories (e.g. ~/Library/Application Support/Block/kaji/).
            // Changing this would orphan existing installations.
            let strategy = choose_app_strategy(AppStrategyArgs {
                top_level_domain: "Block".to_string(),
                author: "Block".to_string(),
                app_name: "kaji".to_string(),
            })
            .expect("kaji requires a home dir");
    ...
    pub(crate) fn path_root() -> Option<PathBuf> {
        Self::validated_path_root(std::env::var_os("KAJI_PATH_ROOT"))
    }
