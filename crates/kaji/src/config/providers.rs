...
pub fn get_active_model(config: &Config) -> Option<String> {
    ...
    config.get_param::<String>("KAJI_MODEL").ok()
}
...
pub fn clear_active_provider(config: &Config) -> Result<(), ConfigError> {
    for key in [ACTIVE_PROVIDER_KEY, "KAJI_PROVIDER", "KAJI_MODEL"] {
...
mod tests {
    ...
    fn test_clear_active_provider_removes_legacy_keys() {
        let config = new_test_config();
        config.set_param("KAJI_PROVIDER", "anthropic").unwrap();
        config.set_param("KAJI_MODEL", "claude").unwrap();
