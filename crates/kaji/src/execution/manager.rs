use crate::agents::mcp_client::KajiMcpHostInfo;
use crate::agents::{Agent, AgentConfig, ExtensionLoadResult, KajiPlatform};
...
impl AgentManager {
        ...
        AGENT_MANAGER
            .get_or_try_init(|| async {
                ...
                let max_sessions = config
                    .get_kaji_max_active_agents()
                    .unwrap_or(DEFAULT_MAX_SESSION);
                let default_mode = config.get_kaji_mode().unwrap_or_default();
    ...
    async fn create_agent_locked(
        ...
        let mut mode = self.agent_config.kaji_mode;
        if let Ok(session) = self
            ...
            .await
        {
            mode = session.kaji_mode;
            info!(kaji_mode = %mode, session_id = %session_id, "Session loaded");
        }

        let mut config = self.agent_config.clone();
        config.kaji_mode = mode;
        ...
        if agent.provider().await.is_err() {
            if let Some(provider) = &*self.default_provider.read().await {
                ...
                let model_config = config
                    ...
                    .unwrap_or_else(|| kaji_providers::model::ModelConfig::new("unknown"));
                agent
...
mod tests {
    ...
    #[test_case(KajiMode::SmartApprove ; "smart_approve")]
    #[tokio::test]
    async fn test_agent_inherits_session_mode(mode: KajiMode) {
        let temp_dir = TempDir::new().unwrap();
        let manager = create_test_manager(&temp_dir).await;
        ...
        assert_eq!(agent.kaji_mode().await, mode);
    }
    ...
    async fn test_session_mode_isolation() {
        ...
        assert_eq!(a1.kaji_mode().await, KajiMode::Approve);
        assert_eq!(a2.kaji_mode().await, KajiMode::Auto);
    }
}
