...
impl TelegramGateway {
    ...
    ///
    /// Files are stored under `<tmp>/kaji_voice/voice_<uuid>.<ext>` so Kaji
    ...
    fn save_voice_file(
        ...
        let dir = std::env::temp_dir().join("kaji_voice");
    ...
    /// Build the text prompt that tells Kaji about a voice message file.
    fn voice_prompt(
        path: &std::path::Path,
        duration: Option<i32>,
        mime_type: Option<&str>,
                    ...
                    match result {
                        Ok(updates) => {
                            for update in updates {
                                ...
                                // Kaji to transcribe the file using CLI tools.
...
mod tests {
    ...
    fn voice_prompt_includes_path_and_duration() {
        let path = std::path::PathBuf::from("/tmp/kaji_voice/voice_test.ogg");
        let prompt = TelegramGateway::voice_prompt(&path, Some(10), Some("audio/ogg"));
        assert!(prompt.contains("/tmp/kaji_voice/voice_test.ogg"));
    ...
    fn voice_prompt_with_mp3_mime() {
        let path = std::path::PathBuf::from("/tmp/kaji_voice/voice_test.mp3");
    ...
    fn save_voice_file_contains_untrusted_mime_before_pairing() {
        ...
        let expected_dir = std::env::temp_dir().join("kaji_voice");
