...
use super::app::KajiApp;

static CLOCK_HTML: &str = include_str!("../kaji_apps/clock.html");
...
pub fn mark_deletable_apps(apps: &mut [KajiApp]) {
...
impl McpAppCache {
    ...
    fn ensure_default_apps(&self) {
        for (uri, html) in DEFAULT_APPS {
            if self.get_app(APPS_EXTENSION_NAME, uri).is_none() {
                if let Ok(mut app) = KajiApp::from_html(html) {
    ...
    pub fn list_apps(&self) -> Result<Vec<KajiApp>, std::io::Error> {
        ...
        for entry in fs::read_dir(&self.cache_dir)? {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("json") {
                match fs::read_to_string(&path) {
                    Ok(content) => match serde_json::from_str::<KajiApp>(&content) {
    ...
    pub fn store_app(&self, app: &KajiApp) -> Result<(), std::io::Error> {
    ...
    pub fn get_app(&self, extension_name: &str, resource_uri: &str) -> Option<KajiApp> {
        ...
        fs::read_to_string(&app_path)
            .ok()
            .and_then(|content| serde_json::from_str::<KajiApp>(&content).ok())
    }
        ...
        for entry in fs::read_dir(&self.cache_dir)? {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("json") {
                if let Ok(content) = fs::read_to_string(&path) {
                    if let Ok(app) = serde_json::from_str::<KajiApp>(&content) {
...
<body></body>
    ...
    fn mark_deletable_apps_protects_bundled_uri_not_name() {
        let mut bundled_clock = KajiApp::from_html(CLOCK_HTML).unwrap();
        bundled_clock.mcp_servers = vec![APPS_EXTENSION_NAME.to_string()];

        let mut user_clock = KajiApp::from_html(CUSTOM_APP_HTML).unwrap();
        ...
        let mut custom = KajiApp::from_html(CUSTOM_APP_HTML).unwrap();
        custom.mcp_servers = vec![APPS_EXTENSION_NAME.to_string()];

        let mut external = KajiApp::from_html(CUSTOM_APP_HTML).unwrap();
    ...
    fn delete_app_removes_cached_entry() {
        with_temp_config(|| {
            ...
            let mut app = KajiApp::from_html(CUSTOM_APP_HTML).unwrap();
            app.mcp_servers = vec![APPS_EXTENSION_NAME.to_string()];
