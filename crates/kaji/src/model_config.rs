...
pub fn model_config_from_user_config_with_session_settings(
    ...
    let model = materialize_model_config_inner(model, provider_name, false)?
        ...
        .with_default_thinking_effort(config.get_kaji_thinking_effort());
...
fn apply_canonical_limits(provider_name: &str, model: ModelConfig) -> ModelConfig {
    if provider_name == kaji_providers::azure_foundry::AZURE_FOUNDRY_PROVIDER_NAME {
        model
    } else {
...
fn materialize_model_config_inner(
    ...
    if model.temperature.is_none() {
        model = model.with_temperature(get_kaji_temperature(config)?);
    }

    if model.toolshim && model.toolshim_model.is_none() {
        model = model.with_toolshim_model(get_kaji_toolshim_model(config)?);
    }

    model = model
        .with_default_context_limit(config.get_kaji_context_limit()?)
        .with_default_max_tokens(config.get_kaji_max_tokens()?);

    if include_default_thinking_effort {
        model = model.with_default_thinking_effort(config.get_kaji_thinking_effort());
    }

    if provider_name == kaji_providers::openai::OPEN_AI_PROVIDER_NAME {
...
fn configured_fast_model_name() -> Option<String> {
    Config::global()
        .get_param::<String>("KAJI_FAST_MODEL")
        .ok()
...
///   1. `KAJI_FAST_MODEL` (user override)
...
fn base_model_config_from_user_config(
    ...
    let mut model = ModelConfig {
        ...
        context_limit: None,
        temperature: get_kaji_temperature(config)?,
        max_tokens: None,
        toolshim: get_kaji_toolshim(config)?.unwrap_or(false),
        toolshim_model: get_kaji_toolshim_model(config)?,
        request_params: None,
        reasoning: None,
...
fn get_kaji_temperature(config: &Config) -> Result<Option<f32>> {
    match config.get_param::<f32>("KAJI_TEMPERATURE") {
        Ok(temp) if temp < 0.0 => Err(anyhow!(
            "Value for 'KAJI_TEMPERATURE' is out of valid range: {temp}"
        )),
...
fn get_kaji_toolshim(config: &Config) -> Result<Option<bool>> {
    match config.get_param::<serde_yaml::Value>("KAJI_TOOLSHIM") {
        Ok(value) => parse_yaml_bool_config("KAJI_TOOLSHIM", value).map(Some),
        Err(ConfigError::NotFound(_)) => Ok(None),
        Err(e) => Err(e.into()),
    }
}
...
pub fn global_toolshim() -> bool {
    get_kaji_toolshim(Config::global())
        .ok()
        .flatten()
        .unwrap_or(false)
}

fn get_kaji_toolshim_model(config: &Config) -> Result<Option<String>> {
    match config.get_param::<String>("KAJI_TOOLSHIM_OLLAMA_MODEL") {
        Ok(value) if value.trim().is_empty() => Err(anyhow!(
            "Invalid value for 'KAJI_TOOLSHIM_OLLAMA_MODEL': '{value}' - cannot be empty if set"
        )),
