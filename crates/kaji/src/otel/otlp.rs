...
/// in an environment where kaji was built without the `grpc-tonic`
...
fn warn_grpc_protocol_skipped_once() {
    GRPC_PROTOCOL_WARNING_EMITTED.call_once(|| {
        eprintln!(
            "kaji otel: OTEL_EXPORTER_OTLP_PROTOCOL is set to a gRPC \
             variant, but this kaji build only includes the HTTP \
...
///
/// kaji's `opentelemetry-otlp` build only enables the `http-proto` /
...
fn signal_protocol_is_http(signal: &str) -> bool {
    ...
    match raw.trim().to_lowercase().as_str() {
        ...
        // gRPC variants require the `grpc-tonic` feature, which kaji
        // does not enable.
        _ => false,
    }
}
...
/// Promotes kaji config-file OTel settings to env vars before exporter build.
...
fn create_resource() -> Resource {
    ...
    let mut builder = Resource::builder_empty()
        .with_attributes([
            KeyValue::new("service.name", "kaji"),
            KeyValue::new("service.version", env!("CARGO_PKG_VERSION")),
            KeyValue::new("service.namespace", "kaji"),
...
fn create_otlp_tracing_layer() -> OtlpResult<OtlpTracingLayer> {
    ...
    let tracer_provider = match exporter {
        ExporterType::Otlp => {
            if !signal_protocol_is_http("traces") {
                ...
                return Err("OTLP traces protocol is grpc but kaji was built without grpc-tonic; skipping traces exporter".into());
            }
    ...
    let tracer = tracer_provider.tracer("kaji");
    ...
    let meter_provider = match exporter {
        ExporterType::Otlp => {
            if !signal_protocol_is_http("metrics") {
                warn_grpc_protocol_skipped_once();
                return Err("OTLP metrics protocol is grpc but kaji was built without grpc-tonic; skipping metrics exporter".into());
            }
            let rt = get_or_create_otel_rt()?;
    ...
    let logger_provider = match exporter {
        ExporterType::Otlp => {
            if !signal_protocol_is_http("logs") {
                warn_grpc_protocol_skipped_once();
                return Err("OTLP logs protocol is grpc but kaji was built without grpc-tonic; skipping logs exporter".into());
            }
            let rt = get_or_create_otel_rt()?;
...
fn create_otlp_metrics_filter() -> FilterFn<impl Fn(&Metadata<'_>) -> bool> {
    FilterFn::new(|metadata: &Metadata<'_>| {
        ...
        if metadata.level() == &Level::DEBUG {
            let target = metadata.target();
            if target.starts_with("kaji::telemetry")
                || target.starts_with("kaji::metrics")
...
mod tests {
    ...
    fn test_create_resource_defaults() {
        ...
        assert_eq!(get("service.name").as_deref(), Some("kaji"));
        assert_eq!(
            get("service.version").as_deref(),
            Some(env!("CARGO_PKG_VERSION"))
        );
        assert_eq!(get("service.namespace").as_deref(), Some("kaji"));
    ...
    #[test_case(
        ...
        Resource::builder_empty()
            .with_attributes([KeyValue::new("service.name", "kaji"), KeyValue::new("service.version", env!("CARGO_PKG_VERSION")), KeyValue::new("service.namespace", "kaji"), KeyValue::new("host.name", session_host()), KeyValue::new("user.name", session_user())])
            .with_detector(Box::new(TelemetryResourceDetector))
            .build();
        "no env vars uses kaji defaults"
    )]
    #[test_case(
        &[("OTEL_SERVICE_NAME", "custom")],
        Resource::builder_empty()
    ...
    #[test_case(&[("RUST_LOG", "kaji=debug"), ("OTEL_LOG_LEVEL", "trace")], Level::TRACE; "directive RUST_LOG falls through to OTEL_LOG_LEVEL")]
    #[test_case(&[("RUST_LOG", "kaji=debug")], Level::INFO; "directive RUST_LOG falls through to default")]
