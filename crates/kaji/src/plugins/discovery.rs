...
fn user_settings_path() -> Option<PathBuf> {
    if let Some(path_root) = Paths::path_root() {
        return Some(path_root.join(".config").join("kaji").join("settings.json"));
    }
    Some(
        dirs::home_dir()?
            .join(".config")
...
fn project_settings_path(project_root: &Path, local: bool) -> PathBuf {
    ...
    project_root.join(".config").join("kaji").join(file)
}
...
mod tests {
    ...
    fn test_config(dir: &Path) -> Config {
        Config::new(dir.join("config.yaml"), "kaji-discovery-test").unwrap()
    }
    ...
    fn project_scope_overrides_user_scope() {
        ...
        write_settings(
            &fake_home.path().join(".config").join("kaji"),
            r#"{"disabledPlugins":["demo"]}"#,
        );
        ...
        let prev = std::env::var("KAJI_PATH_ROOT").ok();
        unsafe { std::env::set_var("KAJI_PATH_ROOT", fake_home.path()) };
        let found = discover(project);
        match prev {
            Some(v) => unsafe { std::env::set_var("KAJI_PATH_ROOT", v) },
            None => unsafe { std::env::remove_var("KAJI_PATH_ROOT") },
        }
