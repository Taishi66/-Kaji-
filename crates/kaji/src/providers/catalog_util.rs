pub use kaji_providers::canonical::catalog::{
...
pub async fn get_providers_by_format(format: ProviderFormat) -> Vec<ProviderCatalogEntry> {
    ...
    kaji_providers::canonical::catalog::get_providers_by_format(format, &native_provider_ids)
}
...
pub async fn get_setup_catalog_entries() -> Vec<ProviderSetupCatalogEntry> {
    ...
    kaji_providers::canonical::catalog::get_setup_catalog_entries(&registry_metadata)
}

pub fn get_provider_setup_category(provider_id: &str) -> Option<ProviderSetupCategory> {
    kaji_providers::canonical::catalog::get_provider_setup_category(provider_id)
}

pub fn get_provider_template(provider_id: &str) -> Option<ProviderTemplate> {
    kaji_providers::canonical::catalog::get_provider_template(provider_id)
}
...
mod tests {
    ...
    #[tokio::test]
    async fn setup_catalog_includes_kaji_and_curated_fields() {
        let entries = get_setup_catalog_entries().await;

        let kaji = entries
            .iter()
            .find(|entry| entry.provider_id == "kaji")
            .expect("setup catalog should include synthetic kaji");
        assert_eq!(kaji.category, ProviderSetupCategory::Agent);
        assert_eq!(kaji.setup_method, ProviderSetupMethod::None);
        assert!(kaji.fields.is_empty());
