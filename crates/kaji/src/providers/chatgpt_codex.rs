...
use kaji_providers::formats::openai_responses::responses_api_to_streaming_message;
...
<html>
  <head>
    <title>kaji - ChatGPT Authorization Successful</title>
    <style>
      body {
...
<html>
  <head>
    <title>kaji - ChatGPT Authorization Failed</title>
    <style>
      body {{
        font-family: system-ui, -apple-system, sans-serif;
        display: flex;
...
impl ChatGptCodexProvider {
    ...
    async fn post_streaming(&self, payload: &Value) -> Result<reqwest::Response, ProviderError> {
        ...
        let request = (self.request_builder)(request)
            .map_err(|e| ProviderError::ExecutionError(e.to_string()))?;
        let response = kaji_providers::http_status::send_bounded(
            request,
...
impl kaji_providers::base::ProviderDescriptor for ChatGptCodexProvider {
...
mod tests {
    ...
    fn inventory_configured_uses_oauth_token_cache() {
        ...
        let _guard = env_lock::lock_env([("KAJI_PATH_ROOT", Some(root_path.as_str()))]);
