...
pub struct ClaudeCodeProvider {
    ...
    #[serde(skip)]
    initial_mode: tokio::sync::Mutex<Option<KajiMode>>,
}
...
impl ClaudeCodeProvider {
    ...
    fn build_stream_json_command(&self) -> Command {
        ...
        // Allow kaji to run inside a Claude Code session.
    ...
    fn apply_permission_flags(cmd: &mut Command) -> Result<bool, ProviderError> {
        ...
        match kaji_mode {
            ...
            KajiMode::SmartApprove | KajiMode::Approve => {
                cmd.arg("--permission-prompt-tool").arg("stdio");
                Ok(true)
            }
            KajiMode::Chat => Ok(false),
        }
    }
...
impl kaji_providers::base::ProviderDescriptor for ClaudeCodeProvider {
...
impl Provider for ClaudeCodeProvider {
    ...
    async fn update_mode(&self, _session_id: &str, mode: KajiMode) -> Result<(), ProviderError> {
...
mod tests {
    ...
    use chrono::Utc;
    use kaji_test_support::session::TEST_SESSION_ID;
    use serde_json::json;
    use std::collections::HashMap;
    use std::fs;
