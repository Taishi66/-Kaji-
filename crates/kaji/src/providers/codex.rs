...
use crate::config::{Config, ExtensionConfig, KajiMode};
use crate::conversation::message::{Message, MessageContent};
use crate::subprocess::configure_subprocess;
use kaji_providers::errors::ProviderError;
use kaji_providers::model::ModelConfig;
use kaji_providers::request_log::{start_log, LoggerHandleExt};
use rmcp::model::Role;
use rmcp::model::Tool;
...
pub struct CodexProvider {
    ...
    #[serde(skip)]
    mode_by_session: tokio::sync::RwLock<HashMap<String, KajiMode>>,
}
...
impl CodexProvider {
    ...
    /// Apply permission flags based on KajiMode
    fn apply_permission_flags(cmd: &mut Command, kaji_mode: KajiMode) -> Result<(), ProviderError> {
        match kaji_mode {
            KajiMode::Auto => {
    ...
    async fn execute_command(
        ...
        if std::env::var("KAJI_CODEX_DEBUG").is_ok() {
        ...
        // This allows users to set KAJI_PROVIDER=codex without needing to specify a model
        ...
        Self::apply_permission_flags(&mut cmd, kaji_mode)?;
...
impl kaji_providers::base::ProviderDescriptor for CodexProvider {
...
impl Provider for CodexProvider {
    ...
    async fn stream(
        ...
        let lines = self
            .execute_command(model_config, system, messages, tools, kaji_mode)
            .await?;
...
mod tests {
    ...
    use kaji_providers::base::ProviderDescriptor as _;
    use kaji_test_support::TEST_IMAGE_B64;
    use std::collections::HashMap;
    ...
    #[test_case(KajiMode::Auto, &["--yolo"] ; "auto_yolo")]
    #[test_case(KajiMode::SmartApprove, &["--full-auto"] ; "smart_approve_full_auto")]
    #[test_case(KajiMode::Approve, &[] as &[&str] ; "approve_no_flags")]
    #[test_case(KajiMode::Chat, &["--sandbox", "read-only"] ; "chat_read_only")]
    fn test_apply_permission_flags(mode: KajiMode, expected: &[&str]) {
