...
impl kaji_providers::base::ProviderDescriptor for CodexAcpProvider {
    fn metadata() -> ProviderMetadata {
        ProviderMetadata::new(
            CODEX_ACP_PROVIDER_NAME,
            "Codex CLI",
            "Use kaji with ChatGPT Plus/Pro or OpenAI API credits via the codex-acp adapter.",
            ACP_CURRENT_MODEL,
            vec![],
            CODEX_ACP_DOC_URL,
            vec![],
        ...
        .with_setup_steps(vec![
            ...
            "Configure kaji in `~/.config/kaji/config.yaml`:\n  KAJI_PROVIDER: codex-acp\n  KAJI_MODEL: current",
            "Restart kaji",
        ])
    }
}
    ...
    fn from_env_with_working_dir(
        ...
        Box::pin(async move {
            ...
            let mode_mapping = HashMap::from([
                (KajiMode::Auto, vec!["agent-full-access".to_string()]),
                (KajiMode::SmartApprove, vec!["agent".to_string()]),
