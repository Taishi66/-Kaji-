...
impl kaji_providers::base::ProviderDescriptor for CopilotAcpProvider {
    fn metadata() -> ProviderMetadata {
        ProviderMetadata::new(
            COPILOT_ACP_PROVIDER_NAME,
            "GitHub Copilot CLI (ACP)",
            "Use kaji with your GitHub Copilot subscription via the Copilot CLI.",
            ACP_CURRENT_MODEL,
            vec![],
            COPILOT_ACP_DOC_URL,
            vec![],
    ...
    fn from_env_with_working_dir(
        ...
        Box::pin(async move {
            ...
            let mode_mapping = HashMap::from([
                (KajiMode::Auto, vec![MODE_AGENT.to_string()]),
                (KajiMode::Approve, vec![MODE_AGENT.to_string()]),
                (KajiMode::SmartApprove, vec![MODE_AGENT.to_string()]),
                (KajiMode::Chat, vec![MODE_PLAN.to_string()]),
            ]);
