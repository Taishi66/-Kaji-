...
fn enrich_model_ids_with_canonical(
    ...
    // For Databricks providers, prefer kaji- prefixed model_ids when there are duplicates.
    // Re-scan: if a later model_id with "kaji-" prefix maps to the same display name,
    // swap it in.
...
mod tests {
    ...
    #[test]
    fn databricks_v2_inventory_prefers_kaji_model_ids_for_duplicate_names() {
        let models = enrich_model_ids_with_canonical(
            "databricks_v2",
            &["databricks-gpt-5-5".to_string(), "kaji-gpt-5-5".to_string()],
        );

        assert!(
            models.iter().any(|model| model.id == "kaji-gpt-5-5"),
            "expected kaji-gpt-5-5 to win duplicate canonical-name tie, got {models:?}"
        );
        assert!(
            !models.iter().any(|model| model.id == "databricks-gpt-5-5"),
            "expected databricks-gpt-5-5 to be replaced by kaji-gpt-5-5, got {models:?}"
        );
    }
