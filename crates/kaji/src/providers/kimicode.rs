...
use kaji_providers::formats::anthropic::{AnthropicFormatOptions, ANTHROPIC_PROVIDER_NAME};
...
impl KimiCodeProvider {
    ...
    async fn post(&self, payload: &Value) -> Result<reqwest::Response, ProviderError> {
        ...
        kaji_providers::http_status::send_bounded(
            request,
...
impl kaji_providers::base::ProviderDescriptor for KimiCodeProvider {
    fn metadata() -> ProviderMetadata {
        ...
        .with_setup_steps(vec![
            "Run `kaji configure` and select 'Kimi Code'",
            "A browser window will open — log in to kimi.com and enter the displayed code",
            "Once authorized, Kaji will save your token automatically",
        ])
    }
}
        ...
        KimiCodeProvider {
            ...
            token_cache: TokenCache {
                path: std::env::temp_dir()
                    .join(format!("kaji-kimicode-test-{}.json", Uuid::new_v4())),
            },
            cached_token: tokio::sync::Mutex::new(None),
