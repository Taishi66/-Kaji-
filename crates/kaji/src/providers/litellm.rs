...
use kaji_providers::cache_semantics::apply_chat_payload_breakpoints;
...
impl kaji_providers::base::ProviderDescriptor for LiteLLMProvider {
...
impl Provider for LiteLLMProvider {
    ...
    async fn stream(
        ...
        let mut payload = kaji_providers::formats::openai::create_request(
            model_config,
            system,
            messages,
            tools,
        ...
        let message = kaji_providers::formats::openai::response_to_message(&response)?;
        let usage = kaji_providers::formats::openai::get_usage(&response);
