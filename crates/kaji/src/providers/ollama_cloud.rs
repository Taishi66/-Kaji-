...
use kaji_providers::api_client::{ApiClient, AuthMethod, TlsConfig};
use kaji_providers::base::ProviderDescriptor;
use kaji_providers::errors::ProviderError;
use kaji_providers::model::ModelConfig;
use kaji_providers::ollama::fetch_ollama_model_names;
use kaji_providers::openai::OpenAiProvider;
use rmcp::model::Tool;
...
fn build_ollama_api_client(
    ...
    let normalized_base_url = kaji_providers::openai::ensure_url_scheme(&config.base_url);
...
impl Provider for OllamaCloudProvider {
    ...
    fn retry_config(&self) -> kaji_providers::retry::RetryConfig {
        ...
        Box::pin(async {
            anyhow::bail!(
                "Ollama Cloud must be configured as a declarative provider. \
                 Run `kaji configure` to set it up."
            )
        })
    }
}
