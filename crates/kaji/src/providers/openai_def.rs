...
use kaji_providers::base::ProviderDescriptor;
...
use kaji_providers::api_client::{ApiClient, AuthMethod};
use kaji_providers::openai::{
...
impl ProviderDescriptor for OpenAiProviderDef {
    fn metadata() -> kaji_providers::base::ProviderMetadata {
...
pub async fn from_env(
    tls_config: Option<kaji_providers::api_client::TlsConfig>,
    ...
    //      honoured so that `OPENAI_HOST=… kaji` keeps working)
    //   2. OPENAI_BASE_URL (env or config) — ecosystem-standard
    //   3. OPENAI_HOST from config file — persisted by `kaji configure`
            ...
            match e {
                ConfigError::NotFound(_) => {
                    if config.requires_auth {
                        anyhow::bail!(
                            "Required API key {} is not set. Configure it via `kaji configure` or set the {} environment variable.",
                            config.api_key_env,
                            config.api_key_env
                        );
                    }
...
pub fn from_custom_config(
    ...
    kaji_providers::openai::from_declarative_config(
        config,
        tls_config,
