...
impl kaji_providers::base::ProviderDescriptor for PiAcpProvider {
    fn metadata() -> ProviderMetadata {
        ProviderMetadata::new(
            PI_ACP_PROVIDER_NAME,
            "Pi",
            "Use kaji with Pi via the pi-acp adapter.",
            ACP_CURRENT_MODEL,
            vec![],
            PI_ACP_DOC_URL,
            vec![],
...
impl ProviderDef for PiAcpProvider {
    ...
    fn from_env_with_working_dir(
        ...
        Box::pin(async move {
            ...
            // Wire it up so model selection from kaji config or Buzz's set_model
