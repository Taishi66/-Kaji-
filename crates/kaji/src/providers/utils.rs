...
use fs_err::File;
use kaji_providers::errors::{GoogleErrorCode, ProviderError};
use kaji_providers::request_log::{install_logger, RequestLogHandle, RequestLogger};
            ...
            if let Some(payload) = &payload {
                if let Some(error) = payload.get("error") {
                    ...
                    if error_status == "INVALID_ARGUMENT"
                        && kaji_providers::http_status::is_context_length_exceeded_message(&error_msg)
                    {
...
pub fn init_kaji_request_log() -> Result<()> {
    INIT_LOGGER
