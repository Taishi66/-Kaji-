...
fn build_authorize_url(pkce: &PkceChallenge, state: &str, nonce: &str) -> Result<String> {
    ...
    // non-allowlisted clients. `referrer=kaji` lets xAI attribute
    // kaji-originated logins.
    let params = [
        ("response_type", "code"),
        ("client_id", CLIENT_ID),
...
async fn poll_device_code_token(device: &DeviceCodeResponse) -> Result<TokenResponse> {
    ...
    loop {
        ...
        match body.error.as_deref() {
            ...
            Some("expired_token") => {
                return Err(anyhow!(
                    "xAI device code expired - please re-run kaji configure"
                ));
            }
            other => {
                let detail = body
...
<html>
  <head>
    <title>kaji - xAI Authorization Successful</title>
    <style>
      body {
  ...
  <body>
    <div class="container">
      ...
      <p>You can close this window and return to kaji.</p>
    </div>
...
<html>
  <head>
    <title>kaji - xAI Authorization Failed</title>
    <style>
      body {{
        font-family: system-ui, -apple-system, sans-serif;
...
async fn perform_loopback_oauth_flow(auth_state: &XaiAuthState) -> Result<TokenData> {
    ...
    if webbrowser::open(&auth_url).is_err() {
        tracing::info!(
            "Please open this URL in your browser to authorize kaji with xAI:\n{}",
            auth_url
        );
    }
...
async fn perform_device_code_flow() -> Result<TokenData> {
    ...
    eprintln!(
        "\nTo authorize kaji with xAI, open this URL in any browser:\n  {}\nand enter code: {}\n",
...
impl kaji_providers::base::ProviderDescriptor for XaiOAuthProvider {
...
mod tests {
    ...
    fn authorize_url_contains_required_oauth_params() {
        ...
        assert!(url.contains("referrer=kaji"));
    ...
    #[test]
    fn token_cache_path_lives_under_kaji_config_dir() {
