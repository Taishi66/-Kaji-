...
use crate::agents::{Agent, AgentConfig, AgentEvent, KajiPlatform, SessionConfig};
use crate::config::paths::Paths;
use crate::config::permission::PermissionManager;
use crate::config::{resolve_extensions_for_new_session, Config, KajiMode};
...
async fn execute_job(
    ...
    agent
        ...
        .await?;
    agent.update_kaji_mode(KajiMode::Auto, &session.id).await?;
...
mod tests {
    ...
    async fn validated_recipe_bytes_and_base_are_persisted_after_source_replacement() {
        let temp_dir = tempdir().unwrap();
        let _guard =
            env_lock::lock_env([("KAJI_PATH_ROOT", Some(temp_dir.path().to_str().unwrap()))]);
    ...
    async fn test_job_runs_on_schedule() {
        let _guard = env_lock::lock_env([
            ("KAJI_PROVIDER", Some("openai")),
            ("KAJI_MODEL", Some("gpt-4o")),
            ("KAJI_MODE", Some("chat")),
        ...
        assert_eq!(sessions[0].kaji_mode, KajiMode::Auto);
