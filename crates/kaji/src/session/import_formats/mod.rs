//! Importers for non-kaji session formats.
//!
//! Kaji's native session export is a JSON-serialized [`crate::session::Session`].
...
//!
//! The strategy is to convert any supported foreign format into kaji's
...
/// the kaji-native session JSON handed to `SessionManager::import_session`.
...
pub(crate) fn build_session_json(session: ImportedSession) -> Value {
    ...
    obj.insert("kaji_mode".into(), json!("auto"));
...
pub enum ImportFormat {
    /// Native kaji session export — a JSON object representing a `Session`.
    Kaji,
...
///   and a `conversation` (or `messages`) field, it's kaji.
...
pub fn detect_format(content: &str) -> ImportFormat {
    ...
    if let Ok(v) = serde_json::from_str::<serde_json::Value>(first_line) {
        ...
        // Claude Code lines always include a sessionId; kaji's native JSON is
    ...
    // Kaji's pretty-printed export starts with `{` and *eventually* contains
...
/// Convert any supported foreign format to a kaji-native session JSON string.
pub fn convert_to_kaji_session_json(content: &str) -> Result<String> {
    match detect_format(content) {
        ImportFormat::Kaji => Ok(upgrade_legacy_token_fields(content)),
...
/// Exports from kaji versions before `usage`/`accumulated_usage` existed
