...
pub const CONFIG_RELAYS_KEY: &str = "KAJI_NOSTR_RELAYS";
...
pub fn build_deeplink(nevent: &str, decryption_key: &str) -> String {
    format!(
        "kaji://sessions/nostr?nevent={}&key={}",
...
pub fn parse_deeplink(deeplink: &str) -> Result<ParsedShareLink> {
    let parsed = url::Url::parse(deeplink).context("Invalid Kaji session share link")?;
    if parsed.scheme() != "kaji"
        || parsed.host_str() != Some("sessions")
        || parsed.path() != "/nostr"
    {
        return Err(anyhow!("Invalid Kaji Nostr session share link"));
    }
...
mod tests {
    ...
    async fn publish_builds_deeplink_and_encrypted_kind_30278_event() {
        ...
        assert!(share.deeplink.starts_with("kaji://sessions/nostr?"));
    ...
    #[test]
    fn parses_deeplink() {
        let parsed = parse_deeplink("kaji://sessions/nostr?nevent=abc&key=def").unwrap();
