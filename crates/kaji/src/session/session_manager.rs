...
impl<'a> SessionUpdateBuilder<'a> {
    ...
    pub fn kaji_mode(mut self, mode: KajiMode) -> Self {
        self.kaji_mode = Some(mode);
        self
    }
...
impl SessionManager {
    ...
    pub async fn create_session(
        ...
        self.storage
            .create_session(working_dir, name, session_type, kaji_mode)
            .await
    }
...
fn deserialize_session_model_config(
    ...
    if provider_name == Some(kaji_providers::azure_foundry::AZURE_FOUNDRY_PROVIDER_NAME) {
        #[derive(Deserialize)]
...
impl SessionStorage {
    ...
    async fn import_legacy_session(pool: &Pool<Sqlite>, session: &Session) -> Result<()> {
        ...
        .bind(session.kaji_mode.to_string())
        .execute(&mut *tx)
        .await?;
    ...
    async fn apply_migration(tx: &mut sqlx::Transaction<'_, Sqlite>, version: i32) -> Result<()> {
        match version {
            ...
            8 => {
                sqlx::query(
                    r#"
                    ALTER TABLE sessions ADD COLUMN kaji_mode TEXT NOT NULL DEFAULT 'auto'
                "#,
                )
                .execute(&mut **tx)
                .await?;
    ...
    async fn apply_update(&self, builder: SessionUpdateBuilder<'_>) -> Result<()> {
        ...
        add_update!(builder.kaji_mode, "kaji_mode");
        ...
        if let Some(kaji_mode) = builder.kaji_mode {
            q = q.bind(kaji_mode.to_string());
        }
    ...
    async fn list_sessions_matching(&self, query: SessionListQuery<'_>) -> Result<Vec<Session>> {
        ...
        let sql = format!(
            ...
            SELECT s.id, s.working_dir, s.name, s.description, s.user_set_name, s.session_type, s.created_at, s.updated_at, s.extension_data,
                   ...
                   s.provider_name, s.model_config_json, s.kaji_mode,
    ...
    async fn import_session(
        ...
        let normalized = super::import_formats::convert_to_kaji_session_json(json)?;
    ...
    async fn copy_session(
        ...
        builder = builder.kaji_mode(original_session.kaji_mode);
...
mod tests {
    ...
    use kaji_providers::conversation::token_usage::{CostSource, ProviderUsage};
    ...
    fn azure_session_model_config_preserves_suffixed_deployment_id() {
        ...
        let config = deserialize_session_model_config(
            Some(kaji_providers::azure_foundry::AZURE_FOUNDRY_PROVIDER_NAME),
            &json,
        )
        .unwrap();
    ...
    fn azure_session_model_config_preserves_explicit_thinking_effort() {
        ...
        assert_eq!(
            ...
            Some(kaji_providers::thinking::ThinkingEffort::Low)
        );
    }
    ...
    fn non_azure_session_model_config_keeps_suffix_normalization() {
        let config = deserialize_session_model_config(
            Some(kaji_providers::openai::OPEN_AI_PROVIDER_NAME),
    ...
    impl Provider for NamingTestProvider {
        ...
        ) -> std::result::Result<MessageStream, kaji_providers::errors::ProviderError> {
    ...
    #[tokio::test]
    async fn test_kaji_mode_persists(mode: KajiMode) {
        let temp_dir = TempDir::new().unwrap();
        let sm = SessionManager::new(temp_dir.path().to_path_buf());
        ...
        let reloaded = sm.get_session(&session.id, false).await.unwrap();
        assert_eq!(reloaded.kaji_mode, mode);
    }

    #[tokio::test]
    async fn test_kaji_mode_update() {
        let temp_dir = TempDir::new().unwrap();
        let sm = SessionManager::new(temp_dir.path().to_path_buf());
        ...
        sm.update(&session.id)
            .kaji_mode(KajiMode::Approve)
            .apply()
            .await
            .unwrap();

        let reloaded = sm.get_session(&session.id, false).await.unwrap();
        assert_eq!(reloaded.kaji_mode, KajiMode::Approve);
    }

    #[tokio::test]
    async fn test_kaji_mode_malformed_defaults_to_auto() {
        let temp_dir = TempDir::new().unwrap();
        let sm = SessionManager::new(temp_dir.path().to_path_buf());
        ...
        sqlx::query("UPDATE sessions SET kaji_mode = 'garbage' WHERE id = ?")
            .bind(&session.id)
            .execute(pool)
            .await
            .unwrap();
        ...
        let reloaded = sm.get_session(&session.id, false).await.unwrap();
        assert_eq!(reloaded.kaji_mode, KajiMode::default());
    }
