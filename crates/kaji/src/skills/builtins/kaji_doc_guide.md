---
name: kaji-doc-guide
description: Reference kaji documentation to create, configure, or explain kaji-specific features like recipes, extensions, sessions, and providers. You MUST read the relevant kaji docs before answering. You MUST NOT rely on training data or assumptions for any kaji-specific fields, values, names, syntax, or commands.
---

Use this skill when working with **kaji-specific features**:
- Creating or editing recipes
- Configuring extensions or providers
- Explaining how kaji features work
- Any kaji configuration or setup task

Do NOT use this skill for:
- General coding tasks unrelated to kaji
- Running existing recipes (just run them directly)

The docs root for this session is `{{KAJI_DOCS_ROOT}}`. It may be a local
...
1. **Read official docs**
   - Read the doc map from `<docs-root>/kaji-docs-map.md`
...
2. **Create/modify content**
   - For kaji configuration files:
      ...
      - **⚠️ STOP: Before showing the user, verify output content MUST match the schema and reference in the kaji official documentation:**
...
3. **MANDATORY VERIFICATION - CHECK ALL THESE ITEMS BEFORE STEP 4**
   ...
   - [ ] You MUST NOT rely on training data or assumptions for any kaji-specific fields, values, names, syntax, or commands.
   ...
   - [ ] List all kaji-specific items in your answer (commands, fields, syntax, values, how to use, explanations, etc.)
...
4. **Provide your answer and include a "Verification Completed" section**
   - For EACH kaji-specific item in your response, cite the specific doc file where you verified it
