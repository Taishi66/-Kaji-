...
fn is_global_agent_file(path: &Path) -> bool {
    ...
    if let Some(home) = dirs::home_dir() {
        ...
        global_roots.push(home.join(".kaji").join("agents"));
...
fn list_agent_dirs(working_dir: Option<&Path>, additional_roots: &[SourceRoot]) -> Vec<SourceRoot> {
    ...
    if let Some(working_dir) = working_dir {
        ...
        dirs.push(SourceRoot {
            path: working_dir.join(".kaji").join("agents"),
            writable: true,
        });
        dirs.push(SourceRoot {
    ...
    if let Some(home) = dirs::home_dir() {
        ...
        dirs.push(SourceRoot {
            path: home.join(".kaji").join("agents"),
            writable: true,
        });
        dirs.push(SourceRoot {
...
mod tests {
    ...
    fn filesystem_skill_suppresses_same_named_builtin() {
        ...
        assert!(!builtins
            .iter()
            .any(|source| source.name == "kaji-doc-guide"));
    ...
    #[test]
    fn project_sources_prefer_agents_directory_over_legacy_kaji() {
        let tmp = TempDir::new().unwrap();
        let agents_skill_dir = tmp
            ...
            .join("skills")
            .join("shared-skill");
        let legacy_skill_dir = tmp.path().join(".kaji").join("skills").join("shared-skill");
        ...
        std::fs::write(
            ...
            build_skill_md("shared-skill", "legacy", "Kaji", &HashMap::new()),
        )
        .unwrap();
    ...
    fn update_rejects_path_traversal() {
        let tmp = TempDir::new().unwrap();
        let project = tmp.path();
        let escaped_dir = project.join(".kaji").join("escaped");
        ...
        let attempted_escape = project.join(".kaji").join("escaped");
        let err = update_source_with_roots(
            SourceType::Skill,
