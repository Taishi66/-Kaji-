...
use kaji::config::declarative_providers::load_provider;
use kaji::config::paths::Paths;
use kaji::config::{Config, ConfigError, DeclarativeProviderConfig};
use kaji_test_support::EnforceSessionId;
...
fn acp_catalog_and_custom_provider_methods_use_core_provider_store() {
    ...
    let _env = env_lock::lock_env([
        ("KAJI_PATH_ROOT", Some(root_path.as_str())),
    ...
    run_test(async move {
        ...
        assert!(
            ...
            "custom provider should be saved in Kaji's declarative provider store"
        );
        ...
        assert_eq!(
            Config::global()
                ...
                .unwrap(),
            "created-custom-key",
            "custom provider create should write through Kaji's config store"
        );
        assert!(
        ...
        assert_eq!(
            Config::global()
                ...
                .unwrap(),
            "updated-custom-key",
            "custom provider update should write through Kaji's config store"
        );
        ...
        for valid_id in ["custom_openai", "openai-compat", "a1"] {
            assert!(
                kaji::config::declarative_providers::validate_provider_id(valid_id).is_ok(),
        ...
        for (name, patch) in [
            ...
            let result =
                send_custom(conn.cx(), "_kaji/unstable/providers/custom/create", payload).await;
