...
use kaji_test_support::{EnforceSessionId, IgnoreSessionId};
...
fn write_acp_global_config(contents: &str) -> PathBuf {
    ...
    let config_dir = kaji::config::paths::Paths::config_dir();
    std::fs::create_dir_all(&config_dir).unwrap();
    let mut contents = contents.to_string();
    if !contents.contains("KAJI_DISABLE_KEYRING") {
        contents.push_str("KAJI_DISABLE_KEYRING: true\n");
    }
    std::fs::write(
        config_dir.join(kaji::config::base::CONFIG_YAML_NAME),
        contents,
    )
    .unwrap();
    config_dir
...
impl Provider for MockProvider {
    ...
    async fn stream(
        ...
        _system: &str,
        _messages: &[kaji::conversation::message::Message],
...
fn steer_chunk_message_ids(updates: &[SessionUpdate]) -> Vec<String> {
    updates
        ...
        .filter_map(|update| {
            ...
            let kaji = chunk.meta.as_ref()?.get("kaji")?;
            kaji.get("steer")?.as_bool().filter(|b| *b)?;
            kaji.get("messageId")?.as_str().map(ToString::to_string)
        })
        .collect()
}
...
fn test_custom_get_extensions() {
    ...
    run_test(async move {
        ...
        let add_result = send_custom(
            conn.cx(),
            "_kaji/unstable/config/extensions/add",
            serde_json::json!({
                "enabled": true,
                "extension": {
                    "type": "mcp",
        ...
        let stored_inline_token = kaji::config::Config::global()
...
fn test_custom_session_extensions_add_list_remove() {
    ...
    run_test(async move {
        ...
        let add_result = send_custom(
            conn.cx(),
            "_kaji/unstable/session/extensions/add",
            serde_json::json!({
...
fn test_steer_session_adds_input_to_active_prompt() {
    ...
    run_test(async move {
        ...
        assert!(
            ...
            "expected a chunk marked _meta.kaji.steer with the steer text, got: {steer_chunks:?}"
        );
...
fn test_custom_list_builtin_skill_sources() {
    ...
    run_test(async move {
        ...
        let builtin = sources
            .iter()
            .find(|source| source.get("name") == Some(&serde_json::json!("kaji-doc-guide")))
            .expect("expected kaji-doc-guide builtin skill");
        ...
        assert_eq!(
            builtin.get("path"),
            Some(&serde_json::json!("builtin://skills/kaji-doc-guide"))
        );
    });
}
...
fn test_custom_preferences_read_save_remove() {
    let config_dir = write_acp_global_config(
        "KAJI_MODEL: gpt-4o\nKAJI_PROVIDER: openai\nKAJI_AUTO_COMPACT_THRESHOLD: 0.7\nKAJI_THINKING_EFFORT: high\nVOICE_AUTO_SUBMIT_PHRASES: send it\n",
    );
    ...
    run_test(async move {
        ...
        assert_eq!(
            ...
            Some(&serde_json::json!([
                ...
                { "key": "kajiThinkingEffort", "value": "high" },
        ...
        send_custom(
            conn.cx(),
            "_kaji/unstable/preferences/save",
            serde_json::json!({
                "values": [
                    { "key": "kajiThinkingEffort", "value": "disabled" },
        ...
        send_custom(
            conn.cx(),
            "_kaji/unstable/preferences/remove",
            serde_json::json!({
        ...
        let response = send_custom(
            ...
            "_kaji/unstable/preferences/read",
            serde_json::json!({
                "keys": ["kajiThinkingEffort", "voiceDictationProvider", "voiceDictationPreferredMic"],
            }),
        )
        .await
        ...
        assert_eq!(
            response.get("values"),
            Some(&serde_json::json!([
                { "key": "kajiThinkingEffort", "value": "off" },
...
fn test_custom_preferences_save_rejects_invalid_values() {
    ...
    run_test(async {
        ...
        let invalid_payloads = [
            ...
            serde_json::json!({
                "values": [{ "key": "kajiThinkingEffort", "value": "bogus" }],
            }),
            serde_json::json!({
                "values": [{ "key": "kajiThinkingEffort", "value": ["high"] }],
            }),
            serde_json::json!({
        ...
        for payload in invalid_payloads {
            let result = send_custom(conn.cx(), "_kaji/unstable/preferences/save", payload).await;
...
fn test_custom_defaults_read() {
    let config_dir =
        write_acp_global_config("KAJI_MODEL: claude-3-5-haiku-latest\nKAJI_PROVIDER: anthropic\n");
...
fn test_custom_dictation_secret_save_delete() {
    ...
    run_test(async move {
        ...
        send_custom(
            conn.cx(),
            "_kaji/unstable/dictation/secret/save",
            serde_json::json!({
                "provider": "groq",
                "value": "groq-key",
            }),
...
fn test_provider_switching_updates_session_state() {
    ...
    run_test(async {
        ...
        conn.set_config_option(&session_id, "provider", "kaji")
            .await
            .expect("provider reset to kaji should succeed");
    });
}
                ...
                Box::pin(async move {
                    Ok(Arc::new(MockProvider {
                        ...
                        supported_models: Ok(vec![
                            "kaji-claude-opus-4-8".to_string(),
