...
use kaji::providers::openai::OpenAiProvider;
use kaji::scheduler::{ScheduledJob, SchedulerError, ValidatedScheduleRecipe};
use kaji::scheduler_trait::SchedulerTrait;
use kaji::session::Session as KajiSession;
use kaji::session_context::SESSION_ID_HEADER;
use kaji_test_support::{ExpectedSessionId, TEST_MODEL};
...
impl SchedulerTrait for FixtureScheduler {
    ...
    async fn sessions(
        ...
        sched_id: &str,
        _limit: usize,
    ) -> Result<Vec<(String, KajiSession)>, SchedulerError> {
...
fn write_global_test_config(config_path: &Path, openai_base_url: &str) {
    ...
    let global_config_path = global_config_dir.join(kaji::config::base::CONFIG_YAML_NAME);
...
#[allow(dead_code)]
pub async fn serve_agent_in_process(agent: Arc<KajiAcpAgent>) -> (DuplexTransport, JoinHandle<()>) {
...
pub async fn spawn_acp_server_in_process(
    ...
    let config_path = data_root.join(kaji::config::base::CONFIG_YAML_NAME);
    if !config_path.exists() {
        fs::write(
            &config_path,
            format!(
                "KAJI_MODEL: {current_model}\nKAJI_PROVIDER: openai\nKAJI_MODE: {}\nKAJI_TOOL_PAIR_SUMMARIZATION: false\n",
                kaji_mode
            ),
        )
        .unwrap();
    ...
    let agent = KajiAcpAgent::new(KajiAcpAgentOptions {
        provider_factory,
        builtin_selection: kaji::acp::server::AcpBuiltinSelection {
