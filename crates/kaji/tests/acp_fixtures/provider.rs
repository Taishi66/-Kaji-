...
use kaji::acp::{AcpProvider, AcpProviderConfig};
...
use kaji_test_support::{ExpectedSessionId, IgnoreSessionId, TEST_MODEL};
...
impl AcpProviderSession {
    ...
    async fn send_message(
        ...
        let mut stream = kaji::session_context::with_session_id(
...
impl Connection for AcpProviderConnection {
    ...
    async fn new(config: TestConnectionConfig, openai: OpenAiFixture) -> Self {
        ...
        let kaji_mode = config.kaji_mode;
        ...
        let provider_config = AcpProviderConfig {
            ...
            mode_mapping: KajiMode::VARIANTS
                .iter()
                .map(|v| {
                    let mode = KajiMode::from_str(v).unwrap();
    ...
    async fn new_session(&mut self) -> anyhow::Result<SessionData<AcpProviderSession>> {
        ...
        let kaji_id = format!("test-session-{}", self.session_counter);
        ...
        let session = AcpProviderSession {
            ...
            session_id: agent_client_protocol::schema::v1::SessionId::new(kaji_id),
