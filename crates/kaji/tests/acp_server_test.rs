...
use kaji::conversation::message::{Message, MessageMetadata};
use kaji::custom_requests::{GetSessionInfoRequest, GetSessionInfoResponse};
use kaji::recipe::{Recipe, Settings};
...
fn include_last_message_snippet_meta(
    ...
    kaji.insert("includeLastMessageSnippet".to_string(), value);
...
fn test_new_session_honors_recipe_model_without_recipe_provider() {
    run_test(async {
        ...
        let recipe = Recipe::builder()
            ...
            .settings(Settings {
                kaji_provider: None,
                kaji_model: Some(recipe_model.to_string()),
                temperature: None,
                max_turns: None,
            })
            .build()
