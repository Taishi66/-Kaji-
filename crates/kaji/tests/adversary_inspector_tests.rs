...
use kaji::conversation::message::{Message, MessageContent, ToolRequest};
use kaji::security::adversary_inspector::AdversaryInspector;
...
async fn test_adversary_disabled_without_config_file() {
    ...
    let inspector = AdversaryInspector::with_config_dir(
        provider,
        Arc::new(kaji::session::SessionManager::new(tmp.path().to_path_buf())),
...
async fn test_adversary_enabled_default_tools() {
    ...
    assert!(matches!(
        results[0].action,
        kaji::tool_inspection::InspectionAction::Allow
    ));
