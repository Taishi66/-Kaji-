...
use kaji::agents::{Agent, AgentEvent, KajiPlatform};
use kaji::config::extensions::{set_extension, ExtensionEntry};
...
mod tests {
    ...
    mod schedule_tool_tests {
        ...
        use kaji::agents::platform_extensions::scheduler::{
            EXTENSION_NAME as SCHEDULER_EXTENSION_NAME, MANAGE_SCHEDULE_TOOL_NAME_COMPLETE,
        };
        use kaji::agents::ExtensionConfig;
        use kaji::agents::{AgentConfig, ScheduleTool};
        use kaji::config::permission::PermissionManager;
        use kaji::config::KajiMode;
        use kaji::scheduler::{ScheduledJob, SchedulerError, ValidatedScheduleRecipe};
        use kaji::scheduler_trait::SchedulerTrait;
        ...
        async fn test_schedule_management_tool_list() {
            ...
            assert!(tool
                ...
                .clone()
                .unwrap_or_default()
                .contains("Manage kaji's internal scheduled recipe execution"));
        }
    ...
    mod retry_tests {
        use super::*;
        use kaji::agents::types::{RetryConfig, SuccessCheck};

        #[tokio::test]
        async fn test_retry_success_check_execution() -> Result<()> {
            use kaji::agents::retry::execute_success_checks;
    ...
    mod max_turns_tests {
        use super::*;
        use async_trait::async_trait;
        use kaji::agents::SessionConfig;
        use kaji::config::KajiMode;
        use kaji::conversation::message::{Message, MessageContent};
        use kaji::providers::base::{
        ...
        impl kaji::providers::base::ProviderDescriptor for MockToolProvider {
        ...
        impl ProviderDef for MockToolProvider {
            ...
            fn from_env(
                _extensions: Vec<kaji::config::ExtensionConfig>,
                _tls_config: Option<kaji::providers::api_client::TlsConfig>,
            ...
            while let Some(response_result) = reply_stream.next().await {
                match response_result {
                    Ok(AgentEvent::Message(response)) => {
                        ...
                        {
                            if let kaji::conversation::message::ActionRequiredData::ToolConfirmation { id, .. } = &action.data {
                                agent.handle_confirmation(
                                    id.clone(),
                                    kaji::permission::PermissionConfirmation {
                                        principal_type: kaji::permission::permission_confirmation::PrincipalType::Tool,
                                        permission: kaji::permission::Permission::AllowOnce,
                                    }
                                ).await;
                            }
                        }
    ...
    mod unparseable_tool_call_tests {
        ...
        impl kaji::providers::base::ProviderDescriptor for UnparseableToolProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
    ...
    mod tool_pair_summarization_tests {
        ...
        impl kaji::providers::base::ProviderDescriptor for SummarizationTestProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        async fn test_batch_summarization_preserves_all_summaries() -> Result<()> {
            ...
            Config::global()
                .set_param("KAJI_TOOL_CALL_CUTOFF", 2)
                .unwrap();
            ...
            Config::global().delete("KAJI_TOOL_CALL_CUTOFF").unwrap();
    ...
    mod extension_manager_tests {
        use super::*;
        use kaji::agents::extension::ExtensionConfig;
        use kaji::agents::platform_extensions::{
            MANAGE_EXTENSIONS_TOOL_NAME, SEARCH_AVAILABLE_EXTENSIONS_TOOL_NAME,
        };
        use kaji::agents::AgentConfig;
        use kaji::config::permission::PermissionManager;
        use kaji::config::KajiMode;
        use kaji::session::SessionManager;
        ...
        async fn setup_agent_with_extension_manager() -> (Agent, String) {
            ...
            let todo_extension_entry = ExtensionEntry {
                ...
                config: ExtensionConfig::Platform {
                    name: "todo".to_string(),
                    description:
                        "Enable a todo list for kaji so it can keep track of what it is doing"
                            .to_string(),
    ...
    mod streaming_persistence_tests {
        ...
        use kaji::config::KajiMode;
        use kaji::conversation::message::Message;
        use kaji::providers::base::{MessageStream, Provider, ProviderDef, ProviderMetadata};
        use kaji::session::session_manager::SessionType;
        use kaji::session::SessionManager;
        use kaji_providers::conversation::token_usage::{ProviderUsage, Usage};
        use kaji_providers::errors::ProviderError;
        ...
        impl kaji::providers::base::ProviderDescriptor for MultiStepProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
    ...
    mod thinking_preservation_tests {
        ...
        impl kaji::providers::base::ProviderDescriptor for ThinkingStreamProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        fn assert_formatter_adds_reasoning_to_tool_calls(messages: &[Message], provider: &str) {
            use kaji_providers::formats::openai::{
                format_messages_with_options, OpenAiFormatOptions,
            };
            use kaji_providers::images::ImageFormat;
        ...
        impl kaji::providers::base::ProviderDescriptor for CombinedThinkingToolProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        impl kaji::providers::base::ProviderDescriptor for MultiToolThinkingProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        async fn test_signed_thinking_not_duplicated_for_anthropic() -> Result<()> {
            use kaji_providers::formats::anthropic::format_messages as anthropic_format;
    ...
    mod goal_checking_tests {
        ...
        impl kaji::providers::base::ProviderDescriptor for GoalTextProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
    ...
    mod frontend_extension_tests {
        use super::*;
        use kaji::agents::{AgentConfig, ExtensionConfig};
        use kaji::config::permission::PermissionManager;
        use kaji::config::KajiMode;
        use kaji::session::session_manager::SessionType;
        use kaji::session::{
    ...
    mod audience_tool_result_tests {
        ...
        use async_trait::async_trait;
        use kaji::agents::{AgentConfig, SessionConfig};
        use kaji::config::{ExtensionConfig, KajiMode, PermissionManager};
        use kaji::conversation::message::{Message, MessageContent};
        use kaji::providers::base::{stream_from_single_message, MessageStream, Provider};
        use kaji::session::{SessionManager, SessionType};
        use kaji_providers::conversation::token_usage::{ProviderUsage, Usage};
        use kaji_providers::errors::ProviderError;
        use kaji_providers::model::ModelConfig;
        use kaji_test_support::McpFixture;
        use rmcp::model::{CallToolRequestParams, Tool};
        use std::path::PathBuf;
        use std::sync::atomic::{AtomicUsize, Ordering};
    ...
    mod empty_turn_tests {
        use super::*;
        use async_trait::async_trait;
        use kaji::agents::final_output_tool::FINAL_OUTPUT_TOOL_NAME;
        use kaji::agents::{AgentConfig, AgentEvent, KajiPlatform, SessionConfig};
        use kaji::config::permission::PermissionManager;
        use kaji::config::KajiMode;
        use kaji::conversation::message::{Message, MessageContent};
        ...
        impl kaji::providers::base::ProviderDescriptor for AssistantOnlyProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        impl kaji::providers::base::ProviderDescriptor for EmptyThenTextProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        async fn test_empty_turn_with_final_output_tool_nudges() -> Result<()> {
            use kaji::agents::final_output_tool::FINAL_OUTPUT_CONTINUATION_MESSAGE;
            use kaji::recipe::Response;
