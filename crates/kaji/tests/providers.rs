...
use kaji::acp::ACP_CURRENT_MODEL;
use kaji::agents::{Agent, AgentConfig, AgentEvent, KajiPlatform, PromptManager, SessionConfig};
...
use kaji::providers::anthropic::ANTHROPIC_DEFAULT_MODEL;
use kaji::providers::azure::AZURE_DEFAULT_MODEL;
use kaji::providers::base::Provider;
#[cfg(feature = "aws-providers")]
use kaji::providers::bedrock::BEDROCK_DEFAULT_MODEL;
use kaji::providers::claude_code::CLAUDE_CODE_DEFAULT_MODEL;
use kaji::providers::codex::CODEX_DEFAULT_MODEL;
use kaji::providers::create_with_named_model;
use kaji::providers::google::GOOGLE_DEFAULT_MODEL;
use kaji::providers::litellm::LITELLM_DEFAULT_MODEL;
use kaji::providers::openai::OPEN_AI_DEFAULT_MODEL;
#[cfg(feature = "aws-providers")]
use kaji::providers::sagemaker_tgi::SAGEMAKER_TGI_DEFAULT_MODEL;
use kaji::providers::snowflake::SNOWFLAKE_DEFAULT_MODEL;
use kaji::providers::xai::XAI_DEFAULT_MODEL;
use kaji::session::{SessionManager, SessionType};
use kaji_providers::databricks::DATABRICKS_DEFAULT_MODEL;
use kaji_providers::errors::ProviderError;
use kaji_test_support::{
...
impl ProviderFixture {
    async fn setup(config: &ProviderTestConfig, mode: KajiMode) -> Result<Self> {
        let mut env_vars: Vec<(&'static str, Option<&str>)> =
            vec![("KAJI_MODE", Some(<&str>::from(mode)))];
        ...
        let model_config = kaji::model_config::model_config_from_user_config(
            &config.name.to_lowercase(),
            config.model_name,
        )?;
    ...
    async fn tool_roundtrip(
        &self,
        prompt: &str,
        model_config: Option<kaji_providers::model::ModelConfig>,
    ) -> Result<Message> {
        let tools = self
            .agent
            .extension_manager
        ...
        let (response1, _) = kaji::session_context::with_session_id(
        ...
        let ctx = kaji::agents::ToolCallContext::new(
        ...
        let (response2, _) = kaji::session_context::with_session_id(
            Some(self.session_id.clone()),
            self.provider.complete(
                &model_config,
                &system,
    ...
    async fn test_basic_response(&self) -> Result<()> {
        ...
        let (response, _) = kaji::session_context::with_session_id(
            Some(self.session_id.clone()),
            self.provider.complete(
                &model_config,
    ...
    async fn test_context_length_exceeded_error(&self) -> Result<()> {
        ...
        let result = kaji::session_context::with_session_id(
            Some(self.session_id.clone()),
            self.provider.complete(
                &model_config,
                "You are a helpful assistant.",
    ...
    async fn test_image_content_support(&self) -> Result<()> {
        let image_config = self.image_model.as_ref().map(|model| {
            kaji_providers::model::ModelConfig::new(model).with_canonical_limits(&self.name)
        });
        let response = self
            .tool_roundtrip(
        ...
        assert!(
            text.contains("hello kaji") || text.contains("test image"),
            "{text}"
        );
    ...
    async fn test_model_switch(&self) -> Result<()> {
        ...
        let alt_config =
            kaji_providers::model::ModelConfig::new(alt).with_canonical_limits(&self.name);
    ...
    async fn test_mode_update(&self) -> Result<()> {
        ...
        self.agent
            .update_kaji_mode(KajiMode::Approve, &self.session_id)
            .await?;
...
async fn test_provider(config: ProviderTestConfig) -> Result<()> {
    ...
    let run_test = |mode: KajiMode| ProviderFixture::setup(&config, mode);

    if run_test(KajiMode::Auto).await.is_err() {
    ...
    let result: Result<()> = async {
        run_test(KajiMode::Auto).await?.test_model_listing().await?;
        run_test(KajiMode::Auto)
            .await?
            .test_basic_response()
            .await?;
        if config.test_mcp_tools {
            run_test(KajiMode::Auto).await?.test_tool_usage().await?;
            run_test(KajiMode::Auto)
                .await?
        ...
        if config.model_switch_name.is_some() {
            run_test(KajiMode::Auto).await?.test_model_switch().await?;
        }
        ...
        if config.test_permissions {
            ...
            if config.test_smart_approve {
                run_test(KajiMode::SmartApprove)
                    .await?
        ...
        if config.test_mode_update {
            run_test(KajiMode::Auto).await?.test_mode_update().await?;
        }
        Ok(())
    }
    .await;
