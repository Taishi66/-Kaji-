...
use kaji::providers::base::Provider as KajiProvider;

static TEST_PROVIDER: Lazy<Mutex<Option<Arc<dyn KajiProvider>>>> = Lazy::new(|| Mutex::new(None));
...
pub async fn set_test_provider(p: Arc<dyn KajiProvider>) {
    let mut guard = TEST_PROVIDER.lock().await;
    *guard = Some(p);
}

pub async fn get_test_provider() -> Option<Arc<dyn KajiProvider>> {
