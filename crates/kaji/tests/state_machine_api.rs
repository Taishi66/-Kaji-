...
use kaji::session::session_manager::token_state_from_session_and_totals;
use kaji::session::{Session, SessionManager, SessionType};
...
async fn custom_pipeline_supports_step_apply_run_tracing_and_usage() -> Result<()> {
    let _env = kaji_test_support::otel::clear_otel_env(&[(
