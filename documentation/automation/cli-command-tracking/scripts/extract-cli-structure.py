#!/usr/bin/env python3
"""
Extract CLI command structure from kaji binary using --help output.

Usage:
    ./extract-cli-structure.py <kaji-binary-path> > output/cli-structure.json
    
Example:
    ./extract-cli-structure.py /path/to/kaji > output/new-cli-structure.json
"""
...
def run_help_command(binary_path: str, command_path: List[str], short: bool = False) -> str:
    ...
    Args:
        binary_path: Path to kaji binary
...
def extract_command_structure(binary_path: str, command_path: List[str] = None, 
    ...
    # Parse command info
    command_name = command_path[-1] if command_path else "kaji"
...
def extract_version(binary_path: str) -> str:
    """Extract version from kaji --version."""
    try:
        result = subprocess.run([binary_path, '--version'], 
                              capture_output=True, text=True, timeout=5)
        # Output is typically "kaji 1.15.0" or similar
...
def main():
    if len(sys.argv) < 2:
        print("Usage: extract-cli-structure.py <kaji-binary-path> [source-version]", file=sys.stderr)
        print("Example: extract-cli-structure.py /usr/local/bin/kaji v1.15.0", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        if result.returncode != 0:
            print(f"Error: {binary_path} is not a valid kaji binary", file=sys.stderr)
            sys.exit(1)
    except Exception as e:
