---
draft: false
title: "Resolving CI Issues with kaji: A Practical Walkthrough"
description: "Leverage kaji to simplify your CI debugging process, fetch detailed information about failed CI runs & annotations directly from GitHub, and even apply fixes directly."
date: 2024-12-11
authors:
  - dalton
---

![CI](kaji-github-ci.png)

Running into Continuous Integration (CI) failures in pull requests can be quite frustrating but they happen very often. In this post, we leverage the GitHub CLI (`gh`) using kaji to simplify your CI debugging process, fetch detailed information about failed CI runs and annotations directly from GitHub, and even apply fixes directly.
...
You'll need the [GitHub CLI](https://cli.github.com/) `gh` to enable kaji's access to CI check run details.  
...
### 2. Configure kaji
Ensure kaji is configured and ready to interact with your repository and local tools. Specifically, you will need to configure a kaji profile with the GitHub toolkit.

Update your `profiles.yaml` file in `~/.config/kaji` with the necessary toolkits:
...
```bash
kaji session start --profile my-profile
```
If the profile’s toolkits are not correctly configured, kaji might fail to access `gh` commands.

## Resolving CI Failures with kaji
Once everything is set up, we can now use kaji to tackle CI issues with the following steps:

### Step 1: Fetch CI Failure Details
Ask kaji to retrieve information about the CI failures for a specific PR.
...
kaji will use the `gh` CLI to check run information and provide a detailed summary of the issues. kaji uses the GitHub CLI to:

- Fetch details about failed CI runs, including logs and annotations.
- Provide a clear and actionable summary of issues.

**Example Command kaji Runs:**
...
```
### Step 2: Request Fixes from kaji
Once kaji provides the failure details, ask it to resolve the issues directly.
...
## Benefits of Using kaji
Using kaji to resolve CI issues streamlines your workflow by:
...
kaji allows you to resolve CI failures efficiently, ensuring confidence in your workflow while reducing the effort required for debugging and testing.

Try it out, and let kaji handle the heavy lifting of CI debugging for you!

<head>
  <meta property="og:title" content="Resolving CI Issues with kaji: A Practical Walkthrough" />
  ...
  <meta property="og:description" content="Leverage kaji to simplify your CI debugging process, fetch detailed information about failed CI runs & annotations directly from GitHub, and even apply fixes directly." />
  ...
  <meta name="twitter:title" content="Resolving CI Issues with kaji: A Practical Walkthrough" />
  <meta name="twitter:description" content="Leverage kaji to simplify your CI debugging process, fetch detailed information about failed CI runs & annotations directly from GitHub, and even apply fixes directly." />
