...
description: Connect Kaji directly to your code editor with this Visual Studio Code MCP.
authors: 
    - tania
---
...
Want to use Kaji in VS Code? On the recent [Wild Kaji Case livestream](https://www.youtube.com/watch?v=hG7AnTw-GLU&ab_channel=BlockOpenSource), hosts [Ebony Louis](https://www.linkedin.com/in/ebonylouis/) and [Adewale Abati](https://www.linkedin.com/in/acekyd/) were joined by [Andrew Gertig](https://www.linkedin.com/in/andrewgertig/), Engineering Lead at Cash App, as he demonstrated the new VSCode MCP and how it brings powerful Kaji-assisted coding capabilities directly into VS Code.

<!--truncate-->

## What is the VSCode MCP?
The [VSCode MCP Server](https://github.com/block/vscode-mcp) and its companion [VSCode Extension](https://marketplace.visualstudio.com/items?itemName=block.vscode-mcp-extension) enable AI agents like Kaji to interact with VS Code through the Model Context Protocol.

As Andrew explained during the stream, an MCP ([Model Context Protocol](https://modelcontextprotocol.io/introduction)) server acts as a proxy between a Large Language Model (LLM) and whatever applications or tools you want to access to, in this case, VS Code. Extensions are add-ons based on this protocol that provide a way to extend Kaji's functionality for your workflow.
...
The extension maintains synchronization between Kaji and your VS Code environment to understand your project structure and make contextually relevant suggestions. During the live demo, this came in handy as Kaji navigated complex codebases with precision.
...
Developers can see proposed changes in real-time with the diff view, making it easy to understand exactly what modifications Kaji is suggesting before accepting them. This was demonstrated when an emoji's sizes visually while preserving existing functionality.
...
- **Smart navigation to specific code locations** - Imagine being able to ask Kaji to take you directly to a function definition or a specific implementation.
...
- **Terminal integration for command execution** - This would allow Kaji to execute commands and display results right in your development environment.
- **Potential VS Code sidebar integration for Kaji chat** - Andrew showed a quick preview of an early prototype showing Kaji running directly inside VS Code.
...
<head>
  ...
  <meta property="og:description" content="Connect Kaji directly to your code editor with this Visual Studio Code MCP." />
  ...
  <meta name="twitter:description" content="Connect Kaji directly to your code editor with this Visual Studio Code MCP." />
