---
title: "Streamlining Detection Development with Kaji Recipes"
description: "A comprehensive guide to using recipes in Kaji to streamline security detection development"
authors:
  - glenn
---

![blog cover](kaji-recipes-detection-development-cover.png)

Creating effective security detections in Panther traditionally requires deep knowledge of detection logic, testing frameworks, and development workflows. The detection engineering team at Block has streamlined this process by building Kaji recipes that automate the entire detection creation lifecycle from initial repository set up to pull request creation.

This blog post explores how to leverage Kaji's [recipe](https://goose-docs.ai/docs/guides/recipes/) and [subrecipe](https://goose-docs.ai/docs/guides/recipes/subrecipes) system to create new detections in Panther with minimal manual intervention, ensuring consistency, quality, and adherence to team standards.
...
### What about .kajihints?
In our [previous post](https://goose-docs.ai/blog/2025/06/02/goose-panther-mcp), we discussed using [.kajihints](/docs/guides/context-engineering/using-kajihints/) to provide persistent context to the Large Language Model (LLM). We continue to use `.kajihints` to define coding standards and universal preferences that guide LLM behavior.

However, to minimize redundancy and avoid conflicting guidance, we adopted a single reference file, `AGENTS.md`, as the source of truth for all agents. Each agent is directed to consult this file, while still supporting agent-specific instructions through their default context files (e.g. `.kajihints`, `CLAUDE.md` etc.) or rules (e.g. `.cursor/rules/`).
...
kaji run --recipe recipe.yaml --params skip_similar_rules_check=true --rule_description="What you want to detect"

# Skip Panther MCP integration
kaji run --recipe recipe.yaml --params skip_panther_mcp=true --rule_description="What you want to detect"

# Create PR automatically
kaji run --recipe recipe.yaml --params create_pr=true --rule_description="What you want to detect"
```
...
## Conclusion
Kaji's recipe and subrecipe system represents a significant advancement in AI-assisted security detection development. By breaking complex workflows into specialized, composable components, teams can achieve:
...
Whether you're building your first Kaji recipe or looking to optimize existing workflows, the patterns and principles outlined here provide a solid foundation for successful automation.
...
- Maintain a **central reference** (e.g. `AGENTS.md`) for all AI agents:
  ...
  - Link steps or sections in `.kajihints`, `CLAUDE.md`, `.cursor/rules/*`, etc., back to this file.
...
<head>
  <meta property="og:title" content="Streamlining Detection Development with Kaji Recipes" />
  ...
  <meta property="og:description" content="A comprehensive guide to using recipes in Kaji to streamline security detection development" />
  ...
  <meta name="twitter:title" content="Streamlining Detection Development with Kaji Recipes" />
  <meta name="twitter:description" content="A comprehensive guide to using recipes in Kaji to streamline security detection development" />
