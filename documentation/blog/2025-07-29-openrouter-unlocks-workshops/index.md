...
description: How we used Open Router to provide frictionless LLM access for Kaji workshops
authors: 
    - rizel
---
...
When my team launched [Kaji](/) in early January 2025, we knew we had something special. We built a free, open source AI agent that leverages the Model Context Protocol. It was inventive in its approach, offering developers a local solution with the flexibility to bring their LLM of choice.

## The LLM Cost Problem

After using the product internally for a few months, my teammates were eager to share Kaji with the developer community through workshops and hackathons. We wanted to provide hands-on experiences where people could actually build with Kaji, because that's how developers fall in love with a product.

But we hit a thorny challenge: while Kaji is free, high-performing LLMs are not.
...
When I came back from parental leave, I was ready to jump back in and tackle this problem head-on. I started by teaming up with Alex Hancock, an MCP steering committee member and Kaji OSS engineer, on planning our first-ever meetup in Boston. My boss, Angie Jones, suggested the meetup would be the perfect moment to get people's hands on Kaji.
...
I built a simple web app around OpenRouter's API. Attendees could visit a link, click a button, and instantly get their own temporary API key. They could plug that into Kaji and start building without having to set up an account on OpenRouter.
...
And it worked. People *actually* used Kaji and loved the entire experience, from the meetup to the talks to Kaji itself.

We've now hosted meetups in Sydney, Berlin, Boston, Atlanta, San Francisco, Texas, and New York. You can read about our past experiences in our [Boston](/blog/2025-03-21-kaji-boston-meetup/index.mdx) and [New York](/blog/2025-04-17-kaji-goes-to-NY/index.mdx) blog posts.

## Upcoming Denver Workshop

We're bringing this hands-on Kaji workshop to Denver on August 5.

Join us for an evening in collaboration with Temporal and Dagger, where you'll get free API credits, build something tangible with Kaji, and learn in-depth about MCP.
...
This system isn't perfect. Right now, it's still a separate experience outside the Kaji interface, and it doesn't scale well for events with different credit amounts or more complex needs.

We're working on it. Kaji engineer, Mic Neale recently opened a [pull request](https://github.com/aaif-goose/goose/pull/3507) to automate Kaji's first-time setup. It simplifies the onboarding flow so new users can log into OpenRouter through their browser, securely authenticate, and get a pre-configured Kaji setup without touching config files or copying API keys. It's a huge leap in user experience and lays the groundwork for future improvements.
...
Want us to run a Kaji workshop or hackathon? We’ll bring the API credits. You bring the builders. 
...
<head>
  ...
  <meta property="og:description" content="How we used Open Router to provide frictionless LLM access for Kaji workshops" />
  ...
  <meta name="twitter:description" content="How we used Open Router to provide frictionless LLM access for Kaji workshops" />
