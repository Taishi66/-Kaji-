...
description: "Learn how to build a full-stack AI app in under an hour using Kaji's subagent orchestration, from planning to testing."
authors: 
    - angie
---
...
That's exactly what we accomplished during the Vibe Coding workshop at [Berkeley's Agentic AI Summit](https://www.youtube.com/live/_w5m3h9jY-w?t=5310), where I demonstrated how to use Kaji's subagent orchestration to spin up an entire development team of AI agents. Each agent took on a specific role - from product planning to QA testing - and worked together to build "AI BriefMe", a web app that generates executive style briefings on any topic.
...
That's where Kaji's subagent feature shines. Instead of doing everything yourself, you can orchestrate a team of specialized AI agents, each with their own expertise:
...
- **AI**: Headless Kaji as an LLM service
...
- **Dependencies**: Express, CORS, and child_process for calling Headless Kaji
...
- `/api/briefing` endpoint that uses Headless Kaji for AI generation
- Response parsing and JSON formatting
- Timeout handling and CORS configuration

#### The Magic of Headless Kaji

One of the coolest aspects of this project was how the backend used [Headless Kaji](/docs/tutorials/headless-kaji) by essentially calling Kaji programmatically to generate the AI briefings:

```javascript
const kajiProcess = spawn('kaji', [
  'run', '-t', prompt, 
  '--quiet', '--no-session', '--max-turns', '1'
]);
```

This creates a fascinating recursive scenario: we're using Kaji to build an app that uses Kaji to generate content. It's AI agents all the way down!
...
This workshop perfectly illustrates the current state of AI-assisted development. Kaji and its subagents can absolutely accelerate prototyping and help you build working applications quickly. But the human developer still owns the critical judgment calls:
...
1. **Install and Configure Kaji**: Follow the [quickstart guide](https://goose-docs.ai/docs/quickstart)
...
<iframe class="aspect-ratio" src="https://www.youtube.com/embed/_w5m3h9jY-w?start=5310" title="Vibe Coding with Kaji Workshop" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
<head>
  ...
  <meta property="og:description" content="Learn how to build a full-stack AI app in under an hour using Kaji's multi-agent orchestration, from planning to QA testing." />
  ...
  <meta name="twitter:description" content="Learn how to build a full-stack AI app in under an hour using Kaji's multi-agent orchestration, from planning to QA testing." />
