...
Kaji recently released support for [MCP-UI](https://mcpui.dev/) which allows MCP servers to suggest and contribute user interface elements back to the agent.
...
Starting from Kaji v1.3.0, you can add MCP-UI as an extension. 

:::tip Add MCP-UI to Kaji
<Tabs groupId="interface">
  <TabItem value="ui" label="kaji Desktop" default>
  [Launch the installer](kaji://extension?type=streamable_http&url=https%3A%2F%2Fmcp-aharvard.netlify.app%2Fmcp&id=mcpuidemo&name=MCP-UI%20Demo&description=Demo%20MCP-UI-enabled%20extension)
  </TabItem>
  <TabItem value="cli" label="kaji CLI">
  Use `kaji configure` to add a `Remote Extension (Streaming HTTP)` extension type with:
