...
description: Why do AI agents forget? Learn how context windows, tokens, and Kaji help you manage memory and long conversations.
authors: 
    - rizel
---
...
### How Kaji uses tokens

Let's talk about how this works in practice. When you use an AI agent like Kaji, you start a session and choose a model like Claude Sonnet 3.7. This model has a context window of 128,000 tokens. This means every session (or conversation) can handle up to 128,000 tokens. If you message "hey" to Kaji, you would have used one token. And when Kaji responds back, you would have used several more tokens. Now you've used a small portion of your 128,000 tokens, and you have the remainder left.
...
But your conversation isn't the only thing using your tokens. Here are other things within Kaji that consume your token budget:

* **System prompt:** A built-in prompt that instructs your agent on how to behave and defines its identity  
  * The system prompt defines Kaji’s name, creator (Block), current date/time, task and extension handling, and response format.  
...
* In addition to your conversation history, Kaji keeps metadata about your conversation, such as timestamps.
...
Luckily, Kaji has an intelligent design for helping you save your context window.

## How Kaji automatically manages your context window

Kaji has a method that auto-compacts (or summarizes) your conversation once it reaches a certain threshold. By default, when you reach 80% of your context window, Kaji summarizes the conversation, preserving key parts while compressing the rest, reducing context window usage so you can stay in your session without starting a new one.

You actually have the ability to customize the threshold. If you think 80% is too little or too much for your workflow, you can set the environment variable `KAJI_AUTO_COMPACT_THRESHOLD` to your preferred threshold.

## How to manage your context window

While Kaji is adept at helping you manage your context window, you can proactively manage it, too. Here are some tips for efficiently managing your context window and your wallet.
...
Use [.kajihints](/docs/guides/context-engineering/using-kajihints/) files to avoid repeating the same instructions. Instead of typing out your project context, coding standards, and preferences in every conversation, define them once in a .kajihints file. This prevents wasting tokens on repetitive explanations and helps Kaji understand your requirements more quickly.
...
<head>
  ...
  <meta property="og:description" content="Why do AI agents forget? Learn how context windows, tokens, and Kaji help you manage memory and long conversations." />
  ...
  <meta name="twitter:description" content="Why do AI agents forget? Learn how context windows, tokens, and Kaji help you manage memory and long conversations." />
