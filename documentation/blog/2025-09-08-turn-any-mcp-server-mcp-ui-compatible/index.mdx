...
- **Prompt Action** → Fires off a prompt to kaji asking it to caption the image like a meme.  
...
```ts
        function shareOnTwitter() {
            ...
            "I didn’t write this tweet… kaji did. (${result.resource_type} included). & here’s how you can do it too 🧵 #MCPUI");
...
> Want to see it live? [Here’s the tweet kaji posted for me](https://x.com/EbonyJLouis/status/1966203455955157337).
...
That’s it. One extra resource, and suddenly kaji renders a full UI.

## Filesystem: Same Pattern

To prove this wasn’t a one-off, I also made the Filesystem MCP server UI-compatible.

**Before:** Text output (what kaji shows by default)
...
And if you zoom out, you’ll see other companies pushing here too. kaji and Postman already support rendering and a couple of UI actions. In kaji right now, a button can fire off a new prompt or open an external link. It’s not the full vision yet, but it’s already enough to start building experiences that feel more like mini-apps than static responses.
...
Download [kaji](/docs/quickstart#install-kaji), give an MCP server a UI facelift of your own, and see the magic for yourself. Boring text prompts will never hit the same again.
