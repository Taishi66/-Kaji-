---
title: "kaji v1.25.0: Sandboxed, Streamlined, and More Secure"
description: "kaji v1.25.0 brings macOS sandboxing, a unified summon extension, rich MCP app UIs, agentic CLI upgrades, and SLSA build provenance."
authors:
  - debbie
image: /img/blog/kaji-v1-25-0.png
---

![Banner image for the kaji v1.25.0 release](/img/blog/kaji-v1-25-0.png)

kaji v1.25.0 is here, and it's one of our most significant releases yet. This version brings macOS sandboxing for enhanced security, a major architectural simplification with the unified summon extension, rich UI rendering for MCP apps, and a wave of improvements to agentic CLI providers. Whether you're running kaji Desktop or the CLI, there's something in this release for you.

Let's break down what's new.

<!--truncate-->

<iframe class="aspect-ratio" src="https://www.youtube.com/embed/9tbYbUkvxW0" title="kaji v1.25.0 release highlights" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

## 🔒 macOS Sandboxing

:::danger Outdated
The macOS seatbelt sandbox described in this section was experimental and has been removed. The `kaji` server process (which executes tools) runs with the same permissions as your user account and is **not** sandboxed at the OS level. For security controls, see [`KAJI_MODE`](/docs/guides/environment-variables#tool-configuration) (`approve`, `smart_approve`) to restrict what tools can run without confirmation.
:::

**The headline feature of v1.25.0 was optional security sandboxing for kaji Desktop on macOS.**

kaji Desktop added an opt-in macOS sandbox (enabled via `KAJI_SANDBOX=true`) powered by [seatbelt](https://github.com/michaelneale/agent-seatbelt-sandbox), the same underlying technology Apple uses to sandbox its own apps.

What it provided at the time:

- **File system restrictions:** kaji could be limited in what directories it reads and writes, preventing it from modifying its own config or accessing sensitive areas outside your project.
- **Network visibility:** You could track and limit what URLs kaji accessed.
- **Zero overhead:** The sandbox used macOS's built-in `sandbox-exec` facility, so there was no performance penalty.
- **Works with any tools:** Because sandboxing happened at the OS level, it applied regardless of which MCP extensions or tools kaji was using.
...
Previously, kaji had two different mechanisms for delegating work: [subagents](https://goose-docs.ai/docs/tutorials/subagents) (for spinning up independent sub-tasks) and Skills (for loading predefined capabilities). They overlapped in confusing ways and made the system harder to understand.

The new **Summon** extension unifies both concepts into two clean tools:

- **`load`**: Load Skills and Recipes into kaji's context, replacing the old Skills system.
...
- Cleaner mental model for how kaji handles delegation
...
**MCP extensions can now render rich, interactive UIs directly inside kaji Desktop.**
...
- **Upgraded rmcp to 0.15.0.** kaji now advertises MCP Apps UI extension capability to servers.
- **Standalone kaji Apps filtering.** The Apps page now filters to show only standalone kaji Apps, making discovery cleaner.

This opens up a whole new class of MCP extensions that can provide dashboards, visualizations, forms, and other interactive experiences right inside your kaji session.

## 📝 Edit Recipe Model & Provider from the GUI

**You can now [edit a recipe's](https://goose-docs.ai/docs/guides/recipes/session-recipes#edit-recipe) model, provider, and extensions directly in kaji Desktop. No YAML editing required.**
...
![Recipe editor in kaji Desktop showing provider and model configuration](recipe-editor.png)

## 🤖 Agentic CLI Providers Level Up

**Claude Code, Codex, and Gemini CLI all received major upgrades in this release.**

kaji's [agentic CLI providers](https://goose-docs.ai/docs/guides/cli-providers), which delegate work to other AI coding agents, got a batch of improvements that make them significantly more capable:
...
Under the hood, kaji converts your extension configurations into the native format each CLI expects:
...
If you've ever seen half a bold tag or a broken code block flash on screen while kaji is streaming a response, this fix is for you. The new `MarkdownBuffer` introduces a state machine parser that tracks open markdown constructs (bold, code blocks, links, etc.) and only flushes content to the terminal when constructs are complete.

The result: smooth, properly formatted markdown that renders progressively without visual glitches.

## 🛡️ SLSA Build Provenance

**Every kaji release artifact now comes with a signed provenance attestation.**
...
This means you can **cryptographically verify** that any kaji artifact was built from the official repository by the official CI pipeline:

```bash
gh attestation verify <artifact> --repo aaif-kaji/kaji
```
...
Ready to try v1.25.0? Head over to our [updating kaji](https://goose-docs.ai/docs/guides/updating-goose) guide for step-by-step instructions on getting the latest version for Desktop or CLI.
...
*kaji is open source. Star us on [GitHub](https://github.com/aaif-goose/goose), and if you build something cool with kaji, we'd love to hear about it!*

<head>
  <meta property="og:title" content="kaji v1.25.0: Sandboxed, Streamlined, and More Secure" />
  <meta property="og:type" content="article" />
  <meta property="og:url" content="https://goose-docs.ai/blog/2026/02/23/goose-v1-25-0" />
  <meta property="og:description" content="kaji v1.25.0 brings macOS sandboxing, a unified summon extension, rich MCP app UIs, agentic CLI upgrades, and SLSA build provenance." />
  ...
  <meta name="twitter:title" content="kaji v1.25.0: Sandboxed, Streamlined, and More Secure" />
  <meta name="twitter:description" content="kaji v1.25.0 brings macOS sandboxing, a unified summon extension, rich MCP app UIs, agentic CLI upgrades, and SLSA build provenance." />
