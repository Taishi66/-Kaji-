---
title: "Orchestrate complex workflows across multiple agents with kaji"
description: "Use kaji to coordinate multi-agent workflows — decompose complex tasks, delegate to specialized agents in parallel, and synthesize the results."
authors:
  - adewale
image: /img/blog/kaji-conductor.png
---

![kaji orchestrating multiple agents like a conductor](/img/blog/kaji-conductor.png)
...
That's what orchestration gives you. Instead of feeding kaji one task at a time and waiting, you can have it coordinate multiple agents working in parallel — each focused on a specific piece of the puzzle, with kaji keeping everything on track.

<!-- truncate -->

## What is orchestration?

If you've used [subagents](/docs/guides/context-engineering/subagents) before, you already know how to delegate work to independent AI instances. Orchestration is the layer above that — the part that decides _who_ does _what_ and _when_. Instead of one kaji session doing everything sequentially, orchestration lets you:

- **Decompose** a complex task into independent subtasks
- **Delegate** each subtask to a separate agent (subagent, ACP provider, or another kaji instance)
...
Here's something you can run right now to see orchestration in action. Ask kaji:
...
kaji will spin up three subagents simultaneously — one for each research task. You'll see them working at the same time in your session, and all three summaries come back roughly together instead of one after another. What would've been three sequential waits becomes one.
...
Not everything can run in parallel. Some workflows have natural phases where later steps depend on earlier results. You can describe this to kaji naturally:
...
kaji understands the dependency structure here. Phase 1 runs two research tasks in parallel. Phase 2 waits for those results before building. Phase 3 kicks off two independent verification tasks in parallel.
...
Here's where things get really interesting. Orchestration works with [ACP providers](/docs/guides/acp-providers), which means you can delegate to entirely different coding agents — not just kaji subagents. 
...
kaji understands that you want different agents handling different parts of the workflow. It delegates each task to the right agent, waits for the results, and brings everything together. This works the same whether you're on the CLI or the desktop app — you just ask in natural language.
...
If you're already using kaji, you can start orchestrating today. The simplest way is just to ask:

```
Do these three things in parallel: [task A], [task B], [task C]
```

kaji handles the rest — spinning up delegates, running them concurrently, and bringing the results together.
...
<head>
  <meta property="og:title" content="Orchestrate Complex Workflows Across Multiple Agents with kaji" />
  <meta property="og:type" content="article" />
  <meta property="og:url" content="https://goose-docs.ai/blog/2026/05/05/goose-as-conductor" />
  <meta property="og:description" content="Use kaji to coordinate multi-agent workflows — decompose complex tasks, delegate to specialized agents in parallel, and synthesize the results." />
  ...
  <meta name="twitter:title" content="Orchestrate Complex Workflows Across Multiple Agents with kaji" />
  <meta name="twitter:description" content="Use kaji to coordinate multi-agent workflows — decompose complex tasks, delegate to specialized agents in parallel, and synthesize the results." />
