...
description: "kaji now supports lifecycle hooks via the Open Plugins spec. Wire shell scripts into PreToolUse, PostToolUse, UserPromptSubmit, SessionStart, and more."
image: /img/blog/kaji-hooks.jpg
authors:
  - alexhancock
---

![Hooks: run your own scripts on every kaji event](/img/blog/kaji-hooks.jpg)

kaji now supports **lifecycle hooks**. Drop a plugin into a directory on disk and kaji will run your shell scripts when things happen during a session: a tool is about to fire, a tool just finished, the user submitted a prompt, the session started, the session ended.
...
kaji follows the [Open Plugins hooks specification](https://open-plugins.com/agent-builders/components/hooks). Any plugin directory under `~/.agents/plugins/<name>/` (user scope) or `<project>/.agents/plugins/<name>/` (project scope) that contains a `hooks/hooks.json` file is auto-discovered at startup.
...
When the event fires, kaji runs the command, sets `PLUGIN_ROOT` in the environment, and pipes a JSON payload to the script on stdin:
...
### 1. Have kaji talk to you when it actually needs you

Pick a handful of events that mean "the human's attention would be useful right now" — a tool failed, the session wrapped, a long-running command finished — and have kaji speak a line when one of them fires:
...
### 2. The "kaji is doing something" desk light 🪿💡

If you have a smart bulb with an HTTP API (Hue, LIFX, Home Assistant, etc.), turn it on when kaji starts a tool call and off when it finishes:
...
Now your desk lamp is a status indicator for the agent. Walk away, glance back, and if it's on, kaji is still working.

### 3. Auto-format every file kaji edits
...
Useful when kaji is working on a long task in another window. The audio cue tells you it's actually doing things instead of sitting there waiting on you.
...
MCP servers give kaji new tools. Hooks go the other direction: they give you a way to react to what kaji is doing, in real time, with whatever language you already know. Bash, Python, a Go binary, a one-line `curl`. It's all just a command on stdin.

The plugin model is small on purpose: a folder, a JSON file, a script. No registration step, no daemon, no rebuild. Drop it in, start kaji, it works. Take it out and kaji doesn't notice it's gone.
