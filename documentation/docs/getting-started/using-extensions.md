...
Extensions are add-ons that provide a way to extend the functionality of kaji by connecting with applications and tools you already use in your workflow. These extensions can be used to add new features, access data and resources, or integrate with other systems.

Extensions are based on the [Model Context Protocol (MCP)](https://github.com/modelcontextprotocol), so you can connect
kaji to a wide ecosystem of capabilities.

kaji automatically checks external extensions for known malware before activation. If a malicious package is detected, the [extension will be blocked](/docs/troubleshooting/known-issues#malicious-package-detected) with a clear error message.

:::tip Tutorials
Check out the [step-by-step tutorials](/docs/category/mcp-servers) for adding and using various kaji extensions
:::

## Built-in Extensions
kaji includes several built-in extensions you can start using out of the box:
...
kaji operates autonomously by default. Combined with the Developer extension's tools, this means kaji can execute commands and modify files without your approval. If you want more control over this behavior, you can configure the [kaji permission mode](/docs/guides/managing-tools/kaji-permissions) and [tool permissions](/docs/guides/managing-tools/tool-permissions). See [Configuring Access Controls](/docs/mcp/developer-mcp#configuring-access-controls) for a brief overview.
:::
...
- [Top of Mind](/docs/mcp/tom-mcp): Inject persistent instructions into kaji's working memory every turn
...
:::info
kaji's built-in extensions are MCP servers in their own right. If you'd like
to use the MCP servers included with kaji with any other agent, you are free to do so.
:::

## Discovering Extensions

kaji provides a [central directory][extensions-directory] of extensions that you can install and use. 

You can also add any other [MCP Server](#mcp-servers) as a kaji extension, even if it's not listed in our directory.
...
You can install any MCP server as a kaji extension. 

Many MCP servers work better when they understand your active workspace. kaji supports [MCP Roots](/docs/guides/mcp-roots), which lets roots-aware extensions see your current session working directory automatically.
...
<Tabs groupId="interface">
  ...
  4. On the `Add custom extension` modal, enter the necessary details
     ...
     - The `Timeout` field lets you set how long kaji should wait for a tool call from this extension to complete
  5. Click `Add` button
  ...
  3. Choose the type of extension you'd like to add:
      - `Built-In Extension`: Use an extension that comes pre-installed with kaji.
...
Extensions can be installed using kaji's deep link protocol. The URL format varies based on the extension type:

<Tabs groupId="interface">
  <TabItem value="stdio" label="StandardIO" default>
```
kaji://extension?cmd=<command>&arg=<argument>&id=<id>&name=<name>&description=<description>
```

Required parameters:
- `cmd`: The base command to run, one of `jbang`, `npx`, `uvx`, `kajid`, or `docker`
...
```
kaji://extension?cmd=npx&arg=-y&arg=%40modelcontextprotocol/server-github&timeout=<timeout>&id=<id>&name=<name>&description=<description>
```
...
```
kaji://extension?url=<remote-streamable-http-url>&type=streamable_http&id=<id>&name=<n>&description=<description>
```
...
```
kaji://extension?url=https%3A%2F%2Fexample.com%2Fstreamable&type=streamable_http&timeout=<timeout>&id=<id>&name=<n>&description=<description>
```
...
### Config Entry
For advanced users, you can also directly edit the config file (`~/.config/kaji/config.yaml`) to add, remove, or update an extension:
...
<Tabs groupId="interface">
  ...
  1. Run the following command to open up kaji's configurations:
      ```sh
      kaji configure
      ```
...
The Smart Extension Recommendation system in kaji automatically identifies and suggests relevant extensions based on your tasks and needs. This section explains how to use this feature effectively and understand its capabilities and limitations.

When you request a task, kaji checks its enabled extensions and their tools to determine if it can fulfill the request. If not, it suggests or enables additional extensions as needed. You can also request specific extensions by name.
...
kaji automatically detects when an extension is needed based on your task requirements. Here's an example of how kaji identifies and enables a needed extension during a conversation:
...
■  kaji would like to enable the following extension, do you approve?
// highlight-start
...
kaji responds to explicit requests for extensions, allowing users to manually enable specific tools they need. Here's an example of how kaji handles a direct request to enable an extension:
...
kaji relies on extension properties to determine how to handle an extension. You can edit these properties if you want to change the extension's display settings and behavior, such as the name, timeout, or environment variables.
...
You can start a tailored kaji session with specific extensions directly from the CLI. 
...
```bash
kaji session --with-builtin "{extension_id}"
```

For example, to enable the Developer and Computer Controller extensions and start a session, you'd run:

```bash
kaji session --with-builtin "developer,computercontroller"
```

Or alternatively:

```bash
kaji session --with-builtin developer --with-builtin computercontroller
```
...
```bash
kaji session --with-extension "{extension command}" --with-extension "{another extension command}"
```
...
```bash
kaji session --with-extension "uvx mcp-server-fetch"
```
...
```bash
kaji session --with-extension "VAR=value command arg1 arg2"
```

For example, to start a session with the [GitHub extension](https://github.com/github/github-mcp-server), you'd run:

```bash
kaji session --with-extension "GITHUB_PERSONAL_ACCESS_TOKEN=<YOUR_TOKEN> npx -y @modelcontextprotocol/server-github"
```
...
```bash
kaji session --with-streamable-http-extension "{extension URL}" --with-streamable-http-extension "{another extension URL}"
```

For example, to start a session with a Streamable HTTP extension, you'd run:

```bash
kaji session --with-streamable-http-extension "https://example.com/streamable"
```

### Extensions in Containers

kaji can use the `--container` flag to run extensions inside your Docker containers for devcontainer workflows. See [Running Extensions in Docker Containers](/docs/tutorials/kaji-in-docker#running-extensions-in-docker-containers) for details.

## Developing Extensions

kaji extensions are implemented with MCP, a standard protocol that allows AI models and agents to securely connect with local or remote resources. Learn how to build your own [extension as an MCP server](https://modelcontextprotocol.io/quickstart/server).
