---
sidebar_position: 90
title: kaji Extension Allowlist
sidebar_label: Extension Allowlist
---

kaji is an extensible framework that, by default, allows you to install any MCP server. However, you may want stricter controls on which MCP servers can be installed as extensions (e.g. in a corporate setting). 

This guide explains how you can create an **allowlist** of safe extensions that work with kaji Desktop and CLI. An allowlist lets administrators control which MCP servers can be installed as kaji extensions. When enabled, kaji will only install extensions that are on the list, and will block installation of any others.

## How It Works

1. The allowlist is a YAML file that contains a list of allowed extension commands.
2. kaji fetches the allowlist from a URL specified by the `KAJI_ALLOWLIST` environment variable.
3. The allowlist is fetched when first needed and is cached. It is refetched on every restart of kaji.
4. When a user attempts to install an extension, kaji checks the MCP server's installation command against the allowlist.
...
Create an environment variable called `KAJI_ALLOWLIST` and set the value to the URL of your YAML file:

```bash
export KAJI_ALLOWLIST=https://example.com/goose-allowlist.yaml
```
...
1. Check if the `KAJI_ALLOWLIST` environment variable is set correctly.
