...
Persistent instructions let you inject text into kaji's working memory every turn. Unlike [`.kajihints`](/docs/guides/context-engineering/using-kajihints), which are loaded at session start and can expand later when kaji discovers nested hint files, persistent instructions are re-read and injected fresh with every interaction. This makes them ideal for behavioral guardrails that must always be enforced, regardless of how the conversation evolves.

## How It Works

kaji has a component called MOIM (Model-Observed Internal Memory) that provides contextual information to the model every turn. This includes things like the current timestamp, working directory, and your todo list. Persistent instructions are injected into this same context, placing your reminders in the model's immediate attention window.
...
|----------|---------|---------|
| [`KAJI_MOIM_MESSAGE_TEXT`](/docs/guides/environment-variables#session-management) | Literal text injected into working memory each turn | Not set |
| [`KAJI_MOIM_MESSAGE_FILE`](/docs/guides/environment-variables#session-management) | Path to a file whose contents are injected each turn. Supports `~/` | Not set |
...
For short, single-purpose reminders, use `KAJI_MOIM_MESSAGE_TEXT`:
...
```bash
export KAJI_MOIM_MESSAGE_TEXT="CRITICAL: This is a production environment. Be extra careful."
export KAJI_MOIM_MESSAGE_FILE="~/.kaji/guardrails.md"
```
...
```bash
export KAJI_MOIM_MESSAGE_TEXT="SECURITY: Do not upload code to external services, create public gists, or share sensitive data. All code in this repository is confidential."
```
...
export KAJI_MOIM_MESSAGE_TEXT="⚠️ PRODUCTION: Double-check all commands. Prefer read-only operations. Always create backups before modifications."

# Development environment  
export KAJI_MOIM_MESSAGE_TEXT="Development environment. Feel free to experiment, but run tests before committing."
```
...
```bash
export KAJI_MOIM_MESSAGE_TEXT="This project uses pnpm, not npm. Always use 'pnpm' for package management commands."
```
...
export KAJI_MOIM_MESSAGE_TEXT="Current focus: Refactoring the authentication module. Don't get sidetracked."

# Clear it when done
unset KAJI_MOIM_MESSAGE_TEXT
```

## Persistent Instructions vs kajihints

| Feature | Persistent Instructions | [kajihints](/docs/guides/context-engineering/using-kajihints) |
|---------|------------------------|-------------|
...
You can use both together: kajihints for project context and persistent instructions for critical guardrails.
...
4. **Use files for complex rules**: If you have multiple guidelines, organize them in a file rather than cramming everything into `KAJI_MOIM_MESSAGE_TEXT`.

5. **Test your guardrails**: After setting up persistent instructions, test that kaji respects them by asking it to do something that should be blocked.
