...
Tool permissions provide fine-grained control over how kaji uses different tools within extensions. This guide will help you understand and configure these permissions effectively.

## Understanding Tools and Extensions

Before diving into permissions, let's clarify the key components:

- **Extensions** are packages that add functionality to kaji (like Developer, Google Drive, etc.)
- **Tools** are specific functions within each extension that kaji can use
...
kaji performs best with fewer than 25 total tools enabled across all extensions. Consider enabling only the extensions you need for your current task.
:::

## Permission Levels

Tool permissions work alongside [kaji permission modes](/docs/guides/managing-tools/kaji-permissions). The mode sets the default behavior, while tool permissions let you override the behavior of specific tools.
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="kaji CLI">
    ...
    2. Select `kaji settings` from the menu
    ```sh
    ┌ kaji-configure
    │
...
3. **Task Focus**
   ...
   - Help kaji make better tool choices
