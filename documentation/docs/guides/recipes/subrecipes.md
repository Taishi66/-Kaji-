...
When the main recipe is run, kaji generates a tool for each subrecipe that:
...
**Usage:**
```bash
kaji run --recipe code-review-pipeline.yaml --params repository_path=/path/to/repo
```
...
**Usage:**
```bash
kaji run --recipe smart-analyzer.yaml --params repository_path=/path/to/project
```
...
**Usage:**
```bash
kaji run --recipe travel-planner.yaml
```
