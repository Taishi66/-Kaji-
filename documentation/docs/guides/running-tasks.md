...
When working with the kaji CLI, you can pass files and instructions to the `kaji run` command to execute tasks and workflows. This could be a simple one-liner command or a complex set of instructions stored in a file.

## Basic Usage

The `kaji run` command starts a new session, begins executing using any arguments provided and exits the session automatically once the task is complete. 

There are multiple ways to run tasks with kaji; check out the [list of options](/docs/guides/kaji-cli-commands.md#run-options).

### Text in the command
```bash
kaji run -t "your instructions here"
```

Using the `-t` flag, one is able to pass a text instruction directly to the command. This is great for quick, one-off commands where you do not need an interactive session with kaji. The instructions will be executed, and the session will end. An example usage could be using in a CI/CD pipeline or running alongside other scripts.

### Using an instruction file
If you have a complex set of instructions or a workflow that you want to automate, you can store them in a file and pass it to the `kaji run` command:

```bash
kaji run -i instructions.md
```
...
### With stdin
You can also pass instructions to kaji using standard input via `-i -`. This is useful when you want to pipe commands from another tool or script into kaji.

#### Simple echo pipe

```bash
echo "What is 2+2?" | kaji run -i -
```
...
If you don't want kaji to exit at the end of the task, you can pass the `-s` or `--interactive` flag to start an interactive session after processing your initial commands:

```bash
kaji run -i instructions.txt -s
```

This is useful when you want to continue working with kaji after your initial commands are processed.
...
kaji run -n my-project -t "initial instructions"

# Resume a previous session
kaji run -n my-project -r
```
...
kaji run --no-session -t "your command here"
```
### Set Provider and Model
You can run kaji sessions with a specific provider and model, which overrides the provider and model settings in your [environment variables](/docs/guides/environment-variables.md).
...
```bash
kaji run --with-builtin "developer,computercontroller" -t "your instructions"
```

- Using custom extensions

```bash
kaji run --with-extension "ENV1=value1 custom-extension-args" -t "your instructions"
```

- Using streamable HTTP extensions

```bash
kaji run --with-streamable-http-extension "https://example.com/streamable" -t "your instructions"
```
...
kaji run --debug -t "your instructions"

# Debug a recipe execution
kaji run --debug --recipe recipe.yaml
```

### JSON Output Format

For automation, scripting, and CI/CD integration, you can get structured output from `kaji run` using the `--output-format` flag:
...
kaji run --output-format json -t "your instructions"

# Stream JSON events in real-time
kaji run --output-format stream-json -t "your instructions"

# Run a recipe with JSON output
kaji run --output-format json --recipe recipe.yaml

# Combine with other options
kaji run --output-format json --no-session -t "automated task"
```
...
```bash
kaji run -t "Create a CHANGELOG.md entry comparing current git branch with main"
```
...
```bash
kaji run --with-builtin "developer,git" -n dev-session -s
```
