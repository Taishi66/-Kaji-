...
This API specification defines the API that kaji uses for ML-based [prompt injection detection](/docs/guides/security/prompt-injection-detection).
...
**kaji's Usage:**
- kaji looks for the label with the highest score
- If the top label is `"INJECTION"` (or `"LABEL_1"`), the score is used as the injection confidence
- If the top label is `"SAFE"` (or `"LABEL_0"`), kaji uses `1.0 - score` as the injection confidence
