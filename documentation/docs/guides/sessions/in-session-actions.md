...
kaji provides features you can use to manage conversations and share information during sessions.
...
- kaji misunderstood your intent and went in the wrong direction
...
<Tabs groupId="interface">
    <TabItem value="ui" label="kaji Desktop" default>
        ...
        4. Click `Edit in Place` to save your changes and reprompt kaji

        kaji removes all conversation history after the edited message and responds contextually from that point.

        :::warning Deleted Context
        With Edit in Place, subsequent conversation history is permanently deleted from the session and removed from kaji's context. Use this option only if you don't need kaji to remember the context that follows the edited message.
        :::

    </TabItem>
    <TabItem value="cli" label="kaji CLI">
        Use the `--edit` flag with `kaji session` to open the session's conversation in your editor as YAML:

        ```bash
        kaji session --resume <session-id> --edit
        ```

        This opens `$VISUAL` / `$EDITOR` / `vi` with the conversation serialized as YAML. After editing and saving, kaji continues the session from the edited conversation. Any later messages you leave in the YAML remain in the session and in kaji's context; remove messages from the YAML if you want kaji to forget them.
...
- Test how different prompts affect kaji's responses
...
<Tabs groupId="interface">
    <TabItem value="ui" label="kaji Desktop" default>
        ...
        kaji creates a new session with conversation history up to and including your edited message. The new session is named "(edited)" and the original session remains unchanged.
    ...
    <TabItem value="cli" label="kaji CLI">
        ...
        ```bash
        kaji session --resume <session-id> --fork --edit
        ```

        This opens `$VISUAL` / `$EDITOR` / `vi` with the conversation serialized as YAML. After editing and saving, kaji creates a new session with the edited conversation and resumes from there. The original session remains unchanged.
...
- **Iterative Prompt Refinement**: Start with a basic prompt, then edit and refine based on kaji's response. This often works better than trying to craft the perfect prompt from the start.
...
Queue messages while kaji is processing a task to manage your workflow. This is useful when:

- You want to prepare next steps while kaji is working
...
:::tip
kaji may perform better when complex tasks are split into subtasks, a technique called [*prompt chaining*](https://www.promptingguide.ai/techniques/prompt_chaining). This structured approach can both improve accuracy and give you more control over the process.
:::

<Tabs groupId="interface">
    <TabItem value="ui" label="kaji Desktop" default>
      Add a message to the queue:
      1. While kaji is processing a response, type your next message
      2. Press `Enter` to add it to the queue (or interrupts if using [interruption keywords](#interrupt-task))
      
      Queued messages appear as numbered cards showing the queue order. The first message in the queue is automatically sent when kaji finishes each response.
      
      :::info Related Features
      - In general, pressing `Enter` while kaji is processing a task queues the message, but clicking `Send` sends the task immediately and [interrupts the task](#interrupt-task)
      - When you type common interrupt keywords like "stop", "wait", or "hold on" in a queued message, kaji pauses until you enter or send the next message and then continues processing the queue
      :::

      #### Queue Management Controls
    
      Queued messages run automatically in order as kaji finishes each task, but you can manage the queue:
      ...
      **Without queuing:**
      ...
      This approach might lead to overwhelming responses where important details get missed or tasks are handled superficially. Even sending a single prompt with clear sequential steps doesn't allow kaji to focus on each task individually or build context progressively.

      **With queuing:**

      1. You send: "Refactor the authentication code to support OAuth 2.0"
      2. While kaji is working, you queue the following messages:
    ...
    <TabItem value="cli" label="kaji CLI">
        Message queuing is not available in the kaji CLI.
    </TabItem>
</Tabs>

## Interrupt Task

Interrupt kaji while it's processing a task to take control of the conversation. This is useful when:

- kaji is heading in the wrong direction
...
<Tabs groupId="interface">
    <TabItem value="ui" label="kaji Desktop" default>
        ...
        kaji stops processing the current task and asks for more information.
        ...
        kaji stops processing the current task and pivots to the new request context.
        
        :::info Related features
        - Clicking `Send` while kaji is processing a task interrupts the task but pressing `Enter` [queues the message](#queue-messages)
        - Typing a stop or pause keyword in a queued message also stops kaji from processing the current task
    ...
    <TabItem value="cli" label="kaji CLI">
        ...
        kaji responds contextually to your new request.
        
    </TabItem>
</Tabs>

## Voice Dictation
Speak to kaji directly instead of typing your prompts.
...
<Tabs groupId="interface">
    ...
    To use voice dictation:
        ...
        The first time you use voice dictation, kaji will request access to your microphone. While recording, you'll see `Listening` and `Transcribing` status indicators. kaji transcribes your speech during natural pauses and adds the text to the chat box. 

        **If you don't see the microphone**, check the [models you have configured](/docs/getting-started/providers.md). For example, using OpenAI as the voice dictation provider requires that you have an OpenAI model configured in kaji, even if using another LLM provider for chat.
    ...
    <TabItem value="cli" label="kaji CLI">
        Voice dictation is not available in the kaji CLI. 
    </TabItem>
</Tabs>

## Spellcheck

Spellcheck is enabled by default in the kaji Desktop chat input field.
...
<Tabs groupId="interface">
    ...
    To disable or re-enable spellcheck:
        ...
        5. Restart kaji for the change to take effect
        
    </TabItem>
    <TabItem value="cli" label="kaji CLI">
        Spellcheck is not available in the kaji CLI.
    </TabItem>
</Tabs>

## Share Files in Session

Provide kaji with context from your codebase, documents, and other files to get more relevant and accurate assistance.

<Tabs groupId="interface">
    <TabItem value="ui" label="kaji Desktop" default>
        Share files with kaji in several ways:
        ...
        4. **Quick File Search**: Use the [`@` shortcut key](/docs/guides/file-management#quick-file-search-in-kaji-desktop) to quickly find and include files
    </TabItem>
    <TabItem value="cli" label="kaji CLI">
...
You can change some settings during a session and they will take effect immediately, rather than requiring you to start a new session. This gives you more control over context and capabilities while you're interacting with kaji.
...
<Tabs groupId="interface">
  ...
  | [**kaji Mode**](/docs/guides/managing-tools/kaji-permissions#configuring-kaji-mode) | <Tornado className="inline" size={16} /> mode switcher | New sessions |
  ...
  | [**kaji Mode**](/docs/guides/managing-tools/kaji-permissions#configuring-kaji-mode) | `/mode [options]` | New sessions |

  :::info
  The CLI supports [additional slash commands](/docs/guides/kaji-cli-commands#slash-commands) but doesn't support mid-session changes to the working directory or model.
  :::
