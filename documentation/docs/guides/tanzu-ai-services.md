...
description: Connect kaji to VMware Tanzu Platform AI Services
---
...
[VMware Tanzu Platform](https://techdocs.broadcom.com/us/en/vmware-tanzu/platform/ai-services/10-3/ai/index.html) provides enterprise-managed LLM access through AI Services. kaji connects to VMware Tanzu Platform as an OpenAI-compatible provider, supporting both **single-model** and **multi-model** service plans with streaming enabled by default.
...
```sh
cf create-service-key my-qwen-coder my-kaji-key --wait
```

Then retrieve the credentials:

```sh
cf service-key my-qwen-coder my-kaji-key
```
...
Single-model plans include a top-level `credentials.api_base` field that has an `/openai` suffix. **Do not use this value.** Always use `credentials.endpoint.api_base` (without `/openai`), because kaji automatically appends the correct path.

Using the wrong value would produce a double-path URL like `.../openai/openai/v1/chat/completions`.
:::

## Step 5: Configure kaji
...
<Tabs groupId="interface">
  ...
  Set the following environment variables before launching kaji:
...
kaji dynamically fetches available models from your Tanzu endpoint. After configuring the provider:
...
To change models later, use **Settings** > **Models** > **Switch models** in Desktop, or run `kaji configure` in the CLI.
...
1. **Environment variables not set**: If using kaji Desktop, env vars from your shell may not be inherited. Use the Settings UI to configure the provider instead.
...
```sh
cf delete-service-key my-qwen-coder my-kaji-key -f
