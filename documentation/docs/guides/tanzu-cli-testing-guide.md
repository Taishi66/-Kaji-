...
- kaji CLI built from the `feat/tanzu-ai-provider` branch
...
export KAJI_CLI=~/claude/kaji-fork/target/release/kaji
...
export KAJI_CLI=/usr/bin/kaji

# If built from source:
export KAJI_CLI=~/kaji-fork/target/release/kaji
...
**Expected:** Session starts with whichever model was selected during `kaji configure`.
...
2. Run `kaji configure` again
3. Change TANZU_AI_ENDPOINT to the single-model endpoint
4. Select the single model
5. Start a new session, verify it works

**Expected:** Both plans work without needing to restart kaji.

## Quick Curl Verification

Before testing with kaji, you can verify endpoints directly:
