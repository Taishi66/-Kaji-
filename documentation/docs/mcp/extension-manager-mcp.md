...
description: Dynamically discover, enable, and disable extensions during kaji sessions
---
...
You don't always need to manually manage extensions. The Extension Manager extension enables kaji to dynamically discover, enable, and disable extensions during active sessions. Based on the task you give it, kaji recognizes when it needs a specific extension, enables it when required, and suggests disabling unused extensions if the bloat is eating up your context window.

Simply describe your task, and kaji will handle the extension management automatically.
...
kaji can work with many extensions, but having too many enabled at once can:
...
The Extension Manager solves this by letting kaji:
...
This creates a more focused, efficient experience where kaji has access to exactly the tools it needs, when it needs them.
