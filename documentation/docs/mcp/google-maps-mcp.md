...
description: Add Google Maps MCP Server as a kaji Extension
unlisted: true
---
...
This tutorial covers how to add the [Google Maps MCP Server](https://www.pulsemcp.com/servers/google-maps-docs) as a kaji extension to enable geocoding, place searching, distance calculations, elevation data retrieval, and directions.

:::tip Quick Install
<Tabs groupId="interface">
  <TabItem value="ui" label="kaji Desktop" default>
  [Launch the installer](kaji://extension?cmd=npx&arg=-y&arg=%40modelcontextprotocol%2Fserver-google-maps&id=google-maps&name=Google%20Maps&description=Google%20Maps%20API%20integration&env=GOOGLE_MAPS_API_KEY%3DGoogle%20Maps%20API%20key)
  </TabItem>
...
<Tabs groupId="interface" defaultValue="ui">
  <TabItem value="ui" label="kaji Desktop">
...
kaji acts as an autonomous agent that tracks a delivery driver’s location, updates the customer in real-time, and adjusts ETAs dynamically based on Google Maps traffic data.

kaji can:

**Monitor Driver Location**
* kaji gets the driver’s GPS coordinates every few seconds.
* If the driver’s location is significantly behind schedule, kaji recalculates the ETA.

**Traffic-Aware ETA Adjustments**
* kaji queries Google Maps for live traffic conditions.
* If delays are detected, kaji updates the customer’s tracker with a new estimated time.

**Dynamic Customer Notifications**
* If the driver is stuck in traffic, kaji sends an alert to the customer: “Your driver is running late due to traffic. Updated ETA: 7:25 PM.”
* If the driver is ahead of schedule, kaji notifies the restaurant to prepare the food earlier.
