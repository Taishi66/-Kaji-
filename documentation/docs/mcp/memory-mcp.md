...
description: Use Memory MCP Server as a kaji Extension
---
...
The Memory extension turns kaji into a knowledgeable assistant by allowing you to teach it personalized key information (e.g. commands, code snippets, preferences and configurations) that it can recall and apply later. Whether it’s project-specific (local) or universal (global) knowledge, kaji learns and remembers what matters most to you.

This tutorial covers enabling and using the Memory MCP Server, which is a built-in kaji extension.  
...
|-------|------|-------------|
| Local (project) | `.kaji/memory/` in your working directory | Project-specific preferences and configs |
| Global (user) | `~/.config/kaji/memory/` | Preferences that apply across all projects |

kaji loads all saved memories at the start of a session and includes them in every prompt sent to the LLM.
...
## Why Use Memory?  
With the Memory extension, you’re not just storing static notes, you’re teaching kaji how to assist you better. Imagine telling kaji:  
...
kaji will recall everything you’ve saved as long as you instruct it to remember. This makes it easier to have consistent results when working with kaji.

For large or detailed instructions, store them in files and instruct kaji to reference those files:
...
kaji also recognizes certain trigger words that signal when to store, retrieve, or remove memory.
...
In this example, I’ll show you how to make kaji a knowledgeable development assistant by teaching it about your project’s API standards. With the Memory extension, kaji can store structured information and recall it when needed, helping you stay consistent in your work.

This means you no longer have to repeat yourself. kaji will remember your project’s requirements and automatically apply them to new tasks.

:::tip  
If you frequently work with API standards or other structured knowledge, kaji may automatically suggest saving them to memory for you when it detects a pattern in your workflow.  
:::
...
### Step 1: Teach kaji Your API Standards
    #### kaji Prompt #1
    ```
    kaji, learn our API development standards:
