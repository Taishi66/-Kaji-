...
description: Add the Square API as a kaji Extension
---
...
This tutorial will get you started with the [Square MCP server](https://developer.squareup.com/docs/mcp) to enable interactive and automated work for your Square seller account. The Square MCP server is an open source project that allows you to interact with the Square API using kaji.
...
<Tabs groupId="remote-or-local">
  ...
  <Tabs groupId="interface">
    <TabItem value="ui" label="kaji Desktop" default>
     ...
     3. kaji should open a browser tab to an OAuth permissions page. Double-check which permissions you want to allow, and click `Grant Access`.
     4. It will ask you to login or reauthenticate to Square, and may ask you to confirm the permissions you want to allow.
     5. In kaji, navigate to the chat
    ...
    <TabItem value="cli" label="kaji CLI">
      ...
      When you start the next session, kaji will open a browser where you can grant permissions and sign in to your Square account.
  ...
  <Tabs groupId="interface">
    <TabItem value="ui" label="kaji Desktop" default>
    [Launch the installer](kaji://extension?cmd=npx&arg=square-mcp-server&arg=start&id=square-mcp&name=Square%20MCP%20Server&description=Square%20API%20MCP%20Server&env=ACCESS_TOKEN%3DYour%20Access%20Token&env=SANDBOX%3Dtrue)
