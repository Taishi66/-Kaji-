...
kaji is an extensible open source AI agent that enhances your software development by automating coding tasks. 
  ...
  <TabItem value="mac" label="macOS" default>
    ...
    <Tabs groupId="interface">
      ...
      <TabItem value="cli" label="kaji CLI">
        Run the following command to install kaji:
  ...
  <TabItem value="windows" label="Windows">
    ...
    <Tabs groupId="interface">
      ...
      <TabItem value="cli" label="kaji CLI">
        ...
        Run the following command in **Git Bash**, **MSYS2**, or **PowerShell** to install the kaji CLI natively on Windows:
        ...
        If you see a PATH warning after installation, you'll need to add kaji to your PATH before running `kaji configure`. See the [Windows CLI installation instructions](/docs/getting-started/installation) for detailed steps.
...
kaji works with [supported LLM providers](/docs/getting-started/providers) that give kaji the AI intelligence it needs to understand your requests. On first use, you'll be prompted to configure your preferred provider.
...
<Tabs groupId="interface">
  ...
  kaji will open a browser for you to authenticate with Tetrate, or create a new account if you don't have one already. When you return to the kaji desktop app, you're ready to begin your first session.
...
## Start Session
Sessions are single, continuous conversations between you and kaji. Let's start one.

<Tabs groupId="interface">
    <TabItem value="ui" label="kaji Desktop" default>
        After choosing an LLM provider, click the `Home` button in the sidebar.

        Type your questions, tasks, or instructions directly into the input field, and kaji will immediately get to work.
    </TabItem>
    <TabItem value="cli" label="kaji CLI">
        1. Make an empty directory (e.g. `kaji-demo`) and navigate to that directory from the terminal.
...
From the prompt, you can interact with kaji by typing your instructions exactly as you would speak to a developer.

Let's ask kaji to make a tic-tac-toe game!

```
create an interactive browser-based tic-tac-toe game in javascript where a player competes against a bot
```

kaji will create a plan and then get right to work on implementing it. Once done, your directory should contain a JavaScript file as well as an HTML page for playing.


## Enable an Extension

While you're able to manually navigate to your working directory and open the HTML file in a browser, wouldn't it be better if kaji did that for you? Let's give kaji the ability to open a web browser by enabling the [`Computer Controller` extension](/docs/mcp/computer-controller-mcp).
...
<Tabs groupId="interface">
    ...
    <TabItem value="ui" label="kaji Desktop" default>
        ...
        5. Now that kaji has browser capabilities, let's ask it to launch your game in a browser:
    </TabItem>
    <TabItem value="cli" label="kaji CLI">
        ...
        ```
        4. Now that kaji has browser capabilities, let's resume your last session:
        ```sh
         kaji session -r
        ```
        5. Ask kaji to launch your game in a browser:
    </TabItem>
</Tabs>
...
## Next Steps
Congrats, you've successfully used kaji to develop a web app! 🎉

Here are some ideas for next steps:
* Continue your session with kaji and improve your game (styling, functionality, etc).
* Browse other available [extensions](/extensions) and install more to enhance kaji's functionality even further.
* Provide kaji with a [set of hints](/docs/guides/context-engineering/using-kajihints) to use within your sessions.
* See how you can set up [access controls](/docs/mcp/developer-mcp#configuring-access-controls) if you don't want kaji to work autonomously.
