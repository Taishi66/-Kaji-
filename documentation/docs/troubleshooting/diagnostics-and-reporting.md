...
description: Use built-in diagnostics, report bugs, and request new features with kaji's integrated support tools.
---
...
kaji provides several built-in features to help you get support, report issues, and request new functionality. This guide covers the diagnostics system, bug reporting, and feature request tools.
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="kaji CLI">
    Use the session diagnostics command to generate a troubleshooting bundle. For complete details and all available options, see the [CLI Commands guide](/docs/guides/kaji-cli-commands#session-diagnostics-options).

    ```sh
    # Generate diagnostics for a specific session
    kaji session diagnostics --session-id <session_id>

    # Interactive selection (prompts you to choose a session)
    kaji session diagnostics

    # Save to a custom location
    kaji session diagnostics --session-id <session_id> --output /path/to/diagnostics.json
    ```
...
The feature request system helps you suggest improvements and new functionality for kaji.
...
## Error Recovery with "Ask kaji"

When certain types of error occur in kaji Desktop (such as failures to activate extensions), you'll see an `Ask kaji` button in the error notification. This feature lets you quickly troubleshoot the issue with kaji's help:

1. When the error occurs, an `Ask kaji` button appears in the error notification
2. Click the button to send the error details to kaji in a chat prompt
3. kaji provides diagnostic suggestions and potential solutions
