...
description: Create interactive UI applications that render inside kaji Desktop
---

import { PanelLeft } from 'lucide-react';

# Building MCP Apps for kaji

MCP Apps let MCP servers return interactive UIs that render directly inside the kaji chat interface, rather than responding with text alone. This allows users to express intent through interaction, which is useful for workflows that require input, iteration, or visual feedback.

:::warning Experimental
MCP Apps support in kaji is experimental and based on a draft specification. The implementation is minimal and may change, and does not yet support advanced capabilities or persistent app windows.
:::
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === "show_demo_app") {
    return {
      ...
      // This metadata tells kaji to render the MCP App
      _meta: {
        ui: {
...
<body class="light">
  <div class="container">
    <h1>🎮 MCP App Demo</h1>
    <p class="subtitle">An interactive UI running inside kaji</p>
    ...
    <div class="info-section">
      ...
      It communicates with kaji via JSON-RPC messages through the sandbox bridge.
      <br><br>
      • Counter uses local state<br>
      • "Send" calls <code>ui/message</code> to append text to chat<br>
      • Theme syncs with kaji's theme setting
    </div>
  </div>
...
1. Restart kaji to load the new extension
2. Prompt kaji: "Show me the demo app"
3. kaji will call the `show_demo_app` tool
4. Your interactive app will render in the chat!

Try:
- Clicking the counter buttons
- Typing a message and clicking "Send"
- Switching kaji between light/dark mode
...
└──────────────────┬───────────────────┘
                   │ postMessage
┌──────────────────▼───────────────────┐
│          kaji Desktop               │  Renders UI, routes messages
└──────────────────┬───────────────────┘
                   │ MCP Protocol
┌──────────────────▼───────────────────┐
...
Your server returns a `ui://` resource URI, kaji fetches the HTML and renders it in an iframe. The app communicates back via `postMessage`—requesting theme info, sending messages to chat, or resizing itself.
