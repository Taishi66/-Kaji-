...
description: Create your own custom MCP Server to use as a kaji extension
---
...
# Building Custom Extensions with kaji

kaji allows you to extend its functionality by creating your own custom extensions, which are built as MCP servers. These extensions are compatible with kaji because it adheres to the [Model Context Protocol (MCP)][mcp-docs]. MCP is an open protocol that standardizes how applications provide context to LLMs. It enables a consistent way to connect LLMs to various data sources and tools, making it ideal for extending functionality in a structured and interoperable way. 

In this guide, we build an MCP server using the [Python SDK for MCP][mcp-python]. We’ll demonstrate how to create an MCP server that reads Wikipedia articles and converts them to Markdown, integrate it as an extension in kaji. You can follow a similar process to develop your own custom extensions for kaji.
...
:::info
kaji supports Tools, Resources, and Prompts from the [Model Context Protocol](https://modelcontextprotocol.io/). See [`mcp_client.rs`](https://github.com/aaif-goose/goose/blob/main/crates/goose/src/agents/mcp_client.rs) for the supported protocol version and client capabilities.
:::
...
## Step 5: Integrate with kaji

To add your MCP server as an extension in kaji:
...
2. Open kaji Desktop and click the <PanelLeft className="inline" size={16} /> button in the top-left to open the sidebar
...
To see any changes you make to your MCP server code after integrating with kaji, re-run `uv pip install .` and then restart kaji Desktop.
:::
...
## Step 6: Use Your Extension in kaji

Once integrated, you can start using your extension in kaji. Open the kaji chat interface and call your tool as needed.

You can verify that kaji has picked up the tools from your custom extension by asking it "what tools do you have?"

![kaji Chat - Ask about tools](../assets/guides/custom-extension-tools.png)

Then, you can try asking questions that require using the extension you added.

![kaji Chat - Use custom extension](../assets/guides/custom-extension-chat.png)

🎉 **Congratulations!** You’ve successfully built and integrated a custom MCP server with kaji.

---

## Advanced Features for MCP Extensions

kaji supports advanced MCP features that can enhance your extensions.

### MCP Sampling: AI-Powered Tools

**[MCP Sampling](/docs/guides/mcp-sampling)** allows your MCP servers to request AI completions from kaji's LLM, transforming simple tools into intelligent agents.
...
- [kaji's implementation](https://github.com/aaif-goose/goose/blob/main/crates/goose/src/agents/mcp_client.rs) currently supports text and image content types
- kaji automatically advertises sampling capability to all MCP servers
...
- Components render securely in isolated sandboxes within kaji Desktop
