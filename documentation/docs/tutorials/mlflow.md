---
description: Integrate kaji with MLflow to observe and evaluate agent performance
---

# Observability with MLflow

This tutorial covers how to integrate kaji with MLflow to trace your kaji sessions and understand how the agent is performing.
...
## Configure kaji to export OTLP to MLflow

kaji exports OpenTelemetry data over OTLP/HTTP. Point the exporter to MLflow's OTLP endpoint:
...
```bash
pip install mlflow
mlflow experiments create --experiment-name "kaji-traces"
...
## Run kaji with MLflow enabled

Start kaji normally. With the OTLP environment variables set, kaji will automatically export traces to MLflow:

```bash
kaji session
```

Open the MLflow UI at `http://localhost:5000` and navigate to the **Traces** tab to see detailed traces of your kaji session, including LLM calls, tool executions, and token usage.

![kaji trace in MLflow](../assets/guides/mlflow-kaji-tracing.png)
...
- [MLflow kaji integration guide](https://mlflow.org/docs/latest/genai/tracing/integrations/listing/goose.html)
