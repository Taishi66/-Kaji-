---
description: Learn how to create and use kaji recipes with this comprehensive tutorial covering prompts, parameters, and MCP servers
---

# Recipes

kaji recipes are files that contain all the details to allow kaji to do one specific task. Since they are contained in just one file, they are easy to share through all the normal ways we share files, including version management systems like git. Let's get started with the simplest recipe possible.
...
```sh
kaji run --recipe trip.yaml
```

## Extensions

kaji recipes have a section where you can specify which [extensions](/docs/guides/recipes/recipe-reference#extensions) kaji can use during execution. kaji will only use the ones you specify. 

Let's say we want to make sure we have good weather during our Europe trip. We can just add a weather extension (this example uses the [weather-mcp-server](https://github.com/TuanKiri/weather-mcp-server) by TuanKiri under the MIT License) to our recipe, modify the prompt a bit and now kaji will check the weather before adding a city to our trip.
...
- `kaji_provider`: The AI provider (e.g., "anthropic", "openai")
- `kaji_model`: The specific model name
- `temperature`: Controls creativity/randomness (0.0-1.0, higher = more creative)

These settings will override your default kaji configuration when this recipe runs.
