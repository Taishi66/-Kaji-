...
description: Add Spraay x402 MCP Server as a kaji Extension for batch crypto payments, onchain data, and AI model access
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

This tutorial covers how to add the [Spraay x402 MCP Server](https://github.com/plagtech/spraay-x402-mcp) as a kaji extension for batch cryptocurrency payments, onchain data, and AI model access on Base.

With the Spraay extension, kaji can send ETH and ERC-20 tokens to up to **200 recipients in a single transaction**, query live token prices, check wallet balances, resolve ENS names, get swap quotes, and chat with 200+ AI models — all pay-per-call via the [x402 protocol](https://x402.org).
...
kaji will use the `spraay_prices` tool to fetch live onchain prices from Uniswap V3.
...
kaji will use `spraay_resolve` to resolve the ENS name, then `spraay_balances` to fetch balances.
...
kaji will use `spraay_swap_quote` to fetch a live quote from Uniswap V3.
...
kaji will use `spraay_batch_execute` to process all payments in a single transaction.
...
kaji will use `spraay_chat` to query from 200+ available AI models.

## How It Works

1. You ask kaji → kaji calls a Spraay tool (e.g. `spraay_prices`)
