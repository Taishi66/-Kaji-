...
You'll use kaji to orchestrate a full software team of subagents:
...
1. [Install kaji](/docs/getting-started/installation)
2. Within kaji, choose your working directory. It's recommended to work in a newly created directory.
3. Add the following to [.kajihints](/docs/guides/context-engineering/using-kajihints/#local-hints-file)
```plaintext
...
```
4. (Optional) Install the [kaji docs extension](/docs/mcp/kaji-docs-mcp) in case you need to ask kaji about itself

## Tasks

You'll be building an AI-powered briefing app by spinning up a team of subagents to help you. Each agent has a clear role. Your job is to figure out how to prompt kaji to delegate the work. 
...
<details>
  ...
  ```md
  You're the Planner agent for a hands-on AI app building session using kaji and subagents. We are building the MVP *right now*.
...
Spin up two developer subagents in parallel to build the core app. One will handle the Express server and backend logic, the other will build the UI and wire up the form. kaji should execute both agents at the same time, not one after the other.

- Use the file structure and API contract from the Architect
- Backend writes the API logic using Headless kaji
...
<details>
  ...
  - Use **Headless kaji** to generate the summary:
    - `kaji run -t "YOUR_PROMPT_HERE" --quiet --no-session --max-turns 1`
...
Now that development is done, spin up two final subagents: a **QA Engineer** and a **Tech Writer**. They'll work together to assess the app's quality and document how to use it. Your job is to prompt kaji in a way that gets both agents to collaborate without overlapping or duplicating work.
...
<details>
  ...
  - 🧪 A **QA Agent** who will:
    ...
    - Create a simple mock that returns fake data instead of calling the real kaji CLI
    - Assert that the response includes: `title`, `date`, and 2–3 `takeaways`
    - Include tests for:
      - Valid topic input
      - Missing or invalid input
      - kaji CLI timeout or error
...
**Important**: Run this in a **separate terminal window** (not in kaji):
```bash
npm start
```
...
- **Don't run the server in kaji** - it will get stuck
- If you need to stop it: Press `Ctrl+C` in the server terminal
- If you need kaji to fix or add something, let it know! Once it's done, restart the server
:::

**Congratulations! You've built a full-stack AI app using kaji subagents!** 🎉

:::warning
Don't expect your app to be production ready. This workshop shows how vibe coding with kaji can accelerate prototyping, but the human still owns the judgment and polish.
:::
...
### Process Timeout
**Cause**: kaji taking too long or hanging
**Solutions**: 
1. **Check your kaji command flags**:
   ```javascript
   ['run', '-t', prompt, '--quiet', '--no-session', '--max-turns', '1']
   ```
2. **Test kaji manually**:
   ```bash
   kaji run -t "Return JSON: {\"test\": \"value\"}" --quiet --no-session --max-turns 1
   ```
...
### JSON Parsing Errors
**Cause**: kaji returns formatted output with color codes
**Solutions**:
...
2. **Add debug logging** to see what you're getting:
   ```javascript
   console.log('Raw kaji response:', aiResponse);
