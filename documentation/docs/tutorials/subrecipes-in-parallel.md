...
kaji recipes can execute multiple [subrecipe](/docs/guides/recipes/subrecipes) instances concurrently using isolated worker processes. This feature enables efficient batch operations, parallel processing of different tasks, and faster completion of complex workflows.
...
Parallel subrecipe execution uses an isolated worker system that automatically manages concurrent task execution. kaji creates individual tasks for each subrecipe instance and distributes them across up to 10 concurrent workers.
...
When running different subrecipes, kaji determines the execution mode based on:
...
When running the same subrecipe with different parameters, kaji determines the execution mode based on:
...
If your prompt implies multiple executions of the same subrecipe, kaji will automatically create parallel instances:
...
In this example, kaji recognizes that "three biggest cities" requires running the weather subrecipe multiple times for different cities, so it executes them in parallel.

If you wanted to run them sequentially, you can just tell kaji:
