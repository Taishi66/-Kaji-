...
import { getKajiInstallLink } from "@site/src/utils/install-links";
...
const getExtensionCommand = (server: MCPServer): string => {
  switch (server.type) {
    case "remote":
      return `kaji session --with-remote-extension "${server.url}"`;
    case "streamable-http":
      return `kaji session --with-streamable-http-extension "${server.url}"`;
    case "local":
    default:
      return `kaji session --with-extension "${server.command}"`;
  }
};
            ...
            <div>
              ...
              <div className="py-4">
                {server.is_builtin && (
                  ...
                  >
                    Can be enabled on the Extensions page in kaji
                  </span>
                  </div>
                )}
              ...
              <div className="card-action">
                {server.is_builtin ? (
                  <div
                    ...
                    title="This extension is built into kaji and can be enabled in the settings page"
                  >
                    Built-in
                  </div>
                ) : server.show_install_link !== false ? (
                  <a
                    href={getKajiInstallLink(server)}
                    target="_blank"
