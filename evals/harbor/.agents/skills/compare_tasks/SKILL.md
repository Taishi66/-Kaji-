...
(notably `n_cache_tokens`, `n_output_tokens`, `cost_usd` on some kaji
...
harbor's fallbacks (reading kaji's `complete` event from `agent/kaji.txt`
...
- `$TRIAL_DIR/agent/<harness>.txt` — raw stream-json or log. The filename
  matches the harness (commonly `kaji.txt` or `pi.txt`; other harnesses
