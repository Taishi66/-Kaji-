# Harbor benchmark tooling for Kaji

A small command-line tool for running and comparing terminal-bench-style
benchmarks against different agent harnesses, models, and kaji builds.
...
kaji-1.30-sonnet46-full            claude-sonnet-4-6            50.6%    23.7h    2.4M       -     3k        -          45/24/2/18
kaji-sonnet46-full-code-mode       claude-sonnet-4-6            57.3%    22.0h   63.3M    1.1M     3k  $206.43          51/20/2/16
...
- `kaji-sonnet46-full-code-mode` and `sonnet46-sum_codem` (both run codemode,
  the latter also enabling summon) lead at **57.3%**.
- Stock kaji (`sonnet46-full`, `developer,todo`) lands at **50.6%**, roughly
...
- `claude-sonnet46-full` at **55.1%** is harbor's vanilla `Kaji` harness
  (curl-installed) — useful sanity check that our `KajiBinaryAgent` adapter
...
## Running a kaji benchmark

The `run` subcommand builds a harbor config that uses our `KajiBinaryAgent`
adapter — it uploads your local kaji binary into each task container,
...
./evals/harbor/cmd.py run /path/to/kaji --job-name my-run

# Different model
./evals/harbor/cmd.py run /path/to/kaji \
...
- max turns: 100
- trials: 1
- installs `libgomp1` in each container (disable with `--no-install-kaji-runtime-deps`)

Use `--dry-run` to print the generated harbor config without launching.

## Running a non-kaji harness
...
jobs_dir: /path/to/kaji/evals/harbor/runs    # so cmd.py picks it up
n_attempts: 1
...
kaji runs. `cmd.py list / show / compare` treats them identically — they're
...
```bash
# Pull everything
./evals/harbor/cmd.py pull tbench@douwe.com:/home/tbench/work/kaji
...
./evals/harbor/cmd.py pull tbench@douwe.com:/home/tbench/work/kaji --delete
```

The remote argument is `user@host:/path/to/kaji` — `pull` appends
...
cd /home/tbench/work/kaji
./evals/harbor/cmd.py run ./target/release/kaji --job-name baseline
./evals/harbor/cmd.py run ./target/release/kaji \
