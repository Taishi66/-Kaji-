"""Harbor agent that runs a caller-provided Kaji binary inside the task container."""
...
from harbor.agents.installed.kaji import Kaji
...
CONTAINER_KAJI_PATH_ROOT = "/installed-agent/kaji-profile"
CONTAINER_CONFIG_PATH = f"{CONTAINER_KAJI_PATH_ROOT}/config/config.yaml"
CONTAINER_RECIPE_PATH = "/installed-agent/harbor-recipe.yaml"
CONTAINER_CA_BUNDLE_PATH = "/installed-agent/ca-certificates.crt"

FATAL_KAJI_NOTIFICATIONS = ("creditsExhausted",)


class KajiBinaryAgent(Kaji):
    """Run a caller-provided Kaji binary in the benchmark environment.

    Differs from harbor's vanilla ``Kaji``:
    ...
    def __init__(
        ...
        *args,
        ...
        install_kaji_runtime_deps: bool = False,
        **kwargs,
    ):
        super().__init__(*args, **kwargs)
        self.kaji_binary = Path(kaji_binary).expanduser().resolve()
        self.config_yaml = config_yaml
        self.extension_entries = extension_entries
        self.install_kaji_runtime_deps = install_kaji_runtime_deps
    ...
    def get_version_command(self) -> str | None:
        return "/installed-agent/kaji --version"
    ...
    def _run_env(self) -> dict[str, str]:
        ...
        env = {
            "KAJI_MODEL": model,
            "KAJI_PROVIDER": provider,
            "KAJI_TELEMETRY_ENABLED": "false",
            "KAJI_TELEMETRY_OFF": "true",
            "CONFIGURE": "false",
            "KAJI_PATH_ROOT": CONTAINER_KAJI_PATH_ROOT,
    ...
    async def _install_kaji_runtime_deps(self, environment: BaseEnvironment) -> None:
        await self.exec_as_root(
            environment,
            command=(
                "command -v apt-get >/dev/null 2>&1 || "
                "(echo 'install_kaji_runtime_deps requires apt-get in the task container' >&2; exit 1); "
                "apt-get update && "
    ...
    async def install(self, environment: BaseEnvironment) -> None:
        if not self.kaji_binary.is_file():
            raise FileNotFoundError(f"Kaji binary does not exist: {self.kaji_binary}")

        await environment.upload_file(self.kaji_binary, "/installed-agent/kaji")
        await self.exec_as_root(environment, command="chmod 755 /installed-agent/kaji")
        if self.install_kaji_runtime_deps:
            await self._install_kaji_runtime_deps(environment)
        await self._ensure_ca_bundle(environment)

        config_dir = f"{CONTAINER_KAJI_PATH_ROOT}/config"
        await self.exec_as_root(
        ...
        await self._chown_to_agent_user(environment, CONTAINER_KAJI_PATH_ROOT, recursive=True)

        await self.exec_as_agent(
            environment,
            command=(
                "mkdir -p ~/.local/bin && "
                "ln -sf /installed-agent/kaji ~/.local/bin/kaji && "
                "~/.local/bin/kaji --version"
            ),
            env={
                "KAJI_DISABLE_KEYRING": "true",
                "KAJI_TELEMETRY_ENABLED": "false",
    ...
    async def run(
        ...
        await self.exec_as_agent(
            ...
            command=(
                ...
                f"kaji run --recipe {shlex.quote(CONTAINER_RECIPE_PATH)} "
                "--output-format stream-json "
                + ((cli_flags + " ") if cli_flags else "")
                + "2>&1 | stdbuf -oL tee /logs/agent/kaji.txt"
            ),
            env=env,
        )
        self._raise_on_fatal_kaji_notification()

    def _raise_on_fatal_kaji_notification(self) -> None:
        log_path = self.logs_dir / "kaji.txt"
        if not log_path.is_file():
            return
        log_text = log_path.read_text(errors="replace")
        for notification in FATAL_KAJI_NOTIFICATIONS:
            if f'"notificationType":"{notification}"' in log_text:
                raise NonZeroAgentExitCodeError(
                    f"Kaji exited without running the task: {notification}. "
    ...
    def populate_context_post_run(self, context: AgentContext) -> None:
        ...
        txt_path = self.logs_dir / "kaji.txt"
