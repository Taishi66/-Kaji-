...
def trial_turns(trial: TrialResult, job_dir: Path) -> int | None:
    ...
    when the trajectory isn't present:
      ...
      * kaji stream-json: count messages with role=assistant
    ...
    kaji_log = trial_dir / "agent" / "kaji.txt"
    if kaji_log.is_file():
        ...
        anon_chunks = 0
        for line in kaji_log.read_text(errors="replace").splitlines():
            line = line.strip()
...
def cmd_task(args: argparse.Namespace) -> int:
    ...
    for trial in matches:
        ...
        if trial_dir.is_dir():
            ...
            agent_log = trial_dir / "agent" / "kaji.txt"
...
def cmd_pull(args: argparse.Namespace) -> int:
    ...
    ``remote`` should be ``user@host:/path/to/kaji`` — we append
    ``/evals/harbor/runs/`` and pull into our own runs/.
    """
    remote = args.remote.rstrip("/")
    if ":" not in remote:
        print("remote must include host:path, e.g. tbench@douwe.com:/home/tbench/work/kaji", file=sys.stderr)
        return 2
