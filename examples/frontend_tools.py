...
KAJI_HOST = "127.0.0.1"
KAJI_PORT = "3001"
KAJI_URL = f"http://{KAJI_HOST}:{KAJI_PORT}"
...
async def setup_agent() -> None:
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post(
            f"{KAJI_URL}/agent/update_provider",
            json={"provider": "databricks", "model": "kaji"},
...
def submit_tool_result(tool_id: str, result: List[Dict[str, Any]]) -> None:
    """Submit the tool execution result back to Kaji.
...
async def chat_loop() -> None:
    """Main chat loop that handles the conversation with Kaji."""
