#!/usr/bin/env bash
# Tiny hook handler that loudly announces every event kaji fires at it.
#
# Kaji pipes the event payload as JSON on stdin and sets PLUGIN_ROOT in the
...
# Both stderr (so it shows up alongside kaji tracing) and a log file.
