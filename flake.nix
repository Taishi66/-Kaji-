{
  description = "kaji - An AI agent CLI";
  ...
  outputs = { self, nixpkgs, flake-utils, rust-overlay }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        cargoToml = builtins.fromTOML (builtins.readFile ./crates/kaji-cli/Cargo.toml);
      ...
      in
      {
        packages.default = pkgs.rustPlatform.buildRustPackage {
          ...
          cargoBuildFlags = [ "--package" "kaji-cli" ];
          ...
          checkPhase = ''
            ...
            # Run tests for kaji-cli package only
            cargo test --package kaji-cli --release
          '';
        ...
        devShells.default = pkgs.mkShell {
          ...
          shellHook = ''
            echo "kaji development environment"
            echo "Rust version: $(rustc --version)"
            echo ""
            echo "Commands:"
            echo "  nix build           - Build kaji CLI"
            echo "  nix run             - Run kaji CLI"
            echo "  cargo build -p kaji-cli - Build with cargo"
            echo "  cargo run -p kaji-cli   - Run with cargo"
          '';
        };
      }
    );
