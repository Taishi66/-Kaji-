...
| `OIDC_AUDIENCE` | The audience your workflow requests (e.g. `kaji-oidc-proxy`) |
...
steps:
  - name: Get OIDC token
    ...
    with:
      script: |
        const token = await core.getIDToken('kaji-oidc-proxy');
  ...
  - name: Call API through proxy
    ...
    run: kaji run --recipe my-recipe.yaml
```
