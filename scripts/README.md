# Kaji Benchmark Scripts

This directory contains scripts for running and analyzing Kaji benchmarks.

## run-benchmarks.sh

This script runs Kaji benchmarks across multiple provider:model pairs and analyzes the results.

### Prerequisites

- Kaji CLI must be built or installed
...
3. For each provider:model pair:
   - Sets the `KAJI_PROVIDER` and `KAJI_MODEL` environment variables
