# build-windows.ps1
# Build Kaji Desktop for Windows with VMware Tanzu Platform provider
# Run this script from the root of the kaji-fork repository in PowerShell
#
# Prerequisites:
...
#
# Usage:
#   cd C:\path\to\kaji-fork
#   .\scripts\build-windows.ps1

$ErrorActionPreference = "Stop"

Write-Host "=== Kaji Windows Build Script ===" -ForegroundColor Cyan
Write-Host ""
...
$kajiBinary = "target\release\kaji.exe"
if (-not (Test-Path $kajiBinary)) {
    Write-Host "Backend binary not found: $kajiBinary" -ForegroundColor Red
    exit 1
}
Copy-Item $kajiBinary "$binDir\" -Force
...
Write-Host "[5/7] Building Kaji SDK, clearing Vite cache, and compiling i18n messages..." -ForegroundColor Yellow
pnpm run build-kaji-sdk
if ($LASTEXITCODE -ne 0) {
    Write-Host "Kaji SDK build or Vite cache cleanup failed!" -ForegroundColor Red
    Pop-Location
    exit 1
}
...
# Step 5: Package
Write-Host "[6/7] Packaging Kaji Desktop..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "Packaged app:  ui\desktop\out\Kaji-win32-x64\Kaji.exe" -ForegroundColor Green
Write-Host "Installer:     ui\desktop\out\make\" -ForegroundColor Green
Write-Host ""
Write-Host "To run the app directly:" -ForegroundColor Yellow
Write-Host "  .\ui\desktop\out\Kaji-win32-x64\Kaji.exe"
Write-Host ""
