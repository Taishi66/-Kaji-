...
BACKUP_DIR="${HOME}/.local/share/kaji/kaji-db-backups"
...
MIGRATIONS_DIR="${HOME}/.local/share/kaji/migrations"
RUST_SESSION_MANAGER="crates/kaji/src/session/session_manager.rs"
...
get_kaji_db_path() {
    if [[ -n "${KAJI_PATH_ROOT:-}" ]]; then
        echo "${KAJI_PATH_ROOT}/data/sessions/sessions.db"
    else
        local possible_paths=(
            "${HOME}/.local/share/kaji/sessions/sessions.db"
            "${HOME}/Library/Application Support/Block/kaji/data/sessions/sessions.db"
        )
...
show_status() {
    echo -e "${BLUE}=== Kaji Database Status ===${NC}"
    ...
    if [[ ! -f "${DB_PATH}" ]]; then
        ...
        echo ""
        echo "This is normal if you haven't run Kaji yet."
        echo "Once you run Kaji, a database will be created automatically."
        return
    fi
...
generate_migrations() {
    if [[ ! -f "${RUST_SESSION_MANAGER}" ]]; then
        ...
        echo "Make sure you're running this from the kaji repository root."
        exit 1
    fi
...
show_help() {
    ...
    echo -e "${BLUE}Kaji Database Migration Helper${NC}"
    echo ""
    ...
    echo "    rm -rf ~/.local/share/kaji/migrations/004_*"
    echo "    $0 generate-migrations"
    echo ""
