...
REPO="${KAJI_GITHUB_REPO:-aaif-kaji/kaji}"
DEST="$HOME/Downloads"
TMPDIR=$(mktemp -d)
...
if ! curl -fSL --connect-timeout 10 -o "$TMPDIR/kaji.zip" "$DOWNLOAD_URL" 2>/dev/null; then
    ...
    gh api "repos/$REPO/actions/artifacts/$ARTIFACT_ID/zip" > "$TMPDIR/kaji.zip"
fi
echo "Done."

unzip -o -q "$TMPDIR/kaji.zip" -d "$TMPDIR/extracted"
