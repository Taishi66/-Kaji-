#!/usr/bin/env bash
# run-benchmarks.sh - Script to run kaji benchmarks across multiple provider:model pairs
...
function show_usage() {
  ...
  echo "  -t, --toolshim           Enable toolshim mode by setting KAJI_TOOLSHIM=1"
  echo "  -m, --toolshim-model     Set the toolshim model (sets KAJI_TOOLSHIM_MODEL)"
...
KAJI_CMD="kaji"
if [ "$DEBUG_MODE" = true ]; then
  if [ -f "./target/debug/kaji" ]; then
    KAJI_CMD="./target/debug/kaji"
    echo "Using debug binary: $KAJI_CMD"
  else
    echo "Warning: Debug binary not found at ./target/debug/kaji. Falling back to system-installed kaji."
  fi
else
  if [ -f "./target/release/kaji" ]; then
    KAJI_CMD="./target/release/kaji"
    echo "Using release binary: $KAJI_CMD"
  else
    echo "Warning: Release binary not found at ./target/release/kaji. Falling back to system-installed kaji."
  fi
fi
...
for ((i=0; i<$COUNT; i++)); do
  ...
  export KAJI_PROVIDER="$provider"
  export KAJI_MODEL="$model"
  
  # Set toolshim environment variables if enabled
  if [ "$TOOLSHIM" = true ]; then
    export KAJI_TOOLSHIM=1
    if [[ -n "$TOOLSHIM_MODEL" ]]; then
      export KAJI_TOOLSHIM_OLLAMA_MODEL="$TOOLSHIM_MODEL"
    fi
  fi
  ...
  if $KAJI_CMD bench --suites "$SUITES" --output "$OUTPUT_FILE" --format json; then
