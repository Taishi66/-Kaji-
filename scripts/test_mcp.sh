...
TEST_PROVIDER=${KAJI_PROVIDER:-anthropic}
TEST_MODEL=${KAJI_MODEL:-claude-haiku-4-5-20251001}
...
TMPFILE=$(mktemp)
(cd "$TESTDIR" && KAJI_PROVIDER="$TEST_PROVIDER" KAJI_MODEL="$TEST_MODEL" \
    "$KAJI_BIN" run --recipe recipe.yaml 2>&1) | tee "$TMPFILE"
