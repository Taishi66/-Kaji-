...
#
# Sets up a workdir with two named subagents on disk, runs kaji against
# several prompts, and validates that kaji delegates to the right subagent
...
#
# Knobs:
#   KAJI_PROVIDER (default: anthropic)
#   KAJI_MODEL    (default: claude-haiku-4-5)
#   SKIP_BUILD     skip cargo build (assumes target/debug/kaji already exists)
...
# won't collide with anything the user might have in ~/.agents, ~/.kaji, or
...
SCRIPT_DIR=$(pwd)
KAJI_BIN="$SCRIPT_DIR/target/debug/kaji"
export PATH="$SCRIPT_DIR/target/debug:$PATH"

export KAJI_PROVIDER="${KAJI_PROVIDER:-anthropic}"
export KAJI_MODEL="${KAJI_MODEL:-claude-haiku-4-5}"

echo "Using provider: $KAJI_PROVIDER"
echo "Using model:    $KAJI_MODEL"
echo ""
...
# ~/.agents/agents, ~/.kaji/agents, or ~/.claude/agents.
#
...
# Run kaji with a prompt in TESTDIR. We use --no-session for hermeticity.
run_kaji() {
  local prompt="$1"
  local outfile="$2"
  (cd "$TESTDIR" && "$KAJI_BIN" run --text "$prompt" --no-session 2>&1) | tee "$outfile"
}
...
You are a validator. You will be given a transcript of a kaji CLI run.
...
EOF
  ...
  local verdict
  verdict=$("$KAJI_BIN" run --text "$judge_prompt" --no-session 2>&1)
...
TMP1=$(mktemp)
run_kaji "@janpier which of your animals can speak?" "$TMP1"
...
TMP2=$(mktemp)
run_kaji "Ask janpier what tricks his animals can do." "$TMP2"
...
TMP3=$(mktemp)
run_kaji "Write me a hello world program in the Forth programming language." "$TMP3"
...
if grep -qE "▸.*delegate" "$TMP3" && grep -qE "^\s*source[[:space:]]+peterjoris\b" "$TMP3"; then
  ...
  assert_judge "$TMP3" \
    "The user asked kaji to write a Hello World program in the Forth programming language. The session had a registered subagent named 'peterjoris' described as a Forth expert. Does the transcript show ANY of: (a) kaji called the delegate tool with source 'peterjoris', or (b) kaji's reply mentions peterjoris (or 'the Forth expert') as the right specialist for this task, or (c) kaji produced syntactically plausible Forth code as the answer? ANY of (a), (b), (c) counts as PASS. Only FAIL if none of those apply." \
...
TMP4=$(mktemp)
run_kaji "What is 2 + 2? Reply with just the digit." "$TMP4"
...
EMPTYDIR=$(mktemp -d)
TMP5=$(mktemp)
(cd "$EMPTYDIR" && "$KAJI_BIN" run --text "@janpier where is the treasure?" --no-session 2>&1) | tee "$TMP5"
