...
describe('applyKajiSessionNotification', () => {
  describe('message_usage', () => {
    it('attaches usage to the message matching messageId', () => {
      const state = makeState();

      const changes = applyKajiSessionNotification(state, messageUsageNotification('a1'));
    ...
    it.each([
      ...
      const changes = applyKajiSessionNotification(state, messageUsageNotification(messageId));
    ...
    it('skips client-made approval rows when falling back', () => {
      ...
      const changes = applyKajiSessionNotification(state, messageUsageNotification('missing'));
  ...
  describe('usage_update', () => {
    it('still maps usage updates into token state', () => {
      const changes = applyKajiSessionNotification(
        makeState(),
        kajiUpdate({
