import type { AgentMention, AvailableCommand } from '@aaif/kaji-sdk';
...
export async function listSlashCommandItems(cwd: string): Promise<AutocompleteDisplayItem[]> {
  ...
  const response = await client.kaji.slashCommandsList_unstable(cwdParam(cwd));
...
export async function listAgentMentionItems(
  ...
  const client = await getAcpClient();
  const response = await client.kaji.agentMentionsList_unstable({
    ...cwdParam(cwd),
    ...(sessionId ? { sessionId } : {}),
  });
