...
if [ -n "${KAJI_PATH_ROOT:-}" ]; then
    RESOLVED_KAJI_CONFIG_DIR="${KAJI_PATH_ROOT}/config"
elif [ -n "${KAJI_CONFIG_DIR:-}" ]; then
    log "KAJI_CONFIG_DIR is deprecated for desktop shims; prefer KAJI_PATH_ROOT."
    RESOLVED_KAJI_CONFIG_DIR="${KAJI_CONFIG_DIR}"
else
    RESOLVED_KAJI_CONFIG_DIR="${HOME}/.config/kaji"
fi
MCP_HERMIT_DIR="${RESOLVED_KAJI_CONFIG_DIR}/mcp-hermit"
mkdir -p "${RESOLVED_KAJI_CONFIG_DIR}"
HERMIT_SETUP_LOCK_DIR="${RESOLVED_KAJI_CONFIG_DIR}/.mcp-hermit-setup.lock"
...
CLEANUP_MARKER="${RESOLVED_KAJI_CONFIG_DIR}/.mcp-hermit-cleanup-v1"
...
log "Checking for KAJI_NPM_REGISTRY and KAJI_NPM_CERT environment variables for custom npm registry setup..."
# Check if KAJI_NPM_REGISTRY is set and accessible
if [ -n "${KAJI_NPM_REGISTRY:-}" ] && curl -s --head --fail "${KAJI_NPM_REGISTRY}" > /dev/null; then
    log "Checking custom kaji registry availability: ${KAJI_NPM_REGISTRY}"
    log "${KAJI_NPM_REGISTRY} is accessible. Using it for npm registry."
    export NPM_CONFIG_REGISTRY="${KAJI_NPM_REGISTRY}"

    # Check if KAJI_NPM_CERT is set and accessible
    if [ -n "${KAJI_NPM_CERT:-}" ] && curl -s --head --fail "${KAJI_NPM_CERT}" > /dev/null; then
        log "Downloading certificate from: ${KAJI_NPM_CERT}"
        curl -sSL -o "${MCP_HERMIT_DIR}/cert.pem" "${KAJI_NPM_CERT}"
        if [ $? -eq 0 ]; then
        ...
        log "KAJI_NPM_CERT is either not set or not accessible. Skipping certificate setup."
    fi

else
    log "KAJI_NPM_REGISTRY is either not set or not accessible. Falling back to default npm registry."
