...
describe('ExtensionInstallModal', () => {
  ...
  describe('Extension Request Handling', () => {
    it('should handle trusted extension (default behaviour, no allowlist)', async () => {
      ...
      await act(async () => {
        await eventHandler({}, 'kaji://extension?cmd=npx&arg=test-extension&name=TestExt');
      });
    ...
    it('should handle trusted extension (from allowlist)', async () => {
      ...
      await act(async () => {
        await eventHandler({}, 'kaji://extension?cmd=npx&arg=test-extension&name=AllowedExt');
      });
    ...
    it('should handle warning mode', async () => {
      ...
      await act(async () => {
        await eventHandler(
          {},
          'kaji://extension?cmd=npx&arg=untrusted-extension&name=UntrustedExt'
        );
      });
    ...
    it('should handle blocked extension', async () => {
      ...
      await act(async () => {
        await eventHandler({}, 'kaji://extension?cmd=npx&arg=blocked-extension&name=BlockedExt');
      });
  ...
  describe('Modal Actions', () => {
    it('should dismiss modal correctly', async () => {
      ...
      await act(async () => {
        await eventHandler({}, 'kaji://extension?cmd=npx&arg=test&name=Test');
      });
    ...
    it('should handle successful extension installation', async () => {
      ...
      expect(addExtensionFromDeepLink).toHaveBeenCalledWith(
        'kaji://extension?cmd=npx&arg=test&name=Test',
        mockAddExtension,
        expect.any(Function)
      );
