...
export const ModelAndProviderProvider: React.FC<ModelAndProviderProviderProps> = ({ children }) => {
  ...
  const getFallbackModelAndProvider = useCallback(async () => {
    const provider = window.appConfig.get('KAJI_DEFAULT_PROVIDER') as string;
    const model = window.appConfig.get('KAJI_DEFAULT_MODEL') as string;
    if (provider && model) {
      try {
  ...
  const getCurrentModelAndProviderForDisplay = useCallback(async () => {
    ...
    const kajiModel = modelProvider.model;
    const kajiProvider = modelProvider.provider;

    // lookup display name
    let metadata: ProviderMetadata;

    try {
      metadata = await getProviderMetadata(String(kajiProvider));
    } catch {
      return { model: kajiModel, provider: kajiProvider };
    }
    const providerDisplayName = metadata.display_name;

    return { model: kajiModel, provider: providerDisplayName };
