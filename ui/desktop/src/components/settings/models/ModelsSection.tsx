  ...
  const loadModelData = useCallback(async () => {
    try {
      ...
      if (providerDisplayName) {
        ...
        const { providerId: kajiProvider } = await acpReadDefaults();
        const providers = await acpListProviderDetails();
        const providerDetailsList = providers.filter((provider) => provider.name === kajiProvider);
