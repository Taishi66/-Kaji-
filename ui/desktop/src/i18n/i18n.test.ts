...
describe('getLocale', () => {
  ...
  it('respects KAJI_LOCALE over navigator.languages', () => {
    mockAppConfig({ KAJI_LOCALE: 'en' });
    vi.stubGlobal('navigator', { languages: ['xx-XX'] });
    expect(getLocale()).toEqual({ locale: 'en', messageLocale: 'en' });
  });

  it('preserves regional tag from KAJI_LOCALE', () => {
    mockAppConfig({ KAJI_LOCALE: 'en-GB' });
    vi.stubGlobal('navigator', { languages: ['xx-XX'] });
  ...
  it('supports explicit Korean locale', () => {
    mockAppConfig({ KAJI_LOCALE: 'ko' });
    vi.stubGlobal('navigator', { languages: ['xx-XX'] });
    expect(getLocale()).toEqual({ locale: 'ko', messageLocale: 'ko' });
  });

  it('supports POSIX-style Korean locale from KAJI_LOCALE', () => {
    mockAppConfig({ KAJI_LOCALE: 'ko_KR' });
    vi.stubGlobal('navigator', { languages: ['xx-XX'] });
    expect(getLocale()).toEqual({ locale: 'ko-KR', messageLocale: 'ko' });
  });
  ...
  it('supports POSIX-style Japanese locale from KAJI_LOCALE', () => {
    mockAppConfig({ KAJI_LOCALE: 'ja_JP' });
    vi.stubGlobal('navigator', { languages: ['xx-XX'] });
    expect(getLocale()).toEqual({ locale: 'ja-JP', messageLocale: 'ja' });
  });
