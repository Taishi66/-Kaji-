...
import { buildLocalServeUrls, findKajiBinaryPath, startKajiServe } from './kajiServe';
...
function makeTempDir(): string {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'kaji-serve-test-'));
...
describe('findKajiBinaryPath', () => {
  ...
  it('uses KAJI_BINARY in development builds', () => {
    const tempDir = makeTempDir();
    const overridePath = makeFile(path.join(tempDir, 'override-kaji'));
    vi.stubEnv('KAJI_BINARY', overridePath);

    expect(findKajiBinaryPath({ isPackaged: false })).toBe(overridePath);
  });

  it('rejects KAJI_BINARY in packaged builds', () => {
    const tempDir = makeTempDir();
    ...
    expect(() => findKajiBinaryPath({ isPackaged: true, resourcesPath })).toThrow(
      'KAJI_BINARY is only supported in development builds'
    );
  });
  ...
  it('prefers the staged binary over target builds in development builds', () => {
    ...
    const resolvedPath = findKajiBinaryPath({ isPackaged: false });
  ...
  it('uses the bundled kaji binary in packaged builds', () => {
    const tempDir = makeTempDir();
    const resourcesPath = path.join(tempDir, 'resources');
    const bundledPath = makeFile(path.join(resourcesPath, 'bin', binaryName));

    expect(findKajiBinaryPath({ isPackaged: true, resourcesPath })).toBe(bundledPath);
  });
});
...
describe('startKajiServe', () => {
  ...
  it.skipIf(process.platform === 'win32')('uses the injected readiness fetch', async () => {
    const tempDir = makeTempDir();
    const kajiPath = makeExecutable(
      path.join(tempDir, 'kaji'),
      '#!/usr/bin/env sh\nwhile true; do sleep 1; done\n'
    );
    vi.stubEnv('KAJI_BINARY', kajiPath);
    ...
    const result = await startKajiServe({
  ...
  it.skipIf(process.platform === 'win32')('captures the TLS fingerprint from stdout', async () => {
    ...
    const kajiPath = makeExecutable(
      ...
      [
        '#!/usr/bin/env sh',
        'printf "KAJID_CERT_FINGERPRINT=AA:BB:CC\\n"',
        'while true; do sleep 1; done',
        '',
      ].join('\n')
    );
  ...
  it.skipIf(process.platform === 'win32')('uses TLS URLs and args when TLS is enabled', async () => {
    ...
    const kajiPath = makeExecutable(
      ...
      [
        '#!/usr/bin/env sh',
        'printf "%s\\n" "$@" > "$TEST_ARGS_PATH"',
        'printf "KAJID_CERT_FINGERPRINT=DD:EE:FF\\n"',
        'while true; do sleep 1; done',
        '',
      ].join('\n')
    );
  ...
  it.skipIf(process.platform === 'win32')('waits for TLS fingerprint after readiness succeeds', async () => {
    ...
    const kajiPath = makeExecutable(
      ...
      [
        '#!/usr/bin/env sh',
        'sleep 0.1',
        'printf "KAJID_CERT_FINGERPRINT=11:22:33\\n"',
        'while true; do sleep 1; done',
        '',
      ].join('\n')
    );
