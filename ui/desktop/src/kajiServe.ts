...
import {
  ...
  createKajiServeStartupDiagnostics,
  type KajiServeStartupDiagnostics,
...
export const defaultLogger: Logger = {
  info: (...args) => console.log('[kaji-serve]', ...args),
  error: (...args) => console.error('[kaji-serve]', ...args),
};

export interface FindKajiBinaryOptions {
  isPackaged?: boolean;
  resourcesPath?: string;
}

type ReadinessFetchInit = Parameters<typeof globalThis.fetch>[1];
export type KajiServeExitSignal = ChildProcess['signalCode'];
type ReadinessFetch = (input: string, init?: ReadinessFetchInit) => Promise<Response>;

export interface StartKajiServeOptions extends FindKajiBinaryOptions {
  dir?: string;
  serverSecret: string;
  tls?: boolean;
  env?: Record<string, string | undefined>;
  /** PATH from the user's login shell, appended so kajid can find CLI providers. */
...
export interface KajiServeResult {
  acpUrl: string;
  workingDir: string;
  ...
  hasExited: () => boolean;
  getExitDetails: () => { code: number | null; signal: KajiServeExitSignal };
  startupDiagnosticsPath: string | null;
  getStartupDiagnostics: () => KajiServeStartupDiagnostics | null;
...
export const findKajiBinaryPath = (options: FindKajiBinaryOptions = {}): string => {
  const { isPackaged = false, resourcesPath } = options;
  const pathFromEnv = process.env.KAJI_BINARY;
  if (pathFromEnv) {
    if (isPackaged) {
      throw new Error('KAJI_BINARY is only supported in development builds');
    }
    ...
    throw new Error(`Invalid KAJI_BINARY path: ${pathFromEnv} (pwd is ${process.cwd()})`);
  }

  const binaryName = process.platform === 'win32' ? 'kaji.exe' : 'kaji';
  ...
  throw new Error(
    `Kaji binary not found in any of the possible paths: ${possiblePaths.join(', ')}`
  );
};
...
const CERT_FINGERPRINT_PREFIX = 'KAJID_CERT_FINGERPRINT=';
...
const waitForKajiServeReady = async (
  statusUrl: string,
  errorLog: string[],
...
const buildKajiServeEnv = (
  ...
  env.KAJI_SERVER__SECRET_KEY = serverSecret;

  return env;
};

export const startKajiServe = async ({
  dir,
  serverSecret,
  tls = false,
  env: additionalEnv = {},
...
}: StartKajiServeOptions): Promise<KajiServeResult> => {
  const workingDir = dir || process.cwd();
  const startupTrace = createKajiServeStartupDiagnostics(diagnosticsDir, workingDir);
  const startupDiagnosticsPath = startupTrace?.diagnosticsPath ?? null;
  const secretKey = serverSecret.trim();
  if (!secretKey) {
    const message = 'KAJI_SERVER__SECRET_KEY is required for kaji serve';
  ...
  let kajiPath: string;
  try {
    kajiPath = findKajiBinaryPath({ isPackaged, resourcesPath });
  } catch (error) {
  ...
  logger.info(`Starting kaji serve from: ${kajiPath} on port ${port} in dir ${workingDir}`);
  if (startupTrace) {
    startupTrace.diagnostics.binaryPath = kajiPath;
  ...
  const spawnOptions = {
    env: buildKajiServeEnv(secretKey, kajiPath, additionalEnv, loginShellPath),
    cwd: workingDir,
    windowsHide: true,
  ...
  const kajiProcess = spawn(kajiPath, args, spawnOptions);
  if (startupTrace) {
    startupTrace.diagnostics.pid = kajiProcess.pid ?? null;
    startupTrace.record('spawn_success', { pid: kajiProcess.pid ?? null });
  }

  let exited = false;
  let spawnFailed = false;
  let exitCode: number | null = null;
  let exitSignal: KajiServeExitSignal = null;
  ...
  const stopStdoutCollection = () => {
    ...
    kajiProcess.stdout?.off('data', onStdoutData);
    kajiProcess.stdout?.resume();
  };
  ...
  kajiProcess.stdout?.on('data', onStdoutData);
  ...
  const onStderrData = (data: Buffer) => {
    ...
    for (const line of lines) {
      if (line.trim() && isFatalError(line)) {
        logger.error(`kaji serve stderr for port ${port} and dir ${workingDir}: ${line}`);
      }
    }
  };

  kajiProcess.stderr?.on('data', onStderrData);

  kajiProcess.on('exit', (code, signal) => {
    exited = true;
    exitCode = code;
    exitSignal = signal;
    logger.info(
      `kaji serve process exited with code ${code} and signal ${signal} for port ${port} and dir ${workingDir}`
    );
    if (startupTrace) {
  ...
  kajiProcess.on('error', (error) => {
    spawnFailed = true;
    errorLog.push(error.message);
    logger.error(`Failed to start kaji serve on port ${port} and dir ${workingDir}`, error);
    startupTrace?.record('spawn_error', { message: error.message, name: error.name });
  });

  const cleanup = async (): Promise<void> => {
    return new Promise<void>((resolve) => {
      if (exited || kajiProcess.killed) {
        resolve();
        return;
      }
      ...
      kajiProcess.once('close', finish);

      logger.info('Terminating kaji serve');
      try {
        if (process.platform === 'win32') {
          if (kajiProcess.pid) {
            spawn('taskkill', ['/pid', kajiProcess.pid.toString(), '/f', '/t']);
          }
        } else {
          kajiProcess.kill('SIGTERM');
        }
      } catch (error) {
        logger.error('Error while terminating kaji serve process:', error);
      }

      setTimeout(() => {
        if (!exited && !kajiProcess.killed && process.platform !== 'win32') {
          kajiProcess.kill('SIGKILL');
        }
        finish();
      }, 5000);
    });
  ...
  const ready = await waitForKajiServeReady(statusUrl, errorLog, () => exited || spawnFailed, {
    healthUrl,
    readinessFetch,
  ...
  const stopOutputCollection = () => {
    stopStdoutCollection();
    kajiProcess.stderr?.off('data', onStderrData);
    kajiProcess.stderr?.resume();
  };
  ...
  if (!ready) {
    ...
    throw new Error(
      withStartupDiagnosticsPath(
        `kaji serve did not become ready on ${statusUrl}.${exitDetails}${stderrDetails}`,
  ...
  if (tls) {
    ...
    if (!fingerprint) {
      ...
      throw new Error(
        withStartupDiagnosticsPath(
          `kaji serve did not emit TLS certificate fingerprint on ${statusUrl}.${exitDetails}${stderrDetails}`,
          startupDiagnosticsPath
        )
      );
    }
