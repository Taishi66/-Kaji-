...
import type { KajiServeResult, Logger } from './kajiServe';
import {
  KAJI_SERVE_EXITED_USER_MESSAGE,
  KajiServeLeaseRegistry,
} from './kajiServeLeaseRegistry';
...
function createKajiServeResult(
  overrides: Partial<Pick<KajiServeResult, 'cleanup' | 'hasExited' | 'getExitDetails'>> = {}
): KajiServeResult {
  return {
    acpUrl: 'ws://127.0.0.1:1234/acp?token=test',
    workingDir: '/tmp',
    process: new EventEmitter() as KajiServeResult['process'],
    errorLog: [],
...
describe('KajiServeLeaseRegistry', () => {
  it('returns the ACP URL for an attached live lease', () => {
    const store = new KajiServeLeaseRegistry(createLogger());
    const lease = store.create(createKajiServeResult(), 'local-secret');
  ...
  it('throws a recovery message after the process exits', () => {
    ...
    const store = new KajiServeLeaseRegistry(logger);
    const result = createKajiServeResult();
    const lease = store.create(result, 'local-secret');
    store.attachWindow(1, lease);

    result.process.emit('exit', 1, null);

    expect(() => store.getAcpUrl(1)).toThrow(KAJI_SERVE_EXITED_USER_MESSAGE);
    expect(() => store.getSecretKey(1)).toThrow(KAJI_SERVE_EXITED_USER_MESSAGE);
    expect(logger.error).toHaveBeenCalledWith(
      'Kaji ACP server exited unexpectedly',
  ...
  it('cleans up once after the last attached window is released', async () => {
    ...
    const store = new KajiServeLeaseRegistry(createLogger());
    const lease = store.create(createKajiServeResult({ cleanup }), 'local-secret');
    store.attachWindow(1, lease);
  ...
  it('creates an external ACP lease without process cleanup', async () => {
    const store = new KajiServeLeaseRegistry(createLogger());
    const lease = store.createExternal('wss://example.com/kaji/acp?token=test', 'external-secret');

    store.attachWindow(1, lease);

    expect(store.getAcpUrl(1)).toBe('wss://example.com/kaji/acp?token=test');
  ...
  it('cleans up external leases after the last attached window is released', async () => {
    ...
    const lease = store.createExternal(
      'wss://example.com/kaji/acp?token=test',
      'external-secret',
      cleanup
    );
    store.attachWindow(1, lease);
