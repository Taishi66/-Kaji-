...
export async function generateDeepLink(recipe: Recipe): Promise<string> {
  ...
  return `kaji://recipe?config=${encoded}`;
}
...
export async function parseDeeplink(deeplink: string): Promise<Recipe | null> {
  try {
    ...
    if (!cleanLink.startsWith('kaji://recipe?config=')) {
      throw new Error('Invalid deeplink format. Expected: kaji://recipe?config=...');
    }

    const recipeEncoded = cleanLink.replace('kaji://recipe?config=', '');
