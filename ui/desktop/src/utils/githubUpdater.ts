...
export class GitHubUpdater {
  private readonly owner = process.env.GITHUB_OWNER || 'aaif-kaji';
  private readonly repo = process.env.GITHUB_REPO || 'kaji';
  private readonly bundleName = process.env.KAJI_BUNDLE_NAME || 'Kaji';
    ...
    try {
      ...
      const response = await fetch(this.apiUrl, {
        headers: {
          ...
          'User-Agent': `Kaji-Desktop/${app.getVersion()}`,
        },
      ...
      const asset = release.assets.find((a) => a.name.toLowerCase() === assetName.toLowerCase()); // keeping comparison to lowercase because Kaji vs kaji
      if (asset) {
