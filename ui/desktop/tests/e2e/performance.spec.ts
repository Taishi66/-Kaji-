...
test.describe('Performance Tests', () => {
  test('measure end-to-end performance for prompt submission', async ({ kajiPage }) => {
    // Start Playwright tracing to capture all performance data
    await kajiPage.context().tracing.start({
      screenshots: true,
      snapshots: true,
      sources: true
    });
    ...
    await kajiPage.evaluate(() => performance.mark('app-ready'));
    ...
    // Mark: Prompt submit
    await kajiPage.evaluate(() => performance.mark('prompt-submit-start'));
    await chatInput.press('Enter');
    await kajiPage.evaluate(() => performance.mark('prompt-submitted'));
    ...
    const loadingText = await kajiPage.locator('[data-testid="loading-indicator"]').textContent();
    if (loadingText?.includes('loading conversation')) {
      await kajiPage.evaluate(() => performance.mark('loading-conversation-start'));
      console.log('✓ Loading conversation detected');

      // Wait for it to change or disappear
      await kajiPage.waitForFunction(() => {
      ...
      await kajiPage.evaluate(() => performance.mark('loading-conversation-end'));
      console.log('✓ Loading conversation complete');
    }

    await kajiPage.evaluate(() => performance.mark('loading-started'));
    ...
    const checkForFirstToken = async () => {
      while (!firstTokenDetected) {
        try {
          const messageContainers = await kajiPage.locator('[data-testid="message-container"]').all();
          if (messageContainers.length > 0) {
            const lastMessage = messageContainers[messageContainers.length - 1];
            const content = await lastMessage.textContent();
            if (content && content.trim().length > 0) {
              await kajiPage.evaluate(() => performance.mark('first-token-received'));
        ...
        await kajiPage.waitForTimeout(50);
      }
    };
    ...
    await kajiPage.evaluate(() => performance.mark('response-complete'));
    console.log('✓ Response complete');

    // Create performance measures
    await kajiPage.evaluate(() => {
    ...
    const metrics = await kajiPage.evaluate(() => {
    ...
    const response = await kajiPage.locator('[data-testid="message-container"]').last();
    ...
    await kajiPage.context().tracing.stop({ path: tracePath });
  ...
  test('measure cold start vs warm cache performance', async ({ kajiPage }) => {
    await kajiPage.context().tracing.start({ screenshots: true, snapshots: true });
    ...
    await kajiPage.evaluate(() => performance.mark('cold-prompt-start'));
    await chatInput.press('Enter');
    ...
    // Second prompt (warm)
    const chatInput2 = await kajiPage.waitForSelector('[data-testid="chat-input"]');
    await chatInput2.fill('Say goodbye');

    await kajiPage.evaluate(() => performance.mark('warm-prompt-start'));
  ...
  test('capture full performance profile with navigation timing', async ({ kajiPage }) => {
    await kajiPage.context().tracing.start({ screenshots: true, snapshots: true });

    console.log('\n=== Full Performance Profile ===\n');

    // Get navigation timing
    const navigationTiming = await kajiPage.evaluate(() => {
    ...
    // Wait for app ready
    await kajiPage.waitForSelector('[data-testid="chat-input"]', { timeout: 30000 });
    await kajiPage.evaluate(() => performance.mark('app-interactive'));

    // Measure time from navigation to interactive
    const appReadyTime = await kajiPage.evaluate(() => {
    ...
    await kajiPage.evaluate(() => performance.mark('user-interaction-start'));
    await chatInput.press('Enter');

    await kajiPage.waitForSelector('[data-testid="loading-indicator"]', { state: 'visible', timeout: 5000 });
    await kajiPage.waitForSelector('[data-testid="loading-indicator"]', { state: 'hidden', timeout: 60000 });
    ...
    const interactionTime = await kajiPage.evaluate(() => {
    ...
    const resourceStats = await kajiPage.evaluate(() => {
