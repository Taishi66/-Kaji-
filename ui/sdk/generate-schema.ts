#!/usr/bin/env node
/**
 * Generates TypeScript types + Zod validators for Kaji custom extension methods.
 *
 * Usage:
...
const SCHEMA_PATH = resolve(ROOT, "crates/kaji/acp-schema.json");
const META_PATH = resolve(ROOT, "crates/kaji/acp-meta.json");
...
export default async function main() {
  ...
  console.log(`\nGenerated Kaji extension schema in ${OUTPUT_DIR}`);
}
...
async function postProcessIndex(meta: {
  ...
  const methodConstants = await prettier.format(
    `
export const KAJI_EXT_METHODS = ${JSON.stringify(meta.methods, null, 2)} as const;

export type KajiExtMethod = (typeof KAJI_EXT_METHODS)[number];

export const KAJI_EXT_NOTIFICATIONS = ${JSON.stringify(meta.notifications ?? [], null, 2)} as const;

export type KajiExtNotification = (typeof KAJI_EXT_NOTIFICATIONS)[number];

export const KAJI_EXT_AGENT_REQUESTS = ${JSON.stringify(meta.agentRequests ?? [], null, 2)} as const;

export type KajiExtAgentRequest = (typeof KAJI_EXT_AGENT_REQUESTS)[number];
`,
    { parser: "typescript" },
  );
...
function methodToHandlerName(method: string): string {
  ...
  let prefix = "";
  if (methodParts[0] == "kaji" && methodParts[1] == "unstable") {
    methodParts.shift();
    methodParts.shift();
    prefix = "unstable_";
  } else if (methodParts[0] == "kaji") {
    methodParts.shift();
  }
...
async function generateClient(meta: {
  ...
  const handlersInterface = `export interface KajiExtNotifications {
${handlerFields.join("\n")}
}`;

  const agentRequestsInterface = `export interface KajiExtAgentRequests {
${agentRequestHandlerFields.join("\n")}
}`;

  const agentRequestDispatcherFn = `export function installKajiExtAgentRequestDispatcher(
...
${agentRequestDispatchCases.join("\n")}
  ...
  const dispatcherFn = `export function installKajiExtNotificationDispatcher(
  callbacks: KajiClientCallbacks,
): Client {
...
export type KajiClientCallbacks =
