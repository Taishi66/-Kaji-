#!/usr/bin/env node
/**
 * Builds the kaji binary for target platforms and places them
 * into the corresponding npm package directories under ui/kaji-binary/.
 *
 * Usage:
...
const NATIVE_DIR = resolve(ROOT, "ui/kaji-binary");
...
function buildTarget(platform: string): void {
  ...
  const pkgDir = resolve(NATIVE_DIR, `kaji-binary-${platform}`);
  const binDir = resolve(pkgDir, "bin");

  console.log(`==> Building kaji for ${platform} (${rustTarget})`);
  ...
  try {
    const featureArgs = platform.startsWith("linux-")
      ...
      : "";
    execSync(
      `cargo build --release --target ${rustTarget} --bin kaji${featureArgs}`,
      {
        cwd: ROOT,
        stdio: "inherit",
      },
...
async function main() {
  ...
  console.log("==> Done. Native packages staged in ui/kaji-binary/");
}
